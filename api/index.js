  ...
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc30 = __getOwnPropDesc(from, key)) || desc30.enumerable });
  }
  return to;
};
...
function fingerprintGenome(root, minimumScore) {
  return (0, import_node_crypto2.createHash)("sha256").update(canonicalizeGenome(root, minimumScore)).digest("hex");
}
...
function fingerprintResearchGenome(input) {
  return (0, import_node_crypto2.createHash)("sha256").update(JSON.stringify({ genome: canonicalGene(input.root), minimumScore: input.minimumScore, datasetVersionKey: input.datasetVersionKey, assumptions: canonicalContext(input.assumptions) })).digest("hex");
}
...
var environmentEnum = (0, import_pg_core.pgEnum)("environment", ["mock", "live"]);
var connectionStatusEnum = (0, import_pg_core.pgEnum)("connectionStatus", ["connected", "failed"]);
var autoTradePolicyStatusEnum = (0, import_pg_core.pgEnum)("autoTradePolicyStatus", ["active", "superseded", "paused"]);
var visibilityEnum = (0, import_pg_core.pgEnum)("visibility", ["public", "hidden"]);
var timeframeEnum = (0, import_pg_core.pgEnum)("timeframe", ["daily", "five_minute"]);
var adjustmentBasisEnum = (0, import_pg_core.pgEnum)("adjustmentBasis", ["adjusted", "unadjusted", "unknown"]);
var qualityStatusEnum = (0, import_pg_core.pgEnum)("qualityStatus", ["draft", "collecting", "ready", "error"]);
var sourceEnum = (0, import_pg_core.pgEnum)("source", ["kiwoom_daily", "kiwoom_daily_five_minute"]);
var originEnum = (0, import_pg_core.pgEnum)("origin", ["seed", "elite", "crossover", "mutation", "manual_expand"]);
var candidateStatusEnum = (0, import_pg_core.pgEnum)("candidateStatus", ["created", "evaluated", "survived", "rejected", "failed"]);
var phaseEnum = (0, import_pg_core.pgEnum)("phase", ["preparing", "opening", "intraday", "closing", "completed", "waiting_for_data", "incomplete", "failed"]);
var dataStatusEnum = (0, import_pg_core.pgEnum)("dataStatus", ["pending", "ready", "waiting", "incomplete", "error"]);
var channelEnum = (0, import_pg_core.pgEnum)("channel", ["intraday_price"]);
var sideEnum = (0, import_pg_core.pgEnum)("side", ["buy", "sell"]);
var orderTypeEnum = (0, import_pg_core.pgEnum)("orderType", ["market", "limit"]);
var orderStatusEnum = (0, import_pg_core.pgEnum)("orderStatus", ["pending_confirmation", "confirmed", "submitting", "blocked", "submitted", "filled", "rejected", "cancelled"]);
var executionOriginEnum = (0, import_pg_core.pgEnum)("executionOrigin", ["manual", "local_node"]);
var eventTypeEnum = (0, import_pg_core.pgEnum)("eventType", ["entry", "mark", "exit"]);
var regimeEnum = (0, import_pg_core.pgEnum)("regime", ["trend_up", "trend_down", "range", "volatile"]);
var scopeEnum = (0, import_pg_core.pgEnum)("scope", ["stored_daily_bars", "external_verification"]);
var trackingStatusEnum = (0, import_pg_core.pgEnum)("trackingStatus", ["tracking", "closed", "cash_only"]);
var minuteResearchStatusEnum = (0, import_pg_core.pgEnum)("minuteResearchStatus", ["evaluated", "promoted", "rejected", "insufficient_validation", "failed"]);
var governanceStatusEnum = (0, import_pg_core.pgEnum)("governanceStatus", ["running", "completed", "failed", "skipped"]);
var rankingRefreshStatusEnum = (0, import_pg_core.pgEnum)("rankingRefreshStatus", ["idle", "running", "ready", "error", "paused"]);
var experimentStatusEnum = (0, import_pg_core.pgEnum)("experimentStatus", ["draft", "queued", "running", "completed", "failed"]);
var evolutionSearchStatusEnum = (0, import_pg_core.pgEnum)("evolutionSearchStatus", ["draft", "queued", "running", "completed", "failed", "cancelled"]);
var generationStatusEnum = (0, import_pg_core.pgEnum)("generationStatus", ["queued", "generating", "evaluating", "completed", "failed"]);
var autonomousPhaseEnum = (0, import_pg_core.pgEnum)("autonomousPhase", ["preparing", "opening", "intraday", "closing", "completed", "waiting_for_data", "incomplete", "failed"]);
var autonomousTaskStatusEnum = (0, import_pg_core.pgEnum)("autonomousTaskStatus", ["running", "completed", "waiting_for_data", "failed"]);
var connectionCheckStatusEnum = (0, import_pg_core.pgEnum)("connectionCheckStatus", ["connected", "failed"]);
var survivalStatusEnum = (0, import_pg_core.pgEnum)("survivalStatus", ["promoted", "observe", "rejected"]);
var collectionRequestStatusEnum = (0, import_pg_core.pgEnum)("collectionRequestStatus", ["queued", "running", "completed", "failed", "cancelled"]);
var dayTradeStatusEnum = (0, import_pg_core.pgEnum)("dayTradeStatus", ["tracking", "closed"]);
var syncEventStatusEnum = (0, import_pg_core.pgEnum)("syncEventStatus", ["success", "partial", "failed"]);
var backtestStatusEnum = (0, import_pg_core.pgEnum)("backtestStatus", ["queued", "running", "completed", "failed"]);
var portfolioStatusEnum = (0, import_pg_core.pgEnum)("portfolioStatus", ["active", "closed"]);
var positionStatusEnum = (0, import_pg_core.pgEnum)("positionStatus", ["open", "closed"]);
var minuteProgramStatusEnum = (0, import_pg_core.pgEnum)("minuteProgramStatus", ["active", "paused"]);
var sweepStatusEnum = (0, import_pg_core.pgEnum)("sweepStatus", ["queued", "running", "completed", "waiting_for_data", "failed"]);
var committeeStatusEnum = (0, import_pg_core.pgEnum)("committeeStatus", ["running", "completed", "failed"]);
var revalidationStatusEnum = (0, import_pg_core.pgEnum)("revalidationStatus", ["queued", "running", "completed", "blocked", "failed"]);
var users = (0, import_pg_core.pgTable)("users", {
  ...
  avatarId: (0, import_pg_core.varchar)("avatarId", { length: 32 }).default("nebula").notNull(),
...
var tradingProfiles = (0, import_pg_core.pgTable)(
  "trading_profiles",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    environment: environmentEnum("environment").default("mock").notNull(),
    accountNumberMasked: (0, import_pg_core.varchar)("accountNumberMasked", { length: 32 }),
    tokenExpiresAt: (0, import_pg_core.timestamp)("tokenExpiresAt"),
    connectionStatus: connectionStatusEnum("connectionStatus").default("failed").notNull(),
    refreshIntervalSeconds: (0, import_pg_core.integer)("refreshIntervalSeconds").default(60).notNull(),
    maxBuyAmount: (0, import_pg_core.integer)("maxBuyAmount").default(5e5).notNull(),
    dailyTradeLimit: (0, import_pg_core.integer)("dailyTradeLimit").default(3).notNull(),
    killSwitch: (0, import_pg_core.boolean)("killSwitch").default(true).notNull(),
    autoTradeEnabled: (0, import_pg_core.boolean)("autoTradeEnabled").default(false).notNull(),
    requireConfirmation: (0, import_pg_core.boolean)("requireConfirmation").default(true).notNull(),
    scheduleCronTaskUid: (0, import_pg_core.varchar)("scheduleCronTaskUid", { length: 65 }),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("trading_profiles_user_unique").on(table.userId)]
);
var autoTradePolicies = (0, import_pg_core.pgTable)(
  "auto_trade_policies",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    ...
    totalCapital: (0, import_pg_core.integer)("totalCapital").notNull(),
    maxConcurrentPositions: (0, import_pg_core.integer)("maxConcurrentPositions").notNull(),
    stopLossPercent: (0, import_pg_core.decimal)("stopLossPercent", { precision: 8, scale: 4 }).notNull(),
    takeProfitPercent: (0, import_pg_core.decimal)("takeProfitPercent", { precision: 8, scale: 4 }).notNull(),
    dailyLossLimitPercent: (0, import_pg_core.decimal)("dailyLossLimitPercent", { precision: 8, scale: 4 }).notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("auto_trade_policies_user_version_unique").on(table.userId, table.version), (0, import_pg_core.index)("auto_trade_policies_user_status_idx").on(table.userId, table.status, table.updatedAt)]
);
var strategyPresets = (0, import_pg_core.pgTable)(
  "strategy_presets",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    ...
    rulesJson: (0, import_pg_core.json)("rulesJson").notNull(),
    scoringJson: (0, import_pg_core.json)("scoringJson"),
    isActive: (0, import_pg_core.boolean)("isActive").default(true).notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("strategy_presets_user_idx").on(table.userId)]
);
var publicStrategyCards = (0, import_pg_core.pgTable)(
  "public_strategy_cards",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    creatorUserId: (0, import_pg_core.integer)("creatorUserId").notNull(),
    sourceCandidateId: (0, import_pg_core.integer)("sourceCandidateId").notNull(),
    sourceSweepId: (0, import_pg_core.integer)("sourceSweepId").notNull(),
    strategyFingerprint: (0, import_pg_core.varchar)("strategyFingerprint", { length: 64 }).notNull(),
    version: (0, import_pg_core.integer)("version").default(1).notNull(),
    parentCardId: (0, import_pg_core.integer)("parentCardId"),
    title: (0, import_pg_core.varchar)("title", { length: 120 }).notNull(),
    rootGenomeJson: (0, import_pg_core.json)("rootGenomeJson").notNull(),
    minimumScore: (0, import_pg_core.integer)("minimumScore").notNull(),
    datasetFingerprint: (0, import_pg_core.varchar)("datasetFingerprint", { length: 64 }).notNull(),
    arenaEvidenceJson: (0, import_pg_core.json)("arenaEvidenceJson").notNull(),
    validationEvidenceJson: (0, import_pg_core.json)("validationEvidenceJson").notNull(),
    visibility: visibilityEnum("visibility").default("public").notNull(),
    publishedAt: (0, import_pg_core.timestamp)("publishedAt").defaultNow().notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("public_strategy_cards_source_candidate_idx").on(table.sourceCandidateId), (0, import_pg_core.index)("public_strategy_cards_visibility_published_idx").on(table.visibility, table.publishedAt), (0, import_pg_core.index)("public_strategy_cards_creator_idx").on(table.creatorUserId, table.publishedAt)]
);
var publicStrategyCardCollections = (0, import_pg_core.pgTable)(
  "public_strategy_card_collections",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    cardId: (0, import_pg_core.integer)("cardId").notNull(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    presetId: (0, import_pg_core.integer)("presetId").notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("public_strategy_card_collections_card_user_unique").on(table.cardId, table.userId), (0, import_pg_core.index)("public_strategy_card_collections_user_idx").on(table.userId, table.createdAt)]
);
var publicStrategyCardComments = (0, import_pg_core.pgTable)(
  "public_strategy_card_comments",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    cardId: (0, import_pg_core.integer)("cardId").notNull(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    body: (0, import_pg_core.varchar)("body", { length: 800 }).notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("public_strategy_card_comments_card_created_idx").on(table.cardId, table.createdAt), (0, import_pg_core.index)("public_strategy_card_comments_user_idx").on(table.userId, table.createdAt)]
);
var publicStrategyCardFavorites = (0, import_pg_core.pgTable)(
  ...
  (table) => [(0, import_pg_core.uniqueIndex)("public_strategy_card_favorites_card_user_unique").on(table.cardId, table.userId), (0, import_pg_core.index)("public_strategy_card_favorites_card_created_idx").on(table.cardId, table.createdAt)]
);
var rankingSnapshots = (0, import_pg_core.pgTable)(
  "ranking_snapshots",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    presetId: (0, import_pg_core.integer)("presetId").notNull(),
    symbol: (0, import_pg_core.varchar)("symbol", { length: 24 }).notNull(),
    name: (0, import_pg_core.varchar)("name", { length: 120 }).notNull(),
    score: (0, import_pg_core.decimal)("score", { precision: 8, scale: 2 }).notNull(),
    price: (0, import_pg_core.integer)("price").notNull(),
    changeRate: (0, import_pg_core.decimal)("changeRate", { precision: 7, scale: 3 }).notNull(),
    matchedRulesJson: (0, import_pg_core.json)("matchedRulesJson").notNull(),
    runKey: (0, import_pg_core.varchar)("runKey", { length: 64 }),
    capturedAt: (0, import_pg_core.timestamp)("capturedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("ranking_snapshots_lookup_idx").on(table.userId, table.presetId, table.capturedAt), (0, import_pg_core.uniqueIndex)("ranking_snapshots_run_symbol_unique").on(table.userId, table.presetId, table.symbol, table.runKey)]
);
var rankingRefreshProfiles = (0, import_pg_core.pgTable)(
  "ranking_refresh_profiles",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    presetId: (0, import_pg_core.integer)("presetId").notNull(),
    universeJson: (0, import_pg_core.json)("universeJson").notNull(),
    maxPagesPerSymbol: (0, import_pg_core.integer)("maxPagesPerSymbol").default(3).notNull(),
    cronExpression: (0, import_pg_core.varchar)("cronExpression", { length: 48 }).default("0 */15 * * * *").notNull(),
    scheduleCronTaskUid: (0, import_pg_core.varchar)("scheduleCronTaskUid", { length: 65 }),
    status: rankingRefreshStatusEnum("status").default("idle").notNull(),
    lastRunKey: (0, import_pg_core.varchar)("lastRunKey", { length: 64 }),
    lastRunAt: (0, import_pg_core.timestamp)("lastRunAt"),
    lastCompletedAt: (0, import_pg_core.timestamp)("lastCompletedAt"),
    lastError: (0, import_pg_core.varchar)("lastError", { length: 500 }),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("ranking_refresh_profiles_user_unique").on(table.userId), (0, import_pg_core.index)("ranking_refresh_profiles_task_idx").on(table.scheduleCronTaskUid)]
);
var htsConditionSnapshots = (0, import_pg_core.pgTable)(
  "hts_condition_snapshots",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    conditionSequence: (0, import_pg_core.varchar)("conditionSequence", { length: 3 }).notNull(),
    conditionName: (0, import_pg_core.varchar)("conditionName", { length: 120 }).notNull(),
    candidatesJson: (0, import_pg_core.json)("candidatesJson").notNull(),
    capturedAt: (0, import_pg_core.timestamp)("capturedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("hts_condition_snapshots_lookup_idx").on(table.userId, table.conditionSequence, table.capturedAt)]
);
var researchDatasets = (0, import_pg_core.pgTable)(
  "research_datasets",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    name: (0, import_pg_core.varchar)("name", { length: 160 }).notNull(),
    source: sourceEnum("source").default("kiwoom_daily").notNull(),
    versionKey: (0, import_pg_core.varchar)("versionKey", { length: 80 }).notNull(),
    visibility: (0, import_pg_core.varchar)("visibility", { length: 50 }).default("private").notNull(),
    randomSeed: (0, import_pg_core.integer)("randomSeed"),
    sourceFingerprint: (0, import_pg_core.varchar)("sourceFingerprint", { length: 64 }),
    universeJson: (0, import_pg_core.json)("universeJson").notNull(),
    startDate: (0, import_pg_core.varchar)("startDate", { length: 10 }).notNull(),
    endDate: (0, import_pg_core.varchar)("endDate", { length: 10 }).notNull(),
    barCount: (0, import_pg_core.integer)("barCount").default(0).notNull(),
    minuteBarCount: (0, import_pg_core.integer)("minuteBarCount").default(0).notNull(),
    ...
    qualityReportJson: (0, import_pg_core.json)("qualityReportJson"),
    sourceCapturedAt: (0, import_pg_core.timestamp)("sourceCapturedAt"),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    readyAt: (0, import_pg_core.timestamp)("readyAt")
  },
  (table) => [(0, import_pg_core.uniqueIndex)("research_datasets_user_version_unique").on(table.userId, table.versionKey), (0, import_pg_core.index)("research_datasets_user_status_idx").on(table.userId, table.qualityStatus, table.createdAt), (0, import_pg_core.index)("research_datasets_public_ready_idx").on(table.visibility, table.qualityStatus, table.readyAt), (0, import_pg_core.index)("research_datasets_source_fingerprint_idx").on(table.sourceFingerprint)]
);
var researchDailyBars = (0, import_pg_core.pgTable)(
  "research_daily_bars",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    datasetId: (0, import_pg_core.integer)("datasetId").notNull(),
    symbol: (0, import_pg_core.varchar)("symbol", { length: 24 }).notNull(),
    date: (0, import_pg_core.varchar)("date", { length: 10 }).notNull(),
    open: (0, import_pg_core.integer)("open").notNull(),
    high: (0, import_pg_core.integer)("high").notNull(),
    low: (0, import_pg_core.integer)("low").notNull(),
    close: (0, import_pg_core.integer)("close").notNull(),
    volume: (0, import_pg_core.decimal)("volume", { precision: 20, scale: 0 }).notNull(),
    turnover: (0, import_pg_core.decimal)("turnover", { precision: 24, scale: 0 }).notNull(),
    source: (0, import_pg_core.varchar)("source", { length: 40 }).default("kiwoom_ka10081").notNull(),
    capturedAt: (0, import_pg_core.timestamp)("capturedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("research_daily_bars_dataset_symbol_date_unique").on(table.datasetId, table.symbol, table.date), (0, import_pg_core.index)("research_daily_bars_dataset_symbol_date_idx").on(table.datasetId, table.symbol, table.date)]
);
var researchFiveMinuteBars = (0, import_pg_core.pgTable)(
  "research_five_minute_bars",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    datasetId: (0, import_pg_core.integer)("datasetId").notNull(),
    symbol: (0, import_pg_core.varchar)("symbol", { length: 24 }).notNull(),
    tradingDate: (0, import_pg_core.varchar)("tradingDate", { length: 10 }).notNull(),
    intervalAt: (0, import_pg_core.timestamp)("intervalAt").notNull(),
    open: (0, import_pg_core.integer)("open").notNull(),
    high: (0, import_pg_core.integer)("high").notNull(),
    low: (0, import_pg_core.integer)("low").notNull(),
    close: (0, import_pg_core.integer)("close").notNull(),
    volume: (0, import_pg_core.decimal)("volume", { precision: 20, scale: 0 }).notNull(),
    source: (0, import_pg_core.varchar)("source", { length: 48 }).default("kiwoom_ka10080_5m_aggregate").notNull(),
    rawFingerprint: (0, import_pg_core.varchar)("rawFingerprint", { length: 64 }).notNull(),
    capturedAt: (0, import_pg_core.timestamp)("capturedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("research_five_minute_dataset_symbol_time_unique").on(table.datasetId, table.symbol, table.intervalAt), (0, import_pg_core.index)("research_five_minute_dataset_symbol_date_idx").on(table.datasetId, table.symbol, table.tradingDate)]
);
var sharedDatasetBacktests = (0, import_pg_core.pgTable)(
  "shared_dataset_backtests",
  {
    ...
    symbol: (0, import_pg_core.varchar)("symbol", { length: 24 }).notNull(),
    assumptionsJson: (0, import_pg_core.json)("assumptionsJson").notNull(),
    resultsJson: (0, import_pg_core.json)("resultsJson").notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("shared_dataset_backtests_user_created_idx").on(table.userId, table.createdAt), (0, import_pg_core.index)("shared_dataset_backtests_dataset_created_idx").on(table.datasetId, table.createdAt)]
);
var strategySurvivalLedgers = (0, import_pg_core.pgTable)(
  "strategy_survival_ledgers",
  {
    ...
    criteriaJson: (0, import_pg_core.json)("criteriaJson").notNull(),
    evidenceJson: (0, import_pg_core.json)("evidenceJson").notNull(),
    improvementPlanJson: (0, import_pg_core.json)("improvementPlanJson").notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("strategy_survival_ledgers_user_created_idx").on(table.userId, table.createdAt), (0, import_pg_core.index)("strategy_survival_ledgers_preset_time_idx").on(table.presetId, table.timeframe, table.createdAt)]
);
var sharedDatasetCollectionRequests = (0, import_pg_core.pgTable)(
  "shared_dataset_collection_requests",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    requestedByUserId: (0, import_pg_core.integer)("requestedByUserId").notNull(),
    randomSeed: (0, import_pg_core.integer)("randomSeed").notNull(),
    symbolCount: (0, import_pg_core.integer)("symbolCount").notNull(),
    sampleDays: (0, import_pg_core.integer)("sampleDays").notNull(),
    status: collectionRequestStatusEnum("status").default("queued").notNull(),
    requestFingerprint: (0, import_pg_core.varchar)("requestFingerprint", { length: 64 }).notNull(),
    plannedUniverseJson: (0, import_pg_core.json)("plannedUniverseJson"),
    datasetId: (0, import_pg_core.integer)("datasetId"),
    acceptedDailyBarCount: (0, import_pg_core.integer)("acceptedDailyBarCount").default(0).notNull(),
    acceptedFiveMinuteBarCount: (0, import_pg_core.integer)("acceptedFiveMinuteBarCount").default(0).notNull(),
    progressJson: (0, import_pg_core.json)("progressJson"),
    resumeCount: (0, import_pg_core.integer)("resumeCount").default(0).notNull(),
    lastError: (0, import_pg_core.varchar)("lastError", { length: 500 }),
    requestedAt: (0, import_pg_core.timestamp)("requestedAt").defaultNow().notNull(),
    startedAt: (0, import_pg_core.timestamp)("startedAt"),
    completedAt: (0, import_pg_core.timestamp)("completedAt")
  },
  (table) => [(0, import_pg_core.uniqueIndex)("shared_dataset_collection_request_fingerprint_unique").on(table.requestFingerprint), (0, import_pg_core.index)("shared_dataset_collection_request_status_idx").on(table.status, table.requestedAt), (0, import_pg_core.index)("shared_dataset_collection_request_user_idx").on(table.requestedByUserId, table.requestedAt)]
);
var kiwoomTerminalConnectionChecks = (0, import_pg_core.pgTable)(
  "kiwoom_terminal_connection_checks",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    terminalIp: (0, import_pg_core.varchar)("terminalIp", { length: 45 }).notNull(),
    status: connectionStatusEnum("status").notNull(),
    errorCode: (0, import_pg_core.varchar)("errorCode", { length: 80 }),
    message: (0, import_pg_core.varchar)("message", { length: 500 }).notNull(),
    verificationJson: (0, import_pg_core.json)("verificationJson"),
    checkedAt: (0, import_pg_core.timestamp)("checkedAt").defaultNow().notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("kiwoom_terminal_checks_user_time_idx").on(table.userId, table.checkedAt), (0, import_pg_core.index)("kiwoom_terminal_checks_ip_time_idx").on(table.terminalIp, table.checkedAt)]
);
var localResearchDailyBars = (0, import_pg_core.pgTable)(
  ...
  (table) => [(0, import_pg_core.uniqueIndex)("local_research_daily_bars_symbol_date_adjustment_unique").on(table.symbol, table.date, table.adjustmentBasis), (0, import_pg_core.index)("local_research_daily_bars_symbol_date_idx").on(table.symbol, table.date, table.capturedAt)]
);
var researchExperiments = (0, import_pg_core.pgTable)(
  "research_experiments",
  {
    ...
    name: (0, import_pg_core.varchar)("name", { length: 160 }).notNull(),
    randomSeed: (0, import_pg_core.integer)("randomSeed").notNull(),
    configurationJson: (0, import_pg_core.json)("configurationJson").notNull(),
    informationCutoffTradingDays: (0, import_pg_core.integer)("informationCutoffTradingDays").default(1).notNull(),
    trainingStartDate: (0, import_pg_core.varchar)("trainingStartDate", { length: 10 }),
    trainingEndDate: (0, import_pg_core.varchar)("trainingEndDate", { length: 10 }),
    validationStartDate: (0, import_pg_core.varchar)("validationStartDate", { length: 10 }),
    validationEndDate: (0, import_pg_core.varchar)("validationEndDate", { length: 10 }),
    status: experimentStatusEnum("status").default("draft").notNull(),
    strategySnapshotJson: (0, import_pg_core.json)("strategySnapshotJson"),
    assumptionsJson: (0, import_pg_core.json)("assumptionsJson"),
    resultsJson: (0, import_pg_core.json)("resultsJson"),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    completedAt: (0, import_pg_core.timestamp)("completedAt")
  },
  (table) => [(0, import_pg_core.index)("research_experiments_user_created_idx").on(table.userId, table.createdAt), (0, import_pg_core.index)("research_experiments_dataset_idx").on(table.datasetId, table.status)]
);
var walkForwardRuns = (0, import_pg_core.pgTable)(
  "walk_forward_runs",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    experimentId: (0, import_pg_core.integer)("experimentId").notNull(),
  ...
  (table) => [(0, import_pg_core.index)("walk_forward_runs_user_created_idx").on(table.userId, table.createdAt), (0, import_pg_core.index)("walk_forward_runs_experiment_idx").on(table.experimentId, table.status)]
);
var evolutionSearches = (0, import_pg_core.pgTable)(
  "evolution_searches",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    ...
    summaryJson: (0, import_pg_core.json)("summaryJson"),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    completedAt: (0, import_pg_core.timestamp)("completedAt")
  },
  (table) => [(0, import_pg_core.index)("evolution_searches_user_created_idx").on(table.userId, table.createdAt), (0, import_pg_core.index)("evolution_searches_dataset_status_idx").on(table.datasetId, table.status)]
);
var evolutionGenerations = (0, import_pg_core.pgTable)(
  "evolution_generations",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    searchId: (0, import_pg_core.integer)("searchId").notNull(),
    generationNumber: (0, import_pg_core.integer)("generationNumber").notNull(),
    populationSize: (0, import_pg_core.integer)("populationSize").notNull(),
    uniqueCandidateCount: (0, import_pg_core.integer)("uniqueCandidateCount").default(0).notNull(),
    survivorCount: (0, import_pg_core.integer)("survivorCount").default(0).notNull(),
    status: generationStatusEnum("status").default("queued").notNull(),
    selectionSummaryJson: (0, import_pg_core.json)("selectionSummaryJson"),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    completedAt: (0, import_pg_core.timestamp)("completedAt")
  },
  (table) => [(0, import_pg_core.uniqueIndex)("evolution_generations_search_number_unique").on(table.searchId, table.generationNumber), (0, import_pg_core.index)("evolution_generations_search_status_idx").on(table.searchId, table.status)]
);
var evolutionCandidates = (0, import_pg_core.pgTable)(
  "evolution_candidates",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    searchId: (0, import_pg_core.integer)("searchId").notNull(),
    generationId: (0, import_pg_core.integer)("generationId").notNull(),
    fingerprint: (0, import_pg_core.varchar)("fingerprint", { length: 64 }).notNull(),
    rootGenomeJson: (0, import_pg_core.json)("rootGenomeJson").notNull(),
    minimumScore: (0, import_pg_core.integer)("minimumScore").notNull(),
    origin: originEnum("origin").notNull(),
    parentCandidateIdsJson: (0, import_pg_core.json)("parentCandidateIdsJson"),
    mutationJson: (0, import_pg_core.json)("mutationJson"),
    inSampleMetricsJson: (0, import_pg_core.json)("inSampleMetricsJson"),
    outOfSampleMetricsJson: (0, import_pg_core.json)("outOfSampleMetricsJson"),
    walkForwardMetricsJson: (0, import_pg_core.json)("walkForwardMetricsJson"),
    fitnessScore: (0, import_pg_core.decimal)("fitnessScore", { precision: 12, scale: 6 }),
    status: candidateStatusEnum("status").default("created").notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    evaluatedAt: (0, import_pg_core.timestamp)("evaluatedAt")
  },
  (table) => [(0, import_pg_core.uniqueIndex)("evolution_candidates_search_fingerprint_unique").on(table.searchId, table.fingerprint), (0, import_pg_core.index)("evolution_candidates_generation_status_idx").on(table.generationId, table.status), (0, import_pg_core.index)("evolution_candidates_search_fitness_idx").on(table.searchId, table.fitnessScore)]
);
var autonomousResearchRuns = (0, import_pg_core.pgTable)(
  "autonomous_research_runs",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    tradingDate: (0, import_pg_core.varchar)("tradingDate", { length: 10 }).notNull(),
    phase: phaseEnum("phase").default("preparing").notNull(),
    runKey: (0, import_pg_core.varchar)("runKey", { length: 96 }).notNull(),
    dataStatus: dataStatusEnum("dataStatus").default("pending").notNull(),
    universeJson: (0, import_pg_core.json)("universeJson"),
    policyVersion: (0, import_pg_core.varchar)("policyVersion", { length: 40 }).notNull(),
    summaryJson: (0, import_pg_core.json)("summaryJson"),
    lastError: (0, import_pg_core.varchar)("lastError", { length: 500 }),
    startedAt: (0, import_pg_core.timestamp)("startedAt").defaultNow().notNull(),
    lastObservedAt: (0, import_pg_core.timestamp)("lastObservedAt"),
    completedAt: (0, import_pg_core.timestamp)("completedAt"),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("autonomous_research_runs_run_key_unique").on(table.runKey), (0, import_pg_core.index)("autonomous_research_runs_date_phase_idx").on(table.tradingDate, table.phase, table.updatedAt)]
);
var autonomousResearchTasks = (0, import_pg_core.pgTable)(
  "autonomous_research_tasks",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    runId: (0, import_pg_core.integer)("runId").notNull(),
    runKey: (0, import_pg_core.varchar)("runKey", { length: 96 }).notNull(),
    phase: (0, import_pg_core.varchar)("phase", { length: 50 }).notNull(),
    status: autonomousTaskStatusEnum("status").default("running").notNull(),
    resultJson: (0, import_pg_core.json)("resultJson"),
    lastError: (0, import_pg_core.varchar)("lastError", { length: 500 }),
    startedAt: (0, import_pg_core.timestamp)("startedAt").defaultNow().notNull(),
    completedAt: (0, import_pg_core.timestamp)("completedAt")
  },
  (table) => [(0, import_pg_core.uniqueIndex)("auto_research_tasks_run_key_uq").on(table.runKey), (0, import_pg_core.index)("auto_research_tasks_run_phase_idx").on(table.runId, table.phase, table.startedAt)]
);
var autonomousResearchBars = (0, import_pg_core.pgTable)(
  "autonomous_research_bars",
  {
    ...
    volume: (0, import_pg_core.decimal)("volume", { precision: 20, scale: 0 }).notNull(),
    turnover: (0, import_pg_core.decimal)("turnover", { precision: 24, scale: 0 }).notNull(),
    source: (0, import_pg_core.varchar)("source", { length: 48 }).default("kiwoom_ka10081").notNull(),
    capturedAt: (0, import_pg_core.timestamp)("capturedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("auto_bars_run_symbol_date_uq").on(table.runId, table.symbol, table.date), (0, import_pg_core.index)("auto_bars_run_symbol_date_idx").on(table.runId, table.symbol, table.date)]
);
var intradayMinuteBars = (0, import_pg_core.pgTable)(
  "intraday_minute_bars",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    tradingDate: (0, import_pg_core.varchar)("tradingDate", { length: 10 }).notNull(),
    symbol: (0, import_pg_core.varchar)("symbol", { length: 24 }).notNull(),
    minuteAt: (0, import_pg_core.timestamp)("minuteAt").notNull(),
    open: (0, import_pg_core.integer)("open").notNull(),
    high: (0, import_pg_core.integer)("high").notNull(),
    low: (0, import_pg_core.integer)("low").notNull(),
    close: (0, import_pg_core.integer)("close").notNull(),
    volume: (0, import_pg_core.decimal)("volume", { precision: 20, scale: 0 }).notNull(),
    source: (0, import_pg_core.varchar)("source", { length: 48 }).default("kiwoom_ka10080").notNull(),
    rawFingerprint: (0, import_pg_core.varchar)("rawFingerprint", { length: 64 }).notNull(),
    capturedAt: (0, import_pg_core.timestamp)("capturedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("intraday_minute_bar_date_symbol_time_uq").on(table.tradingDate, table.symbol, table.minuteAt), (0, import_pg_core.index)("intraday_minute_bar_symbol_time_idx").on(table.symbol, table.minuteAt), (0, import_pg_core.index)("intraday_minute_bar_date_captured_idx").on(table.tradingDate, table.capturedAt)]
);
var autonomousResearchCandidates = (0, import_pg_core.pgTable)(
  "autonomous_research_candidates",
  {
    ...
    rootGenomeJson: (0, import_pg_core.json)("rootGenomeJson").notNull(),
    minimumScore: (0, import_pg_core.integer)("minimumScore").notNull(),
    generationNumber: (0, import_pg_core.integer)("generationNumber").default(0).notNull(),
    status: (0, import_pg_core.varchar)("status", { length: 50 }).default("generated").notNull(),
    inSampleMetricsJson: (0, import_pg_core.json)("inSampleMetricsJson"),
    outOfSampleMetricsJson: (0, import_pg_core.json)("outOfSampleMetricsJson"),
    walkForwardMetricsJson: (0, import_pg_core.json)("walkForwardMetricsJson"),
    simulationJson: (0, import_pg_core.json)("simulationJson"),
    fitnessScore: (0, import_pg_core.decimal)("fitnessScore", { precision: 12, scale: 6 }),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    evaluatedAt: (0, import_pg_core.timestamp)("evaluatedAt"),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("autonomous_research_candidates_run_fingerprint_unique").on(table.runId, table.fingerprint), (0, import_pg_core.index)("autonomous_research_candidates_run_status_idx").on(table.runId, table.status), (0, import_pg_core.index)("autonomous_research_candidates_run_fitness_idx").on(table.runId, table.fitnessScore)]
);
var researchCommitteeReports = (0, import_pg_core.pgTable)(
  "research_committee_reports",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    runId: (0, import_pg_core.integer)("runId").notNull(),
    sourceRunId: (0, import_pg_core.integer)("sourceRunId").notNull(),
    evidenceFingerprint: (0, import_pg_core.varchar)("evidenceFingerprint", { length: 64 }).notNull(),
    policyVersion: (0, import_pg_core.varchar)("policyVersion", { length: 40 }).notNull(),
    model: (0, import_pg_core.varchar)("model", { length: 80 }).notNull(),
    status: committeeStatusEnum("status").default("running").notNull(),
    evidenceJson: (0, import_pg_core.json)("evidenceJson").notNull(),
    memberReviewsJson: (0, import_pg_core.json)("memberReviewsJson"),
    deliberationJson: (0, import_pg_core.json)("deliberationJson"),
    lastError: (0, import_pg_core.varchar)("lastError", { length: 500 }),
    startedAt: (0, import_pg_core.timestamp)("startedAt").defaultNow().notNull(),
    completedAt: (0, import_pg_core.timestamp)("completedAt"),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
  ...
  (table) => [
    (0, import_pg_core.uniqueIndex)("committee_reports_run_evidence_unique").on(table.runId, table.evidenceFingerprint),
    (0, import_pg_core.index)("committee_reports_run_status_idx").on(table.runId, table.status, table.updatedAt)
  ]
);
var researchGovernanceSchedules = (0, import_pg_core.pgTable)(
  "research_governance_schedules",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    taskUid: (0, import_pg_core.varchar)("taskUid", { length: 65 }),
    scheduleVersion: (0, import_pg_core.varchar)("scheduleVersion", { length: 40 }).notNull(),
    cronExpression: (0, import_pg_core.varchar)("cronExpression", { length: 64 }).notNull(),
    isEnabled: (0, import_pg_core.boolean)("isEnabled").default(true).notNull(),
    latestCycleId: (0, import_pg_core.integer)("latestCycleId"),
    lastError: (0, import_pg_core.varchar)("lastError", { length: 500 }),
    lastRequestedAt: (0, import_pg_core.timestamp)("lastRequestedAt"),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("research_governance_schedule_task_uid_unique").on(table.taskUid)]
);
var researchGovernanceCycles = (0, import_pg_core.pgTable)(
  "research_governance_cycles",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    runId: (0, import_pg_core.integer)("runId").notNull(),
    committeeReportId: (0, import_pg_core.integer)("committeeReportId").notNull(),
    evidenceFingerprint: (0, import_pg_core.varchar)("evidenceFingerprint", { length: 64 }).notNull(),
    cycleFingerprint: (0, import_pg_core.varchar)("cycleFingerprint", { length: 64 }).notNull(),
    policyVersion: (0, import_pg_core.varchar)("policyVersion", { length: 40 }).notNull(),
    managerModel: (0, import_pg_core.varchar)("managerModel", { length: 80 }).notNull(),
    status: governanceStatusEnum("status").default("running").notNull(),
    sourceSummaryJson: (0, import_pg_core.json)("sourceSummaryJson").notNull(),
    managerDirectiveJson: (0, import_pg_core.json)("managerDirectiveJson"),
    leaderFollowUpsJson: (0, import_pg_core.json)("leaderFollowUpsJson"),
    lastError: (0, import_pg_core.varchar)("lastError", { length: 500 }),
    startedAt: (0, import_pg_core.timestamp)("startedAt").defaultNow().notNull(),
    completedAt: (0, import_pg_core.timestamp)("completedAt"),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
  ...
  (table) => [
    (0, import_pg_core.uniqueIndex)("research_governance_cycle_fingerprint_unique").on(table.cycleFingerprint),
    (0, import_pg_core.index)("research_governance_cycles_run_status_idx").on(table.runId, table.status, table.updatedAt),
    (0, import_pg_core.index)("research_governance_cycles_committee_idx").on(table.committeeReportId, table.updatedAt)
  ]
);
var researchRevalidationJobs = (0, import_pg_core.pgTable)(
  "research_revalidation_jobs",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    governanceCycleId: (0, import_pg_core.integer)("governanceCycleId").notNull(),
    sourceRunId: (0, import_pg_core.integer)("sourceRunId").notNull(),
    priorityId: (0, import_pg_core.varchar)("priorityId", { length: 80 }).notNull(),
    priorityTitle: (0, import_pg_core.varchar)("priorityTitle", { length: 240 }).notNull(),
    evidenceFingerprint: (0, import_pg_core.varchar)("evidenceFingerprint", { length: 64 }).notNull(),
    jobFingerprint: (0, import_pg_core.varchar)("jobFingerprint", { length: 64 }).notNull(),
    scope: scopeEnum("scope").notNull(),
    status: revalidationStatusEnum("status").default("queued").notNull(),
    acceptanceCriteria: (0, import_pg_core.text)("acceptanceCriteria").notNull(),
    blocker: (0, import_pg_core.varchar)("blocker", { length: 500 }),
    resultJson: (0, import_pg_core.json)("resultJson"),
    lastError: (0, import_pg_core.varchar)("lastError", { length: 500 }),
    startedAt: (0, import_pg_core.timestamp)("startedAt"),
    completedAt: (0, import_pg_core.timestamp)("completedAt"),
  ...
  (table) => [
    (0, import_pg_core.uniqueIndex)("research_revalidation_job_fingerprint_unique").on(table.jobFingerprint),
    (0, import_pg_core.index)("research_revalidation_cycle_status_idx").on(table.governanceCycleId, table.status, table.updatedAt),
    (0, import_pg_core.index)("research_revalidation_source_priority_idx").on(table.sourceRunId, table.priorityId)
  ]
);
var autonomousResearchObservations = (0, import_pg_core.pgTable)(
  "autonomous_research_observations",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    runId: (0, import_pg_core.integer)("runId").notNull(),
    candidateId: (0, import_pg_core.integer)("candidateId"),
    symbol: (0, import_pg_core.varchar)("symbol", { length: 24 }).notNull(),
    name: (0, import_pg_core.varchar)("name", { length: 120 }),
    price: (0, import_pg_core.integer)("price").notNull(),
    changeRate: (0, import_pg_core.decimal)("changeRate", { precision: 7, scale: 3 }),
    source: (0, import_pg_core.varchar)("source", { length: 48 }).notNull(),
    capturedAt: (0, import_pg_core.timestamp)("capturedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("auto_obs_run_cand_symbol_time_uq").on(table.runId, table.candidateId, table.symbol, table.capturedAt), (0, import_pg_core.index)("auto_obs_run_time_idx").on(table.runId, table.capturedAt)]
);
var dayTradeExperiments = (0, import_pg_core.pgTable)(
  "day_trade_experiments",
  {
    ...
    totalCapital: (0, import_pg_core.integer)("totalCapital").notNull(),
    buyFeeRate: (0, import_pg_core.decimal)("buyFeeRate", { precision: 8, scale: 6 }).notNull(),
    sellFeeRate: (0, import_pg_core.decimal)("sellFeeRate", { precision: 8, scale: 6 }).notNull(),
    signalCount: (0, import_pg_core.integer)("signalCount").default(0).notNull(),
    selectedPositionCount: (0, import_pg_core.integer)("selectedPositionCount").default(0).notNull(),
    netValue: (0, import_pg_core.integer)("netValue").default(0).notNull(),
    netPnl: (0, import_pg_core.integer)("netPnl").default(0).notNull(),
    netReturnPercent: (0, import_pg_core.decimal)("netReturnPercent", { precision: 10, scale: 4 }).default("0").notNull(),
    sourceFingerprint: (0, import_pg_core.varchar)("sourceFingerprint", { length: 64 }).notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull(),
    closedAt: (0, import_pg_core.timestamp)("closedAt")
  },
  (table) => [(0, import_pg_core.uniqueIndex)("day_trade_experiments_run_unique").on(table.runId), (0, import_pg_core.index)("day_trade_experiments_date_status_idx").on(table.tradingDate, table.status, table.updatedAt)]
);
var dayTradeExperimentPositions = (0, import_pg_core.pgTable)(
  "day_trade_experiment_positions",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    experimentId: (0, import_pg_core.integer)("experimentId").notNull(),
    candidateId: (0, import_pg_core.integer)("candidateId").notNull(),
    candidateFingerprint: (0, import_pg_core.varchar)("candidateFingerprint", { length: 64 }).notNull(),
    symbol: (0, import_pg_core.varchar)("symbol", { length: 24 }).notNull(),
    name: (0, import_pg_core.varchar)("name", { length: 120 }).notNull(),
    signalCount: (0, import_pg_core.integer)("signalCount").default(1).notNull(),
    quantity: (0, import_pg_core.integer)("quantity").notNull(),
    allocation: (0, import_pg_core.integer)("allocation").notNull(),
    entryPrice: (0, import_pg_core.integer)("entryPrice").notNull(),
    entryAt: (0, import_pg_core.timestamp)("entryAt").notNull(),
    lastPrice: (0, import_pg_core.integer)("lastPrice"),
    lastObservedAt: (0, import_pg_core.timestamp)("lastObservedAt"),
    exitPrice: (0, import_pg_core.integer)("exitPrice"),
    exitAt: (0, import_pg_core.timestamp)("exitAt"),
    buyFee: (0, import_pg_core.integer)("buyFee").default(0).notNull(),
    estimatedExitFee: (0, import_pg_core.integer)("estimatedExitFee").default(0).notNull(),
    netValue: (0, import_pg_core.integer)("netValue").default(0).notNull(),
    netPnl: (0, import_pg_core.integer)("netPnl").default(0).notNull(),
    netReturnPercent: (0, import_pg_core.decimal)("netReturnPercent", { precision: 10, scale: 4 }).default("0").notNull(),
    status: trackingStatusEnum("status").default("tracking").notNull(),
  ...
  (table) => [(0, import_pg_core.uniqueIndex)("day_trade_position_experiment_symbol_unique").on(table.experimentId, table.symbol), (0, import_pg_core.index)("day_trade_positions_candidate_idx").on(table.candidateId, table.updatedAt), (0, import_pg_core.index)("day_trade_positions_experiment_status_idx").on(table.experimentId, table.status, table.updatedAt)]
);
var localResearchNodeSyncEvents = (0, import_pg_core.pgTable)(
  "local_research_node_sync_events",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    experimentId: (0, import_pg_core.integer)("experimentId"),
    tradingDate: (0, import_pg_core.varchar)("tradingDate", { length: 10 }).notNull(),
    channel: channelEnum("channel").default("intraday_price").notNull(),
    status: (0, import_pg_core.varchar)("status", { length: 50 }).notNull(),
    quoteCount: (0, import_pg_core.integer)("quoteCount").default(0).notNull(),
    rejectedQuoteCount: (0, import_pg_core.integer)("rejectedQuoteCount").default(0).notNull(),
    message: (0, import_pg_core.varchar)("message", { length: 500 }),
    observedAt: (0, import_pg_core.timestamp)("observedAt"),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("local_node_sync_experiment_time_idx").on(table.experimentId, table.createdAt), (0, import_pg_core.index)("local_node_sync_date_time_idx").on(table.tradingDate, table.createdAt)]
);
var backtestRuns = (0, import_pg_core.pgTable)(
  "backtest_runs",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    ...
    startDate: (0, import_pg_core.varchar)("startDate", { length: 10 }).notNull(),
    endDate: (0, import_pg_core.varchar)("endDate", { length: 10 }).notNull(),
    initialCapital: (0, import_pg_core.integer)("initialCapital").notNull(),
    totalReturn: (0, import_pg_core.decimal)("totalReturn", { precision: 8, scale: 3 }),
    winRate: (0, import_pg_core.decimal)("winRate", { precision: 6, scale: 2 }),
    tradeCount: (0, import_pg_core.integer)("tradeCount"),
    maxDrawdown: (0, import_pg_core.decimal)("maxDrawdown", { precision: 8, scale: 3 }),
    resultsJson: (0, import_pg_core.json)("resultsJson"),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    completedAt: (0, import_pg_core.timestamp)("completedAt")
  },
  (table) => [(0, import_pg_core.index)("backtest_runs_user_idx").on(table.userId, table.createdAt)]
);
var orderIntents = (0, import_pg_core.pgTable)(
  "order_intents",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    presetId: (0, import_pg_core.integer)("presetId"),
    sourceCandidateId: (0, import_pg_core.integer)("sourceCandidateId"),
    sourceObservationId: (0, import_pg_core.integer)("sourceObservationId"),
    symbol: (0, import_pg_core.varchar)("symbol", { length: 24 }).notNull(),
    name: (0, import_pg_core.varchar)("name", { length: 120 }).notNull(),
    ...
    riskReasonsJson: (0, import_pg_core.json)("riskReasonsJson"),
    autoPolicyId: (0, import_pg_core.integer)("autoPolicyId"),
    autoPolicyVersion: (0, import_pg_core.integer)("autoPolicyVersion"),
    autoPolicySnapshotJson: (0, import_pg_core.json)("autoPolicySnapshotJson"),
    executionOrigin: executionOriginEnum("executionOrigin").default("manual").notNull(),
    dedupeKey: (0, import_pg_core.varchar)("dedupeKey", { length: 160 }),
    confirmationNonce: (0, import_pg_core.varchar)("confirmationNonce", { length: 64 }),
    confirmedAt: (0, import_pg_core.timestamp)("confirmedAt"),
    brokerOrderId: (0, import_pg_core.varchar)("brokerOrderId", { length: 80 }),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("order_intents_user_status_idx").on(table.userId, table.status, table.createdAt), (0, import_pg_core.index)("order_intents_source_observation_idx").on(table.sourceObservationId), (0, import_pg_core.uniqueIndex)("order_intents_user_dedupe_unique").on(table.userId, table.dedupeKey)]
);
var localMinuteCollectionRequests = (0, import_pg_core.pgTable)(
  "local_minute_collection_requests",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    tradingDate: (0, import_pg_core.varchar)("tradingDate", { length: 10 }).notNull(),
    requestKey: (0, import_pg_core.varchar)("requestKey", { length: 64 }).notNull(),
    status: collectionRequestStatusEnum("status").default("queued").notNull(),
    source: (0, import_pg_core.varchar)("source", { length: 48 }).default("public_intraday_monitor").notNull(),
    acceptedBarCount: (0, import_pg_core.integer)("acceptedBarCount").default(0).notNull(),
    rejectedBarCount: (0, import_pg_core.integer)("rejectedBarCount").default(0).notNull(),
    lastError: (0, import_pg_core.varchar)("lastError", { length: 500 }),
    requestedAt: (0, import_pg_core.timestamp)("requestedAt").defaultNow().notNull(),
    startedAt: (0, import_pg_core.timestamp)("startedAt"),
    completedAt: (0, import_pg_core.timestamp)("completedAt"),
    lastSeenAt: (0, import_pg_core.timestamp)("lastSeenAt"),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("local_minute_collection_request_key_uq").on(table.requestKey), (0, import_pg_core.index)("local_minute_collection_status_time_idx").on(table.status, table.updatedAt), (0, import_pg_core.index)("local_minute_collection_date_time_idx").on(table.tradingDate, table.updatedAt)]
);
var orderExecutions = (0, import_pg_core.pgTable)(
  "order_executions",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    orderIntentId: (0, import_pg_core.integer)("orderIntentId").notNull(),
    brokerOrderId: (0, import_pg_core.varchar)("brokerOrderId", { length: 80 }),
    executionStatus: (0, import_pg_core.varchar)("executionStatus", { length: 40 }).notNull(),
    filledQuantity: (0, import_pg_core.integer)("filledQuantity").default(0).notNull(),
    filledPrice: (0, import_pg_core.integer)("filledPrice"),
    responseJson: (0, import_pg_core.json)("responseJson"),
    executedAt: (0, import_pg_core.timestamp)("executedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("order_executions_intent_idx").on(table.orderIntentId)]
);
var positionSnapshots = (0, import_pg_core.pgTable)(
  "position_snapshots",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    ...
    name: (0, import_pg_core.varchar)("name", { length: 120 }).notNull(),
    quantity: (0, import_pg_core.integer)("quantity").notNull(),
    averagePrice: (0, import_pg_core.integer)("averagePrice").notNull(),
    currentPrice: (0, import_pg_core.integer)("currentPrice").notNull(),
    profitLoss: (0, import_pg_core.integer)("profitLoss").notNull(),
    profitLossRate: (0, import_pg_core.decimal)("profitLossRate", { precision: 8, scale: 3 }).notNull(),
    capturedAt: (0, import_pg_core.timestamp)("capturedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("position_snapshots_user_idx").on(table.userId, table.capturedAt)]
);
var paperPortfolios = (0, import_pg_core.pgTable)(
  "paper_portfolios",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    userId: (0, import_pg_core.integer)("userId").notNull(),
    name: (0, import_pg_core.varchar)("name", { length: 120 }).notNull(),
    initialCash: (0, import_pg_core.integer)("initialCash").notNull(),
    cashBalance: (0, import_pg_core.integer)("cashBalance").notNull(),
    status: (0, import_pg_core.varchar)("status", { length: 50 }).default("active").notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("paper_portfolios_user_status_idx").on(table.userId, table.status, table.updatedAt)]
);
var paperPositions = (0, import_pg_core.pgTable)(
  "paper_positions",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    portfolioId: (0, import_pg_core.integer)("portfolioId").notNull(),
    sourceCandidateId: (0, import_pg_core.integer)("sourceCandidateId"),
    symbol: (0, import_pg_core.varchar)("symbol", { length: 24 }).notNull(),
    name: (0, import_pg_core.varchar)("name", { length: 120 }).notNull(),
    quantity: (0, import_pg_core.integer)("quantity").notNull(),
    entryPrice: (0, import_pg_core.integer)("entryPrice").notNull(),
    latestPrice: (0, import_pg_core.integer)("latestPrice").notNull(),
    unrealizedPnl: (0, import_pg_core.integer)("unrealizedPnl").default(0).notNull(),
    status: (0, import_pg_core.varchar)("status", { length: 50 }).default("open").notNull(),
    openedAt: (0, import_pg_core.timestamp)("openedAt").defaultNow().notNull(),
    closedAt: (0, import_pg_core.timestamp)("closedAt"),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("paper_positions_portfolio_status_idx").on(table.portfolioId, table.status, table.updatedAt), (0, import_pg_core.index)("paper_positions_candidate_idx").on(table.sourceCandidateId)]
);
var paperPortfolioPriceEvents = (0, import_pg_core.pgTable)(
  "paper_portfolio_price_events",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    portfolioId: (0, import_pg_core.integer)("portfolioId").notNull(),
    positionId: (0, import_pg_core.integer)("positionId").notNull(),
    eventType: eventTypeEnum("eventType").notNull(),
    price: (0, import_pg_core.integer)("price").notNull(),
    source: (0, import_pg_core.varchar)("source", { length: 48 }).notNull(),
    sourceTimestamp: (0, import_pg_core.timestamp)("sourceTimestamp").notNull(),
    evidenceJson: (0, import_pg_core.json)("evidenceJson").notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.index)("paper_price_events_position_time_idx").on(table.positionId, table.sourceTimestamp), (0, import_pg_core.index)("paper_price_events_portfolio_time_idx").on(table.portfolioId, table.sourceTimestamp)]
);
var minuteResearchPrograms = (0, import_pg_core.pgTable)(
  "minute_research_programs",
  {
    ...
    scheduleCronTaskUid: (0, import_pg_core.varchar)("scheduleCronTaskUid", { length: 65 }),
    configurationJson: (0, import_pg_core.json)("configurationJson").notNull(),
    lastSweepId: (0, import_pg_core.integer)("lastSweepId"),
    lastError: (0, import_pg_core.varchar)("lastError", { length: 500 }),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull(),
    updatedAt: (0, import_pg_core.timestamp)("updatedAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("minute_research_program_user_unique").on(table.userId), (0, import_pg_core.index)("minute_research_program_task_idx").on(table.scheduleCronTaskUid, table.status)]
);
var minuteResearchSweeps = (0, import_pg_core.pgTable)(
  "minute_research_sweeps",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    programId: (0, import_pg_core.integer)("programId").notNull(),
    runKey: (0, import_pg_core.varchar)("runKey", { length: 128 }).notNull(),
    tradingDatesJson: (0, import_pg_core.json)("tradingDatesJson").notNull(),
    datasetFingerprint: (0, import_pg_core.varchar)("datasetFingerprint", { length: 64 }).notNull(),
    configurationJson: (0, import_pg_core.json)("configurationJson").notNull(),
    status: sweepStatusEnum("status").default("queued").notNull(),
    generatedCount: (0, import_pg_core.integer)("generatedCount").default(0).notNull(),
    evaluatedCount: (0, import_pg_core.integer)("evaluatedCount").default(0).notNull(),
    promotedCount: (0, import_pg_core.integer)("promotedCount").default(0).notNull(),
    rejectedCount: (0, import_pg_core.integer)("rejectedCount").default(0).notNull(),
    summaryJson: (0, import_pg_core.json)("summaryJson"),
    lastError: (0, import_pg_core.varchar)("lastError", { length: 500 }),
    startedAt: (0, import_pg_core.timestamp)("startedAt").defaultNow().notNull(),
    completedAt: (0, import_pg_core.timestamp)("completedAt"),
  ...
  (table) => [(0, import_pg_core.uniqueIndex)("minute_research_sweep_run_key_unique").on(table.runKey), (0, import_pg_core.index)("minute_research_sweep_program_status_idx").on(table.programId, table.status, table.updatedAt)]
);
var minuteResearchCandidates = (0, import_pg_core.pgTable)(
  "minute_research_candidates",
  {
    id: (0, import_pg_core.serial)("id").primaryKey(),
    sweepId: (0, import_pg_core.integer)("sweepId").notNull(),
    strategyFingerprint: (0, import_pg_core.varchar)("strategyFingerprint", { length: 64 }).notNull(),
    fingerprint: (0, import_pg_core.varchar)("fingerprint", { length: 64 }).notNull(),
    rootGenomeJson: (0, import_pg_core.json)("rootGenomeJson").notNull(),
    minimumScore: (0, import_pg_core.integer)("minimumScore").notNull(),
    status: minuteResearchStatusEnum("status").default("evaluated").notNull(),
    fitnessScore: (0, import_pg_core.decimal)("fitnessScore", { precision: 14, scale: 6 }).notNull(),
    tradeCount: (0, import_pg_core.integer)("tradeCount").default(0).notNull(),
    winRate: (0, import_pg_core.decimal)("winRate", { precision: 9, scale: 4 }).default("0").notNull(),
    netReturnPercent: (0, import_pg_core.decimal)("netReturnPercent", { precision: 14, scale: 6 }).default("0").notNull(),
    expectancyPercent: (0, import_pg_core.decimal)("expectancyPercent", { precision: 14, scale: 6 }).default("0").notNull(),
    maxDrawdownPercent: (0, import_pg_core.decimal)("maxDrawdownPercent", { precision: 14, scale: 6 }).default("0").notNull(),
    validationTradeCount: (0, import_pg_core.integer)("validationTradeCount").default(0).notNull(),
    validationReturnPercent: (0, import_pg_core.decimal)("validationReturnPercent", { precision: 14, scale: 6 }).default("0").notNull(),
    validationExpectancyPercent: (0, import_pg_core.decimal)("validationExpectancyPercent", { precision: 14, scale: 6 }).default("0").notNull(),
    validationMaxDrawdownPercent: (0, import_pg_core.decimal)("validationMaxDrawdownPercent", { precision: 14, scale: 6 }).default("0").notNull(),
    inSampleMetricsJson: (0, import_pg_core.json)("inSampleMetricsJson").notNull(),
    outOfSampleMetricsJson: (0, import_pg_core.json)("outOfSampleMetricsJson"),
    qualificationJson: (0, import_pg_core.json)("qualificationJson").notNull(),
    collectedPresetId: (0, import_pg_core.integer)("collectedPresetId"),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("minute_research_candidate_sweep_fingerprint_unique").on(table.sweepId, table.fingerprint), (0, import_pg_core.index)("minute_research_candidate_sweep_status_idx").on(table.sweepId, table.status, table.fitnessScore), (0, import_pg_core.index)("minute_research_candidate_strategy_idx").on(table.strategyFingerprint, table.status, table.createdAt)]
);
var minuteResearchDailyMetrics = (0, import_pg_core.pgTable)(
  "minute_research_daily_metrics",
  {
    ...
    candidateId: (0, import_pg_core.integer)("candidateId").notNull(),
    tradingDate: (0, import_pg_core.varchar)("tradingDate", { length: 10 }).notNull(),
    symbolCount: (0, import_pg_core.integer)("symbolCount").default(0).notNull(),
    tradeCount: (0, import_pg_core.integer)("tradeCount").default(0).notNull(),
    winRate: (0, import_pg_core.decimal)("winRate", { precision: 9, scale: 4 }).default("0").notNull(),
    netReturnPercent: (0, import_pg_core.decimal)("netReturnPercent", { precision: 14, scale: 6 }).default("0").notNull(),
    expectancyPercent: (0, import_pg_core.decimal)("expectancyPercent", { precision: 14, scale: 6 }).default("0").notNull(),
    maxDrawdownPercent: (0, import_pg_core.decimal)("maxDrawdownPercent", { precision: 14, scale: 6 }).default("0").notNull(),
    metricsJson: (0, import_pg_core.json)("metricsJson").notNull(),
    createdAt: (0, import_pg_core.timestamp)("createdAt").defaultNow().notNull()
  },
  (table) => [(0, import_pg_core.uniqueIndex)("minute_research_daily_metric_unique").on(table.candidateId, table.tradingDate), (0, import_pg_core.index)("minute_research_daily_metric_sweep_date_idx").on(table.sweepId, table.tradingDate)]
);
var minuteResearchSymbolMetrics = (0, import_pg_core.pgTable)(
  ...
  (table) => [(0, import_pg_core.uniqueIndex)("minute_research_symbol_metric_unique").on(table.candidateId, table.tradingDate, table.symbol), (0, import_pg_core.index)("minute_research_symbol_metric_sweep_regime_idx").on(table.sweepId, table.regime, table.tradingDate)]
);
...
async function getDb() {
  if (!_db && process.env.DATABASE_URL) {
    try {
      _client = (0, import_postgres.default)(process.env.DATABASE_URL, { prepare: false, idle_timeout: 20, connect_timeout: 10, ssl: "require" });
...
var requireOperator = t.middleware(async (opts) => {
  ...
  if (!ctx.user || !isOperatorUser(ctx.user)) {
    throw new import_server2.TRPCError({ code: "FORBIDDEN", message: "\uC6B4\uC601\uC790 \uAD8C\uD55C\uC774 \uD544\uC694\uD569\uB2C8\uB2E4." });
  }
...
async function getLatestLocalSnapshotBars(db, symbol) {
  const datasets = await db.select().from(researchDatasets).where((0, import_drizzle_orm2.and)((0, import_drizzle_orm2.eq)(researchDatasets.qualityStatus, "ready"), (0, import_drizzle_orm2.like)(researchDatasets.versionKey, "local-ka10081:%"))).orderBy((0, import_drizzle_orm2.desc)(researchDatasets.readyAt), (0, import_drizzle_orm2.desc)(researchDatasets.id)).limit(12);
  for (const dataset of datasets) {
    const rows = await db.select().from(researchDailyBars).where((0, import_drizzle_orm2.and)((0, import_drizzle_orm2.eq)(researchDailyBars.datasetId, dataset.id), (0, import_drizzle_orm2.eq)(researchDailyBars.symbol, symbol))).orderBy((0, import_drizzle_orm2.asc)(researchDailyBars.date));
...
// server/routers/quant.ts
var ruleSchema = import_zod2.z.object({
  id: import_zod2.z.string().min(1),
  type: import_zod2.z.enum(["macd_rising", "ma_position", "high_return", "turnover"]),
  enabled: import_zod2.z.boolean(),
  weight: import_zod2.z.number().int().min(0).max(100),
  config: import_zod2.z.record(import_zod2.z.string(), import_zod2.z.union([import_zod2.z.string(), import_zod2.z.number(), import_zod2.z.boolean()]))
});
function requireDailyBarsForEvaluation(bars) {
  if (!bars.length) throw new import_server3.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC2E4\uB370\uC774\uD130 \uC5C6\uC74C: \uD0A4\uC6C0 ka10081\uC5D0\uC11C \uC77C\uBD09 \uB370\uC774\uD130\uB97C \uBC1B\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4. \uC9C0\uC815 \uB2E8\uB9D0\xB7OAuth \uC0C1\uD0DC\uC640 \uC885\uBAA9\uCF54\uB4DC\uB97C \uD655\uC778\uD558\uC138\uC694." });
  return bars;
}
function requireUserRequestedExternalVerification() {
  if (!isExternalResearchVerificationEnabled()) {
    throw new import_server3.TRPCError({ code: "PRECONDITION_FAILED", message: `${externalVerificationPausedMessage}: \uC0AC\uC6A9\uC790\uAC00 \uC694\uCCAD\uD558\uBA74 \uC77D\uAE30 \uC804\uC6A9 \uC77C\uBD09 \uC870\uD68C\uB97C \uC9C4\uD589\uD569\uB2C8\uB2E4.` });
  }
}
var barSchema = import_zod2.z.object({
  date: import_zod2.z.string(),
  open: import_zod2.z.number().positive(),
  high: import_zod2.z.number().positive(),
  low: import_zod2.z.number().positive(),
  close: import_zod2.z.number().positive(),
  volume: import_zod2.z.number().nonnegative(),
  turnover: import_zod2.z.number().nonnegative()
});
var quantRouter = router({
  ...
  evaluateConditions: publicProcedure.input(import_zod2.z.object({ rules: import_zod2.z.array(ruleSchema), bars: import_zod2.z.array(barSchema).min(1) })).query(({ input }) => evaluateStrategy(input.rules, input.bars)),
  rankCandidates: publicProcedure.input(import_zod2.z.object({
    rules: import_zod2.z.array(ruleSchema).min(1),
    candidates: import_zod2.z.array(import_zod2.z.object({ symbol: import_zod2.z.string().min(1), name: import_zod2.z.string().min(1), bars: import_zod2.z.array(barSchema).min(1) })),
    limit: import_zod2.z.number().int().min(1).max(500).default(200)
  })).query(({ input }) => rankCandidates(input.rules, input.candidates, input.limit)),
  dailyBars: operatorProcedure.input(import_zod2.z.object({
    symbol: import_zod2.z.string().regex(/^\d{6}$/, "\uAD6D\uB0B4\uC8FC\uC2DD 6\uC790\uB9AC \uC885\uBAA9\uCF54\uB4DC\uAC00 \uD544\uC694\uD569\uB2C8\uB2E4."),
    baseDate: import_zod2.z.string().regex(/^\d{8}$/).optional(),
    maxPages: import_zod2.z.number().int().min(1).max(10).default(3)
  })).query(async ({ input }) => {
    const db = await getDb();
    if (!db) throw new import_server3.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  ...
  evaluatePreset: operatorProcedure.input(import_zod2.z.object({
    presetId: import_zod2.z.number().int().positive(),
    symbol: import_zod2.z.string().regex(/^\d{6}$/, "\uAD6D\uB0B4\uC8FC\uC2DD 6\uC790\uB9AC \uC885\uBAA9\uCF54\uB4DC\uAC00 \uD544\uC694\uD569\uB2C8\uB2E4."),
    maxPages: import_zod2.z.number().int().min(1).max(10).default(3)
  })).query(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new import_server3.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const preset = (await db.select().from(strategyPresets).where((0, import_drizzle_orm3.and)((0, import_drizzle_orm3.eq)(strategyPresets.id, input.presetId), (0, import_drizzle_orm3.eq)(strategyPresets.userId, ctx.user.id))).limit(1))[0];
    if (!preset) throw new import_server3.TRPCError({ code: "NOT_FOUND", message: "\uD3C9\uAC00\uD560 \uD504\uB9AC\uC14B\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const rules = import_zod2.z.array(ruleSchema).parse(preset.rulesJson);
    const snapshot = await getLatestLocalSnapshotBars(db, input.symbol);
  ...
  createOrderIntent: operatorProcedure.input(import_zod2.z.object({
    symbol: import_zod2.z.string().min(1).max(24),
    name: import_zod2.z.string().min(1).max(120),
    side: import_zod2.z.enum(["buy", "sell"]),
    quantity: import_zod2.z.number().int().positive(),
    price: import_zod2.z.number().int().positive(),
    presetId: import_zod2.z.number().int().positive().optional()
  })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new import_server3.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const profile = (await db.select().from(tradingProfiles).where((0, import_drizzle_orm3.eq)(tradingProfiles.userId, ctx.user.id)).limit(1))[0];
    if (!profile) throw new import_server3.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC2E4\uAC70\uB798 \uC548\uC804 \uD55C\uB3C4\uB97C \uBA3C\uC800 \uC800\uC7A5\uD574\uC57C \uD569\uB2C8\uB2E4." });
    const settings = {
    ...
    const todayOrders = await db.select().from(orderIntents).where((0, import_drizzle_orm3.eq)(orderIntents.userId, ctx.user.id));
...
var ruleSchema2 = import_zod3.z.object({
  id: import_zod3.z.string().min(1),
  type: import_zod3.z.enum(["macd_rising", "ma_position", "high_return", "turnover"]),
  enabled: import_zod3.z.boolean(),
  weight: import_zod3.z.number().int().min(0).max(100),
  config: import_zod3.z.record(import_zod3.z.string(), import_zod3.z.union([import_zod3.z.string(), import_zod3.z.number(), import_zod3.z.boolean()]))
});
var expressionSchema = import_zod3.z.lazy(() => import_zod3.z.object({
  id: import_zod3.z.string().min(1),
  logic: import_zod3.z.enum(["AND", "OR", "NOT"]),
  enabled: import_zod3.z.boolean().default(true),
  children: import_zod3.z.array(import_zod3.z.union([ruleSchema2, expressionSchema])).min(1)
}));
var presetInput = import_zod3.z.object({
  name: import_zod3.z.string().trim().min(1).max(120),
  description: import_zod3.z.string().trim().max(500).optional(),
  rules: import_zod3.z.array(ruleSchema2).min(1),
  expression: expressionSchema.optional(),
  isActive: import_zod3.z.boolean().default(true)
});
async function requireDb() {
  const db = await getDb();
  if (!db) throw new import_server4.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  return db;
}
var presetsRouter = router({
  list: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb();
    return db.select().from(strategyPresets).where((0, import_drizzle_orm4.eq)(strategyPresets.userId, ctx.user.id)).orderBy((0, import_drizzle_orm4.desc)(strategyPresets.updatedAt));
  }),
  detail: operatorProcedure.input(import_zod3.z.object({ id: import_zod3.z.number().int().positive() })).query(async ({ ctx, input }) => {
    const db = await requireDb();
    const preset = (await db.select().from(strategyPresets).where((0, import_drizzle_orm4.and)((0, import_drizzle_orm4.eq)(strategyPresets.id, input.id), (0, import_drizzle_orm4.eq)(strategyPresets.userId, ctx.user.id))).limit(1))[0];
    if (!preset) throw new import_server4.TRPCError({ code: "NOT_FOUND", message: "\uC870\uD68C\uD560 \uD504\uB9AC\uC14B\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    return preset;
  }),
  save: operatorProcedure.input(presetInput.extend({ id: import_zod3.z.number().int().positive().optional() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb();
    ...
    if (input.id) {
      const result = await db.update(strategyPresets).set(values).where((0, import_drizzle_orm4.and)((0, import_drizzle_orm4.eq)(strategyPresets.id, input.id), (0, import_drizzle_orm4.eq)(strategyPresets.userId, ctx.user.id)));
      if (result.length === 0) throw new import_server4.TRPCError({ code: "NOT_FOUND", message: "\uC800\uC7A5\uD560 \uD504\uB9AC\uC14B\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
      return { id: input.id, updated: true };
    }
    const [created] = await db.insert(strategyPresets).values({ userId: ctx.user.id, ...values }).returning();
    return { id: created.id, updated: false };
  }),
  remove: operatorProcedure.input(import_zod3.z.object({ id: import_zod3.z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb();
    const result = await db.delete(strategyPresets).where((0, import_drizzle_orm4.and)((0, import_drizzle_orm4.eq)(strategyPresets.id, input.id), (0, import_drizzle_orm4.eq)(strategyPresets.userId, ctx.user.id)));
    if (result.length === 0) throw new import_server4.TRPCError({ code: "NOT_FOUND", message: "\uC0AD\uC81C\uD560 \uD504\uB9AC\uC14B\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    return { success: true };
  })
});
...
var mapForgeError = (response, detail, rpc) => {
  ...
  return new import_server5.TRPCError({
    code,
...
// server/routers/tradingProfile.ts
var safetyInput = import_zod4.z.object({
  maxBuyAmount: import_zod4.z.number().int().min(1e4).max(1e9),
  dailyTradeLimit: import_zod4.z.number().int().min(1).max(100),
  killSwitch: import_zod4.z.boolean(),
  autoTradeEnabled: import_zod4.z.boolean(),
  refreshIntervalSeconds: import_zod4.z.number().int().min(60).max(86400)
});
var autoPolicyInput = import_zod4.z.object({
  totalCapital: import_zod4.z.number().int().min(1e5).max(1e9),
  maxConcurrentPositions: import_zod4.z.number().int().min(1).max(20),
  stopLossPercent: import_zod4.z.number().min(0.1).max(30),
  takeProfitPercent: import_zod4.z.number().min(0.1).max(100),
  dailyLossLimitPercent: import_zod4.z.number().min(0.1).max(30)
});
async function requireDb2() {
  const db = await getDb();
  if (!db) throw new import_server6.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  return db;
}
var tradingProfileRouter = router({
  get: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb2();
    const profile = (await db.select().from(tradingProfiles).where((0, import_drizzle_orm5.eq)(tradingProfiles.userId, ctx.user.id)).limit(1))[0];
    return { profile: profile ?? null, broker: new KiwoomClient().getStatus() };
  }),
  getAutoPolicy: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb2();
    const policy = (await db.select().from(autoTradePolicies).where((0, import_drizzle_orm5.eq)(autoTradePolicies.userId, ctx.user.id)).orderBy((0, import_drizzle_orm5.desc)(autoTradePolicies.version)).limit(1))[0] ?? null;
    return { policy, broker: new KiwoomClient().getStatus() };
  }),
  setSimpleMode: operatorProcedure.input(import_zod4.z.object({ mode: import_zod4.z.enum(["paper", "live_ready"]) })).mutation(async ({ ctx, input }) => {
    const db = await requireDb2();
    const broker = new KiwoomClient().getStatus();
    if (input.mode === "live_ready" && (!broker.hasCredentials || !broker.fixedIpRegistered)) {
      throw new import_server6.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC2E4\uC804 \uC900\uBE44\uC5D0\uB294 \uD0A4\uC6C0 \uC778\uC99D \uC815\uBCF4\uC640 \uC9C0\uC815 \uACF5\uC778 IP \uB4F1\uB85D\uC774 \uBAA8\uB450 \uD544\uC694\uD569\uB2C8\uB2E4." });
    }
    const current = (await db.select().from(tradingProfiles).where((0, import_drizzle_orm5.eq)(tradingProfiles.userId, ctx.user.id)).limit(1))[0];
    const values = {
    ...
    if (current) {
      await db.update(tradingProfiles).set(values).where((0, import_drizzle_orm5.eq)(tradingProfiles.id, current.id));
  ...
  saveAutoPolicy: operatorProcedure.input(autoPolicyInput).mutation(async ({ ctx, input }) => {
    const db = await requireDb2();
    const broker = new KiwoomClient().getStatus();
    if (!broker.mayTransmitOrders) throw new import_server6.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC2E4\uC81C \uC8FC\uBB38 \uC804\uC1A1\uC774 \uD65C\uC131\uD654\uB41C \uD658\uACBD\uC5D0\uC11C\uB9CC \uC790\uB3D9 \uC2E4\uD22C \uC815\uCC45\uC744 \uC800\uC7A5\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const current = (await db.select().from(autoTradePolicies).where((0, import_drizzle_orm5.eq)(autoTradePolicies.userId, ctx.user.id)).orderBy((0, import_drizzle_orm5.desc)(autoTradePolicies.version)).limit(1))[0];
    const policy = normalizeAutoTradePolicy(input);
    if (current?.status === "active") await db.update(autoTradePolicies).set({ status: "superseded" }).where((0, import_drizzle_orm5.eq)(autoTradePolicies.id, current.id));
  ...
  saveSafety: operatorProcedure.input(safetyInput).mutation(async ({ ctx, input }) => {
    ...
    const existing = (await db.select({ id: tradingProfiles.id }).from(tradingProfiles).where((0, import_drizzle_orm5.eq)(tradingProfiles.userId, ctx.user.id)).limit(1))[0];
    if (existing) {
      await db.update(tradingProfiles).set(values).where((0, import_drizzle_orm5.eq)(tradingProfiles.id, existing.id));
  ...
  configureRankingRefresh: operatorProcedure.input(import_zod4.z.object({ cron: import_zod4.z.string().regex(/^\d+\s+\d+(?:[\d,*/-]*)\s+\d+(?:[\d,*/-]*)\s+[\d*/,-]+\s+[\d*/,-]+\s+[\d*/,-]+$/, "6\uD544\uB4DC UTC cron \uD615\uC2DD\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."), enabled: import_zod4.z.boolean() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb2();
    const profile = (await db.select().from(tradingProfiles).where((0, import_drizzle_orm5.eq)(tradingProfiles.userId, ctx.user.id)).limit(1))[0];
    if (!profile) throw new import_server6.TRPCError({ code: "PRECONDITION_FAILED", message: "\uB7AD\uD0B9 \uAC31\uC2E0 \uC124\uC815\uC744 \uC800\uC7A5\uD558\uAE30 \uC804\uC5D0 \uC2E4\uAC70\uB798 \uC548\uC804 \uD55C\uB3C4\uB97C \uC800\uC7A5\uD574\uC57C \uD569\uB2C8\uB2E4." });
    const sessionToken = (0, import_cookie3.parse)(ctx.req.headers.cookie ?? "")[COOKIE_NAME] ?? "";
    ...
    await db.update(tradingProfiles).set({ scheduleCronTaskUid: created.taskUid }).where((0, import_drizzle_orm5.eq)(tradingProfiles.id, profile.id));
    return created;
  })
});
...
async function requireDb3() {
  const db = await getDb();
  if (!db) throw new import_server7.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  return db;
}
function assertResearchObservationForOrder(observation) {
  if (!observation.candidateId || !(observation.source.startsWith("kiwoom_ka10032") || observation.source.startsWith("kiwoom_ka10081"))) {
    throw new import_server7.TRPCError({ code: "PRECONDITION_FAILED", message: "\uD0A4\uC6C0 \uC2E4\uC81C \uAC00\uACA9 \uAD00\uCC30\uACFC \uC5F0\uAD6C \uD6C4\uBCF4\uAC00 \uC5F0\uACB0\uB41C \uACBD\uC6B0\uC5D0\uB9CC \uC8FC\uBB38 \uCD08\uC548\uC744 \uB9CC\uB4E4 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
  }
}
var ordersRouter = router({
  list: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb3();
    return db.select().from(orderIntents).where((0, import_drizzle_orm6.eq)(orderIntents.userId, ctx.user.id));
  }),
  listExecutions: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb3();
    const intents = await db.select().from(orderIntents).where((0, import_drizzle_orm6.eq)(orderIntents.userId, ctx.user.id));
    const executions = await db.select().from(orderExecutions).orderBy((0, import_drizzle_orm6.desc)(orderExecutions.executedAt));
  ...
  createFromResearchObservation: operatorProcedure.input(import_zod5.z.object({ observationId: import_zod5.z.number().int().positive(), quantity: import_zod5.z.number().int().positive().max(1e6) })).mutation(async ({ ctx, input }) => {
    const db = await requireDb3();
    const observation = (await db.select().from(autonomousResearchObservations).where((0, import_drizzle_orm6.eq)(autonomousResearchObservations.id, input.observationId)).limit(1))[0];
    if (!observation) throw new import_server7.TRPCError({ code: "NOT_FOUND", message: "\uC2E4\uC81C \uAC00\uACA9 \uAD00\uCC30 \uAE30\uB85D\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    assertResearchObservationForOrder(observation);
    const profile = (await db.select().from(tradingProfiles).where((0, import_drizzle_orm6.eq)(tradingProfiles.userId, ctx.user.id)).limit(1))[0];
    if (!profile) throw new import_server7.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC8FC\uBB38 \uC548\uC804 \uD55C\uB3C4\uB97C \uBA3C\uC800 \uC800\uC7A5\uD574\uC57C \uD569\uB2C8\uB2E4." });
    const today = /* @__PURE__ */ new Date();
    today.setHours(0, 0, 0, 0);
    const todayOrders = await db.select().from(orderIntents).where((0, import_drizzle_orm6.eq)(orderIntents.userId, ctx.user.id));
    const confirmedToday = todayOrders.filter((order) => order.confirmedAt && order.confirmedAt >= today && ["confirmed", "submitted", "filled"].includes(order.status)).length;
  ...
  confirm: operatorProcedure.input(import_zod5.z.object({ id: import_zod5.z.number().int().positive(), acknowledged: import_zod5.z.literal(true) })).mutation(async ({ ctx, input }) => {
    const db = await requireDb3();
    const intent = (await db.select().from(orderIntents).where((0, import_drizzle_orm6.and)((0, import_drizzle_orm6.eq)(orderIntents.id, input.id), (0, import_drizzle_orm6.eq)(orderIntents.userId, ctx.user.id))).limit(1))[0];
    if (!intent) throw new import_server7.TRPCError({ code: "NOT_FOUND", message: "\uD655\uC778\uD560 \uC8FC\uBB38 \uC758\uB3C4\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    if (intent.status !== "pending_confirmation") throw new import_server7.TRPCError({ code: "PRECONDITION_FAILED", message: "\uD655\uC778 \uB300\uAE30 \uC0C1\uD0DC\uC758 \uC8FC\uBB38\uB9CC \uC2B9\uC778\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const confirmationNonce = (0, import_node_crypto.randomUUID)();
    await db.update(orderIntents).set({ status: "confirmed", confirmationNonce, confirmedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm6.eq)(orderIntents.id, intent.id));
    return { id: intent.id, status: "confirmed", confirmationNonce };
  }),
  transmit: operatorProcedure.input(import_zod5.z.object({ id: import_zod5.z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    if (!isExternalResearchVerificationEnabled()) {
      throw new import_server7.TRPCError({ code: "PRECONDITION_FAILED", message: `${externalVerificationPausedMessage}: \uC8FC\uBB38 \uC804\uC1A1\uC740 \uC5F0\uAD6C \uC804\uC6A9 \uC11C\uBE44\uC2A4\uC5D0\uC11C \uC2E4\uD589\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4.` });
    }
    const db = await requireDb3();
    const intent = (await db.select().from(orderIntents).where((0, import_drizzle_orm6.and)((0, import_drizzle_orm6.eq)(orderIntents.id, input.id), (0, import_drizzle_orm6.eq)(orderIntents.userId, ctx.user.id))).limit(1))[0];
    const profile = (await db.select().from(tradingProfiles).where((0, import_drizzle_orm6.eq)(tradingProfiles.userId, ctx.user.id)).limit(1))[0];
    if (!intent || !profile) throw new import_server7.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC8FC\uBB38 \uC758\uB3C4 \uB610\uB294 \uC2E4\uAC70\uB798 \uC548\uC804 \uC124\uC815\uC774 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const confirmedToday = (await db.select().from(orderIntents).where((0, import_drizzle_orm6.eq)(orderIntents.userId, ctx.user.id))).filter((order) => ["confirmed", "submitted", "filled"].includes(order.status)).length;
    const client = new KiwoomClient();
    try {
      ...
      throw new import_server7.TRPCError({ code: "PRECONDITION_FAILED", message });
    }
    const claim = await db.update(orderIntents).set({ status: "submitting" }).where((0, import_drizzle_orm6.and)(
      (0, import_drizzle_orm6.eq)(orderIntents.id, intent.id),
      (0, import_drizzle_orm6.eq)(orderIntents.userId, ctx.user.id),
      (0, import_drizzle_orm6.eq)(orderIntents.status, "confirmed")
    ));
    if (claim.length !== 1) {
      throw new import_server7.TRPCError({ code: "CONFLICT", message: "\uC774 \uC8FC\uBB38\uC740 \uC774\uBBF8 \uC804\uC1A1 \uCC98\uB9AC \uC911\uC774\uAC70\uB098 \uCC98\uB9AC\uB418\uC5C8\uC2B5\uB2C8\uB2E4." });
    }
    try {
      const token = await client.getAccessToken();
      ...
      await db.update(orderIntents).set({ status: "submitted", brokerOrderId: brokerOrder.orderNumber }).where((0, import_drizzle_orm6.and)((0, import_drizzle_orm6.eq)(orderIntents.id, intent.id), (0, import_drizzle_orm6.eq)(orderIntents.status, "submitting")));
      ...
      await db.update(orderIntents).set({ status: "rejected", riskReasonsJson: [message] }).where((0, import_drizzle_orm6.and)((0, import_drizzle_orm6.eq)(orderIntents.id, intent.id), (0, import_drizzle_orm6.eq)(orderIntents.status, "submitting")));
...
// server/routers/backtests.ts
var barSchema2 = import_zod6.z.object({
  date: import_zod6.z.string(),
  open: import_zod6.z.number().positive(),
  high: import_zod6.z.number().positive(),
  low: import_zod6.z.number().positive(),
  close: import_zod6.z.number().positive(),
  volume: import_zod6.z.number().nonnegative(),
  turnover: import_zod6.z.number().nonnegative()
});
var ruleSchema3 = import_zod6.z.object({
  id: import_zod6.z.string(),
  type: import_zod6.z.enum(["macd_rising", "macd_level", "ma_position", "high_return", "new_high", "turnover", "rsi", "bollinger", "stochastic", "atr_percent", "volume_ratio", "close_change", "gap_percent", "intrabar_position"]),
  enabled: import_zod6.z.boolean(),
  weight: import_zod6.z.number(),
  config: import_zod6.z.record(import_zod6.z.string(), import_zod6.z.union([import_zod6.z.string(), import_zod6.z.number(), import_zod6.z.boolean()]))
});
async function requireDb4() {
  const db = await getDb();
  if (!db) throw new import_server8.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  return db;
}
var backtestsRouter = router({
  list: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb4();
    return db.select().from(backtestRuns).where((0, import_drizzle_orm7.eq)(backtestRuns.userId, ctx.user.id)).orderBy((0, import_drizzle_orm7.desc)(backtestRuns.createdAt));
  }),
  run: operatorProcedure.input(import_zod6.z.object({
    presetId: import_zod6.z.number().int().positive(),
    bars: import_zod6.z.array(barSchema2).min(60),
    initialCapital: import_zod6.z.number().int().positive().default(1e7),
    minScore: import_zod6.z.number().min(0).max(100).default(70),
    holdingDays: import_zod6.z.number().int().min(1).max(60).default(5),
    feeRate: import_zod6.z.number().min(0).max(0.1).default(0)
  })).mutation(async ({ ctx, input }) => {
    const db = await requireDb4();
    const preset = (await db.select().from(strategyPresets).where((0, import_drizzle_orm7.and)((0, import_drizzle_orm7.eq)(strategyPresets.id, input.presetId), (0, import_drizzle_orm7.eq)(strategyPresets.userId, ctx.user.id))).limit(1))[0];
    if (!preset) throw new import_server8.TRPCError({ code: "NOT_FOUND", message: "\uBC31\uD14C\uC2A4\uD2B8\uD560 \uD504\uB9AC\uC14B\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const rules = import_zod6.z.array(ruleSchema3).parse(preset.rulesJson);
...
async function requireDb5() {
  const db = await getDb();
  if (!db) throw new import_server9.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  return db;
}
var accountRouter = router({
  listPositions: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb5();
    const snapshots = await db.select().from(positionSnapshots).where((0, import_drizzle_orm8.eq)(positionSnapshots.userId, ctx.user.id)).orderBy((0, import_drizzle_orm8.desc)(positionSnapshots.capturedAt));
  ...
  syncPositions: operatorProcedure.mutation(async ({ ctx }) => {
    if (!isExternalResearchVerificationEnabled()) {
      throw new import_server9.TRPCError({ code: "PRECONDITION_FAILED", message: `${externalVerificationPausedMessage}: \uACC4\uC88C\xB7\uD3EC\uC9C0\uC158 \uB3D9\uAE30\uD654\uB294 \uC5F0\uAD6C \uC804\uC6A9 \uBC94\uC704\uC5D0\uC11C \uC2E4\uD589\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4.` });
    }
    const db = await requireDb5();
    const client = new KiwoomClient();
    try {
      ...
      throw new import_server9.TRPCError({ code: "PRECONDITION_FAILED", message: error instanceof Error ? error.message : "\uACC4\uC88C \uB3D9\uAE30\uD654\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4." });
    }
  })
});
...
var ruleSchema4 = import_zod7.z.object({
  id: import_zod7.z.string(),
  type: import_zod7.z.enum(["macd_rising", "macd_level", "ma_position", "high_return", "new_high", "turnover", "rsi", "bollinger", "stochastic", "atr_percent", "volume_ratio", "close_change", "gap_percent", "intrabar_position"]),
  enabled: import_zod7.z.boolean(),
  weight: import_zod7.z.number(),
  config: import_zod7.z.record(import_zod7.z.string(), import_zod7.z.union([import_zod7.z.string(), import_zod7.z.number(), import_zod7.z.boolean()]))
});
async function refreshLiveRanking(input) {
  const db = await getDb();
  if (!db) throw new import_server10.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  const preset = (await db.select().from(strategyPresets).where((0, import_drizzle_orm9.and)((0, import_drizzle_orm9.eq)(strategyPresets.id, input.presetId), (0, import_drizzle_orm9.eq)(strategyPresets.userId, input.userId))).limit(1))[0];
  if (!preset) throw new import_server10.TRPCError({ code: "NOT_FOUND", message: "\uB7AD\uD0B9\uC5D0 \uC0AC\uC6A9\uD560 \uD504\uB9AC\uC14B\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  const rules = import_zod7.z.array(ruleSchema4).parse(preset.rulesJson);
  const candidates = [];
  const failedSymbols = [];
  ...
  if (ranked.length) {
    ...
    if (input.runKey) {
      await db.insert(rankingSnapshots).values(values).onConflictDoUpdate({
        ...
        set: { runKey: import_drizzle_orm9.sql`excluded.runKey` }
      });
    } else {
...
var rankingsRouter = router({
  turnover: operatorProcedure.input(import_zod8.z.object({
    market: import_zod8.z.enum(["000", "001", "101"]).default("000"),
    includeManagedStocks: import_zod8.z.boolean().default(false),
    exchange: import_zod8.z.enum(["KRX", "NXT", "INTEGRATED"]).default("KRX")
  })).query(async ({ input }) => {
    if (!isExternalResearchVerificationEnabled()) throw new import_server11.TRPCError({ code: "PRECONDITION_FAILED", message: `${externalVerificationPausedMessage}: \uC0AC\uC6A9\uC790\uAC00 \uC694\uCCAD\uD558\uBA74 \uC77D\uAE30 \uC804\uC6A9 \uAC70\uB798\uB300\uAE08 \uC21C\uC704 \uC870\uD68C\uB97C \uC9C4\uD589\uD569\uB2C8\uB2E4.` });
    const client = new KiwoomClient();
    const { token } = await client.getAccessToken();
    return client.getTurnoverRankings(token, input);
  }),
  refresh: operatorProcedure.input(import_zod8.z.object({
    presetId: import_zod8.z.number().int().positive(),
    universe: import_zod8.z.array(import_zod8.z.object({ symbol: import_zod8.z.string().regex(/^\d{6}$/), name: import_zod8.z.string().min(1).max(120).optional() })).min(1).max(20),
    maxPagesPerSymbol: import_zod8.z.number().int().min(1).max(10).default(3)
  })).mutation(async ({ ctx, input }) => {
  ...
  latest: publicProcedure.query(async () => {
    ...
    const [owner] = await db.select({ id: users.id }).from(users).where((0, import_drizzle_orm10.eq)(users.openId, ownerOpenId)).limit(1);
    if (!owner) return { capturedAt: null, items: [] };
    const [latestRow] = await db.select({ capturedAt: rankingSnapshots.capturedAt }).from(rankingSnapshots).where((0, import_drizzle_orm10.eq)(rankingSnapshots.userId, owner.id)).orderBy((0, import_drizzle_orm10.desc)(rankingSnapshots.capturedAt)).limit(1);
    if (!latestRow) return { capturedAt: null, items: [] };
    const items = await db.select().from(rankingSnapshots).where((0, import_drizzle_orm10.and)((0, import_drizzle_orm10.eq)(rankingSnapshots.userId, owner.id), (0, import_drizzle_orm10.eq)(rankingSnapshots.capturedAt, latestRow.capturedAt))).orderBy((0, import_drizzle_orm10.desc)(rankingSnapshots.score));
...
var universeSchema = import_zod9.z.array(import_zod9.z.object({ symbol: import_zod9.z.string().regex(/^\d{6}$/), name: import_zod9.z.string().min(1).max(120).optional() })).min(1).max(20);
var cronSchema = import_zod9.z.string().regex(/^\d+\s+\d+(?:[\d,*/-]*)\s+\d+(?:[\d,*/-]*)\s+[\d*/,-]+\s+[\d*/,-]+\s+[\d*/,-]+$/, "6\uD544\uB4DC UTC cron \uD615\uC2DD\uC774 \uD544\uC694\uD569\uB2C8\uB2E4.");
async function requireDb6() {
  const db = await getDb();
  if (!db) throw new import_server12.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  return db;
}
var rankingRefreshRouter = router({
  get: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb6();
    return (await db.select().from(rankingRefreshProfiles).where((0, import_drizzle_orm11.eq)(rankingRefreshProfiles.userId, ctx.user.id)).limit(1))[0] ?? null;
  }),
  save: operatorProcedure.input(import_zod9.z.object({ presetId: import_zod9.z.number().int().positive(), universe: universeSchema, maxPagesPerSymbol: import_zod9.z.number().int().min(1).max(10).default(3), cron: cronSchema, enabled: import_zod9.z.boolean() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb6();
    const existing = (await db.select().from(rankingRefreshProfiles).where((0, import_drizzle_orm11.eq)(rankingRefreshProfiles.userId, ctx.user.id)).limit(1))[0];
    const sessionToken = (0, import_cookie4.parse)(ctx.req.headers.cookie ?? "")[COOKIE_NAME] ?? "";
    ...
    if (existing?.scheduleCronTaskUid) {
      ...
      await db.update(rankingRefreshProfiles).set(baseValues).where((0, import_drizzle_orm11.eq)(rankingRefreshProfiles.id, existing.id));
    ...
    if (existing) {
      await db.update(rankingRefreshProfiles).set({ ...baseValues, scheduleCronTaskUid: created.taskUid }).where((0, import_drizzle_orm11.eq)(rankingRefreshProfiles.id, existing.id));
...
// server/routers/research.ts
init_evolution();
var dateSchema = import_zod10.z.string().regex(/^\d{4}-\d{2}-\d{2}$/);
var universeSchema2 = import_zod10.z.array(import_zod10.z.object({ symbol: import_zod10.z.string().regex(/^\d{6}$/), name: import_zod10.z.string().min(1).max(120).optional() })).min(1).max(500);
var storedRuleSchema = import_zod10.z.object({ id: import_zod10.z.string(), type: import_zod10.z.enum(["macd_rising", "ma_position", "high_return", "turnover"]), enabled: import_zod10.z.boolean(), weight: import_zod10.z.number(), config: import_zod10.z.record(import_zod10.z.string(), import_zod10.z.union([import_zod10.z.string(), import_zod10.z.number(), import_zod10.z.boolean()])) });
var assumptionsSchema = import_zod10.z.object({ entryTiming: import_zod10.z.enum(["next_open", "next_close"]), feeRate: import_zod10.z.number().min(0).max(0.1), slippageBps: import_zod10.z.number().min(0).max(1e4), maxHoldingDays: import_zod10.z.number().int().min(1).max(365), maxConcurrentPositions: import_zod10.z.number().int().min(1).max(500) });
var walkForwardConfigurationSchema = import_zod10.z.object({ experimentId: import_zod10.z.number().int().positive(), trainingDays: import_zod10.z.number().int().min(20).max(1e4), validationDays: import_zod10.z.number().int().min(5).max(1e4), stepDays: import_zod10.z.number().int().min(1).max(1e4) });
var evolutionRuleTypeSchema = import_zod10.z.enum(["macd_rising", "ma_position", "high_return", "turnover", "rsi", "bollinger", "stochastic", "atr_percent", "volume_ratio"]);
var evolutionSearchConfigurationSchema = import_zod10.z.object({
  populationSize: import_zod10.z.number().int().min(10).max(100),
  minRules: import_zod10.z.number().int().min(1).max(20),
  maxRules: import_zod10.z.number().int().min(1).max(20),
  maxDepth: import_zod10.z.number().int().min(1).max(5),
  allowedRuleTypes: import_zod10.z.array(evolutionRuleTypeSchema).min(1),
  eliteCount: import_zod10.z.number().int().min(1).max(50),
  crossoverRate: import_zod10.z.number().min(0).max(1),
  mutationRate: import_zod10.z.number().min(0).max(1),
  minimumTrades: import_zod10.z.number().int().min(1).max(1e3),
  maxDrawdownLimit: import_zod10.z.number().min(-100).max(0),
  holdingDays: import_zod10.z.number().int().min(1).max(365),
  feeRate: import_zod10.z.number().min(0).max(0.1),
  slippageBps: import_zod10.z.number().min(0).max(1e4),
  informationCutoffTradingDays: import_zod10.z.number().int().min(1).max(20),
  entryTiming: import_zod10.z.enum(["next_open", "next_close"])
});
var manualEvolutionChangeSchema = import_zod10.z.discriminatedUnion("kind", [
  import_zod10.z.object({ kind: import_zod10.z.literal("rule_numeric"), targetNodeId: import_zod10.z.string().min(1).max(160), key: import_zod10.z.string().min(1).max(80), next: import_zod10.z.number().finite().min(0).max(1e7) }),
  import_zod10.z.object({ kind: import_zod10.z.literal("group_logic"), targetNodeId: import_zod10.z.string().min(1).max(160), next: import_zod10.z.enum(["AND", "OR", "NOT"]) })
]);
async function requireDb7() {
  const db = await getDb();
  if (!db) throw new import_server13.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB9AC\uC11C\uCE58 \uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  return db;
}
var researchRouter = router({
  listDatasets: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb7();
    return db.select().from(researchDatasets).where((0, import_drizzle_orm13.eq)(researchDatasets.userId, ctx.user.id)).orderBy((0, import_drizzle_orm13.desc)(researchDatasets.createdAt));
  }),
  createDataset: operatorProcedure.input(import_zod10.z.object({
    name: import_zod10.z.string().trim().min(2).max(160),
    versionKey: import_zod10.z.string().trim().min(3).max(80),
    universe: universeSchema2,
    startDate: dateSchema,
    endDate: dateSchema,
    adjustmentBasis: import_zod10.z.enum(["adjusted", "unadjusted", "unknown"]).default("unknown")
  })).mutation(async ({ ctx, input }) => {
    if (input.startDate > input.endDate) throw new import_server13.TRPCError({ code: "BAD_REQUEST", message: "\uB370\uC774\uD130\uC14B \uC2DC\uC791\uC77C\uC740 \uC885\uB8CC\uC77C\uBCF4\uB2E4 \uB2A6\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const db = await requireDb7();
  ...
  collectDataset: operatorProcedure.input(import_zod10.z.object({ datasetId: import_zod10.z.number().int().positive(), maxPagesPerSymbol: import_zod10.z.number().int().min(1).max(10).default(10) })).mutation(async ({ ctx, input }) => {
    if (!isExternalResearchVerificationEnabled()) {
      throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: `${externalVerificationPausedMessage}: \uC0AC\uC6A9\uC790\uAC00 \uC694\uCCAD\uD558\uBA74 \uC77D\uAE30 \uC804\uC6A9 \uB9AC\uC11C\uCE58 \uB370\uC774\uD130\uC14B \uC218\uC9D1\uC744 \uC9C4\uD589\uD569\uB2C8\uB2E4.` });
    }
    const db = await requireDb7();
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDatasets.id, input.datasetId), (0, import_drizzle_orm13.eq)(researchDatasets.userId, ctx.user.id))).limit(1))[0];
    if (!dataset) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uC218\uC9D1\uD560 \uB9AC\uC11C\uCE58 \uB370\uC774\uD130\uC14B\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    if (dataset.qualityStatus !== "draft") throw new import_server13.TRPCError({ code: "CONFLICT", message: "\uCD08\uC548 \uB370\uC774\uD130\uC14B\uB9CC \uCD5C\uCD08 \uC218\uC9D1\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4. \uAE30\uC874 \uC6D0\uBCF8\uC744 \uBC14\uAFB8\uB824\uBA74 \uC0C8 \uBC84\uC804\uC744 \uB9CC\uB4DC\uC138\uC694." });
    if (dataset.adjustmentBasis === "unknown") throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC2E4\uC8FC\uAC00 \uB370\uC774\uD130\uC14B \uC218\uC9D1 \uC804 \uAC00\uACA9 \uC870\uC815 \uAE30\uC900\uC744 adjusted \uB610\uB294 unadjusted\uB85C \uD655\uC815\uD558\uC138\uC694." });
    const universe = universeSchema2.parse(dataset.universeJson);
    await db.update(researchDatasets).set({ qualityStatus: "collecting", qualityReportJson: { state: "collecting", requestedSymbols: universe.map((item) => item.symbol) } }).where((0, import_drizzle_orm13.eq)(researchDatasets.id, dataset.id));
    try {
      const client = new KiwoomClient();
      const token = await client.getAccessToken();
      const collected = [];
      ...
      await db.update(researchDatasets).set({ qualityStatus: "ready", barCount, sourceCapturedAt: /* @__PURE__ */ new Date(), readyAt: /* @__PURE__ */ new Date(), qualityReportJson: { state: "ready", source: "kiwoom_ka10081", adjustmentBasis: dataset.adjustmentBasis, symbols: collected, barCount } }).where((0, import_drizzle_orm13.eq)(researchDatasets.id, dataset.id));
      ...
      await db.update(researchDatasets).set({ qualityStatus: "error", qualityReportJson: { state: "error", error: message } }).where((0, import_drizzle_orm13.eq)(researchDatasets.id, dataset.id));
      throw new import_server13.TRPCError({ code: "BAD_GATEWAY", message: `\uB9AC\uC11C\uCE58 \uB370\uC774\uD130\uC14B \uC218\uC9D1 \uC2E4\uD328: ${message}` });
    }
  }),
  listEvolutionSearches: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb7();
    return db.select().from(evolutionSearches).where((0, import_drizzle_orm13.eq)(evolutionSearches.userId, ctx.user.id)).orderBy((0, import_drizzle_orm13.desc)(evolutionSearches.createdAt));
  }),
  listEvolutionGenerations: operatorProcedure.input(import_zod10.z.object({ searchId: import_zod10.z.number().int().positive() })).query(async ({ ctx, input }) => {
    const db = await requireDb7();
    const search = (await db.select().from(evolutionSearches).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(evolutionSearches.id, input.searchId), (0, import_drizzle_orm13.eq)(evolutionSearches.userId, ctx.user.id))).limit(1))[0];
    if (!search) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uC9C4\uD654\uD615 \uD0D0\uC0C9 \uAE30\uB85D\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    return db.select().from(evolutionGenerations).where((0, import_drizzle_orm13.eq)(evolutionGenerations.searchId, search.id)).orderBy((0, import_drizzle_orm13.asc)(evolutionGenerations.generationNumber));
  }),
  listEvolutionCandidates: operatorProcedure.input(import_zod10.z.object({ searchId: import_zod10.z.number().int().positive(), generationId: import_zod10.z.number().int().positive().optional() })).query(async ({ ctx, input }) => {
    const db = await requireDb7();
    const search = (await db.select().from(evolutionSearches).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(evolutionSearches.id, input.searchId), (0, import_drizzle_orm13.eq)(evolutionSearches.userId, ctx.user.id))).limit(1))[0];
    if (!search) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uC9C4\uD654\uD615 \uD0D0\uC0C9 \uAE30\uB85D\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    return db.select().from(evolutionCandidates).where(input.generationId ? (0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(evolutionCandidates.searchId, search.id), (0, import_drizzle_orm13.eq)(evolutionCandidates.generationId, input.generationId)) : (0, import_drizzle_orm13.eq)(evolutionCandidates.searchId, search.id)).orderBy((0, import_drizzle_orm13.desc)(evolutionCandidates.fitnessScore), (0, import_drizzle_orm13.desc)(evolutionCandidates.createdAt));
  }),
  listEvolutionGenerationSummaries: operatorProcedure.input(import_zod10.z.object({ searchId: import_zod10.z.number().int().positive() })).query(async ({ ctx, input }) => {
    const db = await requireDb7();
    const search = (await db.select().from(evolutionSearches).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(evolutionSearches.id, input.searchId), (0, import_drizzle_orm13.eq)(evolutionSearches.userId, ctx.user.id))).limit(1))[0];
    if (!search) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uC9C4\uD654\uD615 \uD0D0\uC0C9 \uAE30\uB85D\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const [generations, candidates] = await Promise.all([
      db.select().from(evolutionGenerations).where((0, import_drizzle_orm13.eq)(evolutionGenerations.searchId, search.id)).orderBy((0, import_drizzle_orm13.asc)(evolutionGenerations.generationNumber)),
      db.select().from(evolutionCandidates).where((0, import_drizzle_orm13.eq)(evolutionCandidates.searchId, search.id)).orderBy((0, import_drizzle_orm13.desc)(evolutionCandidates.fitnessScore))
    ]);
  ...
  createEvolutionSearch: operatorProcedure.input(import_zod10.z.object({
    datasetId: import_zod10.z.number().int().positive(),
    name: import_zod10.z.string().trim().min(2).max(160),
    randomSeed: import_zod10.z.number().int().min(1).max(2147483647),
    configuration: evolutionSearchConfigurationSchema
  })).mutation(async ({ ctx, input }) => {
    if (input.configuration.maxRules < input.configuration.minRules) throw new import_server13.TRPCError({ code: "BAD_REQUEST", message: "\uCD5C\uB300 \uADDC\uCE59 \uC218\uB294 \uCD5C\uC18C \uADDC\uCE59 \uC218\uBCF4\uB2E4 \uC791\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    if (input.configuration.eliteCount > input.configuration.populationSize) throw new import_server13.TRPCError({ code: "BAD_REQUEST", message: "\uC5D8\uB9AC\uD2B8 \uBCF4\uC874 \uC218\uB294 \uD6C4\uBCF4 \uC218\uB97C \uB118\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const db = await requireDb7();
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDatasets.id, input.datasetId), (0, import_drizzle_orm13.eq)(researchDatasets.userId, ctx.user.id))).limit(1))[0];
    if (!dataset || dataset.qualityStatus !== "ready") throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC9C4\uD654\uD615 \uD0D0\uC0C9\uC740 ready \uC0C1\uD0DC\uC758 \uACE0\uC815 \uC2E4\uC81C \uB370\uC774\uD130\uC14B\uC5D0\uC11C\uB9CC \uC2DC\uC791\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
  ...
  evaluateEvolutionCandidate: operatorProcedure.input(import_zod10.z.object({ candidateId: import_zod10.z.number().int().positive(), symbol: import_zod10.z.string().regex(/^\d{6}$/) })).mutation(async ({ ctx, input }) => {
    const db = await requireDb7();
    const candidate = (await db.select().from(evolutionCandidates).where((0, import_drizzle_orm13.eq)(evolutionCandidates.id, input.candidateId)).limit(1))[0];
    if (!candidate) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uD3C9\uAC00\uD560 \uC720\uC804\uC790 \uD6C4\uBCF4\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const search = (await db.select().from(evolutionSearches).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(evolutionSearches.id, candidate.searchId), (0, import_drizzle_orm13.eq)(evolutionSearches.userId, ctx.user.id))).limit(1))[0];
    if (!search) throw new import_server13.TRPCError({ code: "FORBIDDEN", message: "\uD574\uB2F9 \uC720\uC804\uC790 \uD6C4\uBCF4\uC5D0 \uC811\uADFC\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDatasets.id, search.datasetId), (0, import_drizzle_orm13.eq)(researchDatasets.userId, ctx.user.id))).limit(1))[0];
    if (!dataset || dataset.qualityStatus !== "ready") throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: "ready \uC0C1\uD0DC\uC758 \uACE0\uC815 \uC2E4\uC81C \uB370\uC774\uD130\uC14B\uC5D0\uC11C\uB9CC \uC720\uC804\uC790\uB97C \uD3C9\uAC00\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const configuration = evolutionSearchConfigurationSchema.parse(search.configurationJson);
    const bars = await db.select().from(researchDailyBars).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDailyBars.datasetId, dataset.id), (0, import_drizzle_orm13.eq)(researchDailyBars.symbol, input.symbol))).orderBy((0, import_drizzle_orm13.asc)(researchDailyBars.date));
    if (bars.length < 60) throw new import_server13.TRPCError({ code: "BAD_REQUEST", message: "\uC120\uD0DD \uC885\uBAA9\uC758 \uACE0\uC815 \uC77C\uBD09\uC774 60\uAC1C \uBBF8\uB9CC\uC785\uB2C8\uB2E4." });
    ...
    await db.update(evolutionCandidates).set({ status: "evaluated", inSampleMetricsJson, fitnessScore: String(fitnessScore), evaluatedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm13.eq)(evolutionCandidates.id, candidate.id));
    return { candidateId: candidate.id, datasetVersionKey: dataset.versionKey, symbol: input.symbol, metrics, fitnessScore };
  }),
  validateEvolutionCandidate: operatorProcedure.input(import_zod10.z.object({ candidateId: import_zod10.z.number().int().positive(), symbol: import_zod10.z.string().regex(/^\d{6}$/), validationStartDate: dateSchema })).mutation(async ({ ctx, input }) => {
    const db = await requireDb7();
    const candidate = (await db.select().from(evolutionCandidates).where((0, import_drizzle_orm13.eq)(evolutionCandidates.id, input.candidateId)).limit(1))[0];
    if (!candidate) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uAC80\uC99D\uD560 \uC720\uC804\uC790 \uD6C4\uBCF4\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const search = (await db.select().from(evolutionSearches).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(evolutionSearches.id, candidate.searchId), (0, import_drizzle_orm13.eq)(evolutionSearches.userId, ctx.user.id))).limit(1))[0];
    if (!search) throw new import_server13.TRPCError({ code: "FORBIDDEN", message: "\uD574\uB2F9 \uC720\uC804\uC790 \uD6C4\uBCF4\uC5D0 \uC811\uADFC\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDatasets.id, search.datasetId), (0, import_drizzle_orm13.eq)(researchDatasets.userId, ctx.user.id))).limit(1))[0];
    if (!dataset || dataset.qualityStatus !== "ready") throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: "ready \uC2E4\uC81C \uB370\uC774\uD130\uC14B\uC5D0\uC11C\uB9CC \uB3C5\uB9BD \uAC80\uC99D\uC744 \uC2E4\uD589\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const rows = await db.select().from(researchDailyBars).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDailyBars.datasetId, dataset.id), (0, import_drizzle_orm13.eq)(researchDailyBars.symbol, input.symbol))).orderBy((0, import_drizzle_orm13.asc)(researchDailyBars.date));
    const validationStartIndex = rows.findIndex((row) => row.date >= input.validationStartDate);
    if (validationStartIndex < 60) throw new import_server13.TRPCError({ code: "BAD_REQUEST", message: "\uB3C5\uB9BD \uAC80\uC99D \uC2DC\uC791\uC77C \uC774\uC804\uC5D0 \uC9C0\uD45C \uACC4\uC0B0\uC6A9 \uC77C\uBD09 60\uAC1C \uC774\uC0C1\uC774 \uD544\uC694\uD569\uB2C8\uB2E4." });
    const configuration = evolutionSearchConfigurationSchema.parse(search.configurationJson);
    ...
    await db.update(evolutionCandidates).set({ outOfSampleMetricsJson }).where((0, import_drizzle_orm13.eq)(evolutionCandidates.id, candidate.id));
  ...
  runEvolutionCandidateWalkForward: operatorProcedure.input(import_zod10.z.object({ candidateId: import_zod10.z.number().int().positive(), symbol: import_zod10.z.string().regex(/^\d{6}$/), trainingDays: import_zod10.z.number().int().min(60).max(1e4), validationDays: import_zod10.z.number().int().min(5).max(1e4), stepDays: import_zod10.z.number().int().min(1).max(1e4) })).mutation(async ({ ctx, input }) => {
    const db = await requireDb7();
    const candidate = (await db.select().from(evolutionCandidates).where((0, import_drizzle_orm13.eq)(evolutionCandidates.id, input.candidateId)).limit(1))[0];
    if (!candidate) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uC6CC\uD06C\uD3EC\uC6CC\uB4DC\uD560 \uC720\uC804\uC790 \uD6C4\uBCF4\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    if (candidate.status !== "survived") throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC6CC\uD06C\uD3EC\uC6CC\uB4DC\uB294 \uC120\uBC1C\uB41C \uC0DD\uC874 \uC720\uC804\uC790\uC5D0\uC11C\uB9CC \uC2E4\uD589\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const search = (await db.select().from(evolutionSearches).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(evolutionSearches.id, candidate.searchId), (0, import_drizzle_orm13.eq)(evolutionSearches.userId, ctx.user.id))).limit(1))[0];
    if (!search) throw new import_server13.TRPCError({ code: "FORBIDDEN", message: "\uD574\uB2F9 \uC720\uC804\uC790 \uD6C4\uBCF4\uC5D0 \uC811\uADFC\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDatasets.id, search.datasetId), (0, import_drizzle_orm13.eq)(researchDatasets.userId, ctx.user.id))).limit(1))[0];
    if (!dataset || dataset.qualityStatus !== "ready") throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: "ready \uC2E4\uC81C \uB370\uC774\uD130\uC14B\uC758 \uC0DD\uC874 \uC720\uC804\uC790\uB9CC \uC6CC\uD06C\uD3EC\uC6CC\uB4DC\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const configuration = evolutionSearchConfigurationSchema.parse(search.configurationJson);
    const rows = await db.select().from(researchDailyBars).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDailyBars.datasetId, dataset.id), (0, import_drizzle_orm13.eq)(researchDailyBars.symbol, input.symbol))).orderBy((0, import_drizzle_orm13.asc)(researchDailyBars.date));
    ...
    await db.update(evolutionCandidates).set({ walkForwardMetricsJson }).where((0, import_drizzle_orm13.eq)(evolutionCandidates.id, candidate.id));
    return { candidateId: candidate.id, datasetVersionKey: dataset.versionKey, symbol: input.symbol, result };
  }),
  manuallyExpandEvolutionCandidate: operatorProcedure.input(import_zod10.z.object({ candidateId: import_zod10.z.number().int().positive(), change: manualEvolutionChangeSchema })).mutation(async ({ ctx, input }) => {
    const db = await requireDb7();
    const parent = (await db.select().from(evolutionCandidates).where((0, import_drizzle_orm13.eq)(evolutionCandidates.id, input.candidateId)).limit(1))[0];
    if (!parent) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uD655\uC7A5\uD560 \uBD80\uBAA8 \uC720\uC804\uC790\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    if (parent.status !== "survived") throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: "\uD655\uC7A5 \uC2E4\uD5D8\uC740 \uC120\uBC1C\uB41C \uC0DD\uC874 \uC720\uC804\uC790\uC5D0\uC11C\uB9CC \uC2DC\uC791\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const search = (await db.select().from(evolutionSearches).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(evolutionSearches.id, parent.searchId), (0, import_drizzle_orm13.eq)(evolutionSearches.userId, ctx.user.id))).limit(1))[0];
    if (!search) throw new import_server13.TRPCError({ code: "FORBIDDEN", message: "\uD574\uB2F9 \uC720\uC804\uC790 \uD6C4\uBCF4\uC5D0 \uC811\uADFC\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDatasets.id, search.datasetId), (0, import_drizzle_orm13.eq)(researchDatasets.userId, ctx.user.id))).limit(1))[0];
    if (!dataset || dataset.qualityStatus !== "ready") throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: "ready \uC2E4\uC81C \uB370\uC774\uD130\uC14B\uC758 \uC0DD\uC874 \uC720\uC804\uC790\uB9CC \uD655\uC7A5\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const configuration = evolutionSearchConfigurationSchema.parse(search.configurationJson);
    ...
    const existing = (await db.select().from(evolutionCandidates).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(evolutionCandidates.searchId, search.id), (0, import_drizzle_orm13.eq)(evolutionCandidates.fingerprint, fingerprint2))).limit(1))[0];
    if (existing) throw new import_server13.TRPCError({ code: "CONFLICT", message: "\uAC19\uC740 \uC5F0\uAD6C \uAC00\uC815\uC744 \uAC00\uC9C4 \uC911\uBCF5 \uC720\uC804\uC790\uAC00 \uC774\uBBF8 \uC874\uC7AC\uD569\uB2C8\uB2E4." });
    const latestGeneration = (await db.select().from(evolutionGenerations).where((0, import_drizzle_orm13.eq)(evolutionGenerations.searchId, search.id)).orderBy((0, import_drizzle_orm13.desc)(evolutionGenerations.generationNumber)).limit(1))[0];
  ...
  advanceEvolutionGeneration: operatorProcedure.input(import_zod10.z.object({ searchId: import_zod10.z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb7();
    const search = (await db.select().from(evolutionSearches).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(evolutionSearches.id, input.searchId), (0, import_drizzle_orm13.eq)(evolutionSearches.userId, ctx.user.id))).limit(1))[0];
    if (!search) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uC9C4\uD654\uD615 \uD0D0\uC0C9 \uAE30\uB85D\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const configuration = evolutionSearchConfigurationSchema.parse(search.configurationJson);
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDatasets.id, search.datasetId), (0, import_drizzle_orm13.eq)(researchDatasets.userId, ctx.user.id))).limit(1))[0];
    if (!dataset) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uC9C4\uD654\uD615 \uD0D0\uC0C9 \uB370\uC774\uD130\uC14B\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const currentGeneration = (await db.select().from(evolutionGenerations).where((0, import_drizzle_orm13.eq)(evolutionGenerations.searchId, search.id)).orderBy((0, import_drizzle_orm13.desc)(evolutionGenerations.generationNumber)).limit(1))[0];
    if (!currentGeneration) throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC774\uC804 \uC138\uB300\uAC00 \uC5C6\uB294 \uD0D0\uC0C9\uC740 \uB2E4\uC74C \uC138\uB300\uB97C \uB9CC\uB4E4 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const candidates = await db.select().from(evolutionCandidates).where((0, import_drizzle_orm13.eq)(evolutionCandidates.generationId, currentGeneration.id)).orderBy((0, import_drizzle_orm13.desc)(evolutionCandidates.fitnessScore));
    ...
    if (scored.length < configuration.eliteCount) throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: `\uB2E4\uC74C \uC138\uB300\uC5D0\uB294 \uD3C9\uAC00 \uC644\uB8CC \uD6C4\uBCF4\uAC00 \uCD5C\uC18C ${configuration.eliteCount}\uAC1C \uD544\uC694\uD569\uB2C8\uB2E4.` });
    ...
    await Promise.all(candidates.map((candidate) => db.update(evolutionCandidates).set({ status: survivors.some((survivor) => survivor.candidateId === candidate.id) ? "survived" : candidate.status === "evaluated" ? "rejected" : candidate.status }).where((0, import_drizzle_orm13.eq)(evolutionCandidates.id, candidate.id))));
  ...
  listExperiments: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb7();
    return db.select().from(researchExperiments).where((0, import_drizzle_orm13.eq)(researchExperiments.userId, ctx.user.id)).orderBy((0, import_drizzle_orm13.desc)(researchExperiments.createdAt));
  }),
  createExperiment: operatorProcedure.input(import_zod10.z.object({
    datasetId: import_zod10.z.number().int().positive(),
    presetId: import_zod10.z.number().int().positive(),
    name: import_zod10.z.string().trim().min(2).max(160),
    datasetVersionKey: import_zod10.z.string().trim().min(3).max(80),
    strategyVersionLabel: import_zod10.z.string().trim().min(2).max(160),
    informationCutoffTradingDays: import_zod10.z.number().int().min(1).max(20),
    training: import_zod10.z.object({ startDate: dateSchema, endDate: dateSchema }).optional(),
    validation: import_zod10.z.object({ startDate: dateSchema, endDate: dateSchema }).optional(),
    ...
    const db = await requireDb7();
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDatasets.id, input.datasetId), (0, import_drizzle_orm13.eq)(researchDatasets.userId, ctx.user.id))).limit(1))[0];
    if (!dataset) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uB9AC\uC11C\uCE58 \uB370\uC774\uD130\uC14B\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    if (dataset.versionKey !== spec.datasetVersionKey) throw new import_server13.TRPCError({ code: "BAD_REQUEST", message: "\uC120\uD0DD\uD55C \uB370\uC774\uD130\uC14B\uACFC \uC2E4\uD5D8 \uB370\uC774\uD130\uC14B \uBC84\uC804\uC774 \uC77C\uCE58\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4." });
    const preset = (await db.select().from(strategyPresets).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(strategyPresets.id, input.presetId), (0, import_drizzle_orm13.eq)(strategyPresets.userId, ctx.user.id))).limit(1))[0];
    if (!preset) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uB9AC\uC11C\uCE58 \uC870\uAC74\uC2DD\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  ...
  runExperiment: operatorProcedure.input(import_zod10.z.object({ experimentId: import_zod10.z.number().int().positive(), symbol: import_zod10.z.string().regex(/^\d{6}$/), initialCapital: import_zod10.z.number().int().positive().default(1e7), minScore: import_zod10.z.number().min(0).max(100).default(70) })).mutation(async ({ ctx, input }) => {
    const db = await requireDb7();
    const experiment = (await db.select().from(researchExperiments).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchExperiments.id, input.experimentId), (0, import_drizzle_orm13.eq)(researchExperiments.userId, ctx.user.id))).limit(1))[0];
    if (!experiment) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uC2E4\uD589\uD560 \uB9AC\uC11C\uCE58 \uC2E4\uD5D8\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDatasets.id, experiment.datasetId), (0, import_drizzle_orm13.eq)(researchDatasets.userId, ctx.user.id))).limit(1))[0];
    if (!dataset || dataset.qualityStatus !== "ready") throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC6D0\uBCF8 \uC77C\uBD09\uC774 \uACE0\uC815\xB7\uAC80\uC99D\uB41C ready \uB370\uC774\uD130\uC14B\uC5D0\uC11C\uB9CC \uC2E4\uD5D8\uC744 \uC2E4\uD589\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const allBars = await db.select().from(researchDailyBars).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDailyBars.datasetId, dataset.id), (0, import_drizzle_orm13.eq)(researchDailyBars.symbol, input.symbol))).orderBy((0, import_drizzle_orm13.asc)(researchDailyBars.date));
    ...
    if (periodBars.length < 60) throw new import_server13.TRPCError({ code: "BAD_REQUEST", message: "\uC120\uD0DD \uC885\uBAA9\uC758 \uACE0\uC815 \uC77C\uBD09\uC774 \uC2E4\uD5D8 \uAE30\uAC04\uC5D0 60\uAC1C \uBBF8\uB9CC\uC785\uB2C8\uB2E4." });
    const snapshot = experiment.strategySnapshotJson;
    const rules = import_zod10.z.array(storedRuleSchema).parse(snapshot.rulesJson);
    ...
    await db.update(researchExperiments).set({ status: "completed", resultsJson, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm13.eq)(researchExperiments.id, experiment.id));
    return { experimentId: experiment.id, datasetVersionKey: dataset.versionKey, symbol: input.symbol, result };
  }),
  createWalkForwardRun: operatorProcedure.input(import_zod10.z.object({
    experimentId: import_zod10.z.number().int().positive(),
    trainingDays: import_zod10.z.number().int().min(20).max(1e4),
    validationDays: import_zod10.z.number().int().min(5).max(1e4),
    stepDays: import_zod10.z.number().int().min(1).max(1e4)
  })).mutation(async ({ ctx, input }) => {
    const db = await requireDb7();
    const experiment = (await db.select().from(researchExperiments).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchExperiments.id, input.experimentId), (0, import_drizzle_orm13.eq)(researchExperiments.userId, ctx.user.id))).limit(1))[0];
    if (!experiment) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uC6CC\uD06C\uD3EC\uC6CC\uB4DC \uB300\uC0C1 \uC2E4\uD5D8\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  ...
  listWalkForwardRuns: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb7();
    return db.select().from(walkForwardRuns).where((0, import_drizzle_orm13.eq)(walkForwardRuns.userId, ctx.user.id)).orderBy((0, import_drizzle_orm13.desc)(walkForwardRuns.createdAt));
  }),
  runWalkForward: operatorProcedure.input(import_zod10.z.object({ walkForwardRunId: import_zod10.z.number().int().positive(), symbol: import_zod10.z.string().regex(/^\d{6}$/), minScore: import_zod10.z.number().min(0).max(100).default(70) })).mutation(async ({ ctx, input }) => {
    const db = await requireDb7();
    const run = (await db.select().from(walkForwardRuns).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(walkForwardRuns.id, input.walkForwardRunId), (0, import_drizzle_orm13.eq)(walkForwardRuns.userId, ctx.user.id))).limit(1))[0];
    if (!run) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uC6CC\uD06C\uD3EC\uC6CC\uB4DC \uC2E4\uD589 \uAE30\uB85D\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const experiment = (await db.select().from(researchExperiments).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchExperiments.id, run.experimentId), (0, import_drizzle_orm13.eq)(researchExperiments.userId, ctx.user.id))).limit(1))[0];
    if (!experiment) throw new import_server13.TRPCError({ code: "NOT_FOUND", message: "\uC6CC\uD06C\uD3EC\uC6CC\uB4DC\uC5D0 \uC5F0\uACB0\uB41C \uC2E4\uD5D8\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDatasets.id, experiment.datasetId), (0, import_drizzle_orm13.eq)(researchDatasets.userId, ctx.user.id))).limit(1))[0];
    if (!dataset || dataset.qualityStatus !== "ready") throw new import_server13.TRPCError({ code: "PRECONDITION_FAILED", message: "ready \uC0C1\uD0DC\uC758 \uACE0\uC815 \uB370\uC774\uD130\uC14B\uC5D0\uC11C\uB9CC \uC6CC\uD06C\uD3EC\uC6CC\uB4DC\uB97C \uC2E4\uD589\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const configuration = walkForwardConfigurationSchema.parse(run.configurationJson);
    const allBars = await db.select().from(researchDailyBars).where((0, import_drizzle_orm13.and)((0, import_drizzle_orm13.eq)(researchDailyBars.datasetId, dataset.id), (0, import_drizzle_orm13.eq)(researchDailyBars.symbol, input.symbol))).orderBy((0, import_drizzle_orm13.asc)(researchDailyBars.date));
    const snapshot = experiment.strategySnapshotJson;
    const rules = import_zod10.z.array(storedRuleSchema).parse(snapshot.rulesJson);
    const assumptions = assumptionsSchema.parse(experiment.assumptionsJson);
    await db.update(walkForwardRuns).set({ status: "running" }).where((0, import_drizzle_orm13.eq)(walkForwardRuns.id, run.id));
    try {
      ...
      await db.update(walkForwardRuns).set({ status: "completed", resultsJson, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm13.eq)(walkForwardRuns.id, run.id));
      ...
      await db.update(walkForwardRuns).set({ status: "failed", resultsJson: { error: message } }).where((0, import_drizzle_orm13.eq)(walkForwardRuns.id, run.id));
      throw new import_server13.TRPCError({ code: "BAD_REQUEST", message });
    }
  })
});
...
var PublicHistoricalBacktestRunner = class {
  ...
  async runLocalSnapshotInternal(datasetId) {
    ...
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm14.eq)(researchDatasets.id, datasetId)).limit(1))[0];
    ...
    let run = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm14.eq)(autonomousResearchRuns.runKey, runKey)).limit(1))[0];
    ...
    if (!run) {
      ...
      run = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm14.eq)(autonomousResearchRuns.runKey, runKey)).limit(1))[0];
    }
    if (!run) return { status: "waiting", runId: null, message: "\uBD88\uBCC0 \uC2E4\uC81C \uC77C\uBD09 \uC2A4\uB0C5\uC0F7 \uC2E4\uD589 \uAE30\uB85D\uC744 \uB9CC\uB4E4\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4.", reused: true };
    let stage = "snapshot_prepare";
    try {
      const storedRows = await db.select().from(researchDailyBars).where((0, import_drizzle_orm14.eq)(researchDailyBars.datasetId, dataset.id)).orderBy(researchDailyBars.symbol, researchDailyBars.date);
  ...
  async runFresh() {
    ...
    try {
      ...
      const priorHistoricalRuns = await db.select({ universeJson: autonomousResearchRuns.universeJson }).from(autonomousResearchRuns).where((0, import_drizzle_orm14.and)((0, import_drizzle_orm14.eq)(autonomousResearchRuns.dataStatus, "ready"), (0, import_drizzle_orm14.like)(autonomousResearchRuns.runKey, `%${historicalMarker}%`))).orderBy((0, import_drizzle_orm14.desc)(autonomousResearchRuns.updatedAt)).limit(20);
  ...
  async runReuse() {
    ...
    const now = this.now();
    const historicalRuns = await db.select().from(autonomousResearchRuns).orderBy((0, import_drizzle_orm14.desc)(autonomousResearchRuns.updatedAt)).limit(40);
    ...
    for (const staleRunId of reuseRecovery.staleRunIds) {
      await db.update(autonomousResearchRuns).set({ phase: "incomplete", dataStatus: "incomplete", lastError: "\uC800\uC7A5 \uC2E4\uC81C \uC77C\uBD09 \uC7AC\uC2E4\uD589\uC774 \uC911\uB2E8\uB418\uC5B4 \uB2E4\uC74C \uC694\uCCAD\uC5D0\uC11C \uBCF5\uAD6C\uD588\uC2B5\uB2C8\uB2E4.", updatedAt: now }).where((0, import_drizzle_orm14.eq)(autonomousResearchRuns.id, staleRunId));
    }
    ...
    const storedRows = await db.select().from(autonomousResearchBars).where((0, import_drizzle_orm14.eq)(autonomousResearchBars.runId, sourceRunId)).orderBy(autonomousResearchBars.symbol, autonomousResearchBars.date);
    ...
    const run = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm14.eq)(autonomousResearchRuns.id, created.id)).limit(1))[0];
  ...
  async evaluateAndPersist(input) {
    ...
    const survivors = (await input.db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm14.eq)(autonomousResearchCandidates.runId, input.run.id)).orderBy((0, import_drizzle_orm14.desc)(autonomousResearchCandidates.fitnessScore))).filter((candidate) => candidate.status === "survived");
    for (const candidate of survivors) {
      ...
      await input.db.update(autonomousResearchCandidates).set({ walkForwardMetricsJson: { configuration: { trainingDays: 60, validationDays: 20, stepDays: 20 }, metrics: { totalReturn, maxDrawdown, tradeCount }, foldCount: folds.length }, updatedAt: input.now }).where((0, import_drizzle_orm14.eq)(autonomousResearchCandidates.id, candidate.id));
    }
    ...
    await input.db.update(autonomousResearchRuns).set({ phase: "completed", dataStatus: "ready", universeJson: input.universe.map((item) => ({ symbol: item.symbol, name: item.name })), summaryJson: summary, lastError: null, lastObservedAt: input.now, completedAt: input.now, updatedAt: input.now }).where((0, import_drizzle_orm14.eq)(autonomousResearchRuns.id, input.run.id));
  ...
  async waitForData(db, run, now, error) {
    ...
    await db.update(autonomousResearchRuns).set({ ...transition, updatedAt: now }).where((0, import_drizzle_orm14.eq)(autonomousResearchRuns.id, run.id));
...
async function getLatestResearchCommitteeReport(runId) {
  const db = await getDb();
  if (!db) return null;
  return (await db.select().from(researchCommitteeReports).where((0, import_drizzle_orm15.eq)(researchCommitteeReports.runId, runId)).orderBy((0, import_drizzle_orm15.desc)(researchCommitteeReports.updatedAt)).limit(1))[0] ?? null;
}
...
async function buildEvidence(runId) {
  ...
  const run = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm15.eq)(autonomousResearchRuns.id, runId)).limit(1))[0];
  ...
  const [candidateRows, barRows] = await Promise.all([
    db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm15.eq)(autonomousResearchCandidates.runId, run.id)).orderBy((0, import_drizzle_orm15.desc)(autonomousResearchCandidates.fitnessScore)).limit(20),
    db.select().from(autonomousResearchBars).where((0, import_drizzle_orm15.eq)(autonomousResearchBars.runId, sourceRunId)).orderBy(autonomousResearchBars.symbol, autonomousResearchBars.date)
  ]);
  ...
  const evidenceFingerprint = (0, import_node_crypto3.createHash)("sha256").update(JSON.stringify({ policy: RESEARCH_COMMITTEE_POLICY_VERSION, evidence: fingerprintEvidence })).digest("hex");
...
async function prepareResearchCommitteeRun(runId) {
  ...
  const existing = (await db.select().from(researchCommitteeReports).where((0, import_drizzle_orm15.and)((0, import_drizzle_orm15.eq)(researchCommitteeReports.runId, run.id), (0, import_drizzle_orm15.eq)(researchCommitteeReports.evidenceFingerprint, evidenceFingerprint))).limit(1))[0];
  ...
  if (existing) {
    ...
    await db.update(researchCommitteeReports).set({ sourceRunId, policyVersion: RESEARCH_COMMITTEE_POLICY_VERSION, model, status: "running", evidenceJson: evidence, memberReviewsJson: null, deliberationJson: null, lastError: null, startedAt: /* @__PURE__ */ new Date(), completedAt: null }).where((0, import_drizzle_orm15.eq)(researchCommitteeReports.id, reportId));
  } else {
  ...
  const report = (await db.select().from(researchCommitteeReports).where((0, import_drizzle_orm15.eq)(researchCommitteeReports.id, reportId)).limit(1))[0];
...
async function completeResearchCommittee(job) {
  try {
    ...
    await job.db.update(researchCommitteeReports).set({ status: "completed", memberReviewsJson: reviews, deliberationJson: deliberation, completedAt: /* @__PURE__ */ new Date(), lastError: null }).where((0, import_drizzle_orm15.eq)(researchCommitteeReports.id, job.reportId));
  } catch (error) {
    const message = error instanceof Error ? error.message.slice(0, 500) : "\uC704\uC6D0\uD68C \uBD84\uC11D\uC744 \uC644\uB8CC\uD558\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4.";
    await job.db.update(researchCommitteeReports).set({ status: "failed", lastError: message, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm15.eq)(researchCommitteeReports.id, job.reportId));
    throw error;
  }
}
...
async function calculateStoredDailyBarRevalidation(input) {
  const db = await getDb();
  if (!db) throw new Error("\uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uC5D0 \uC5F0\uACB0\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.");
  const run = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm16.eq)(autonomousResearchRuns.id, input.sourceRunId)).limit(1))[0];
  ...
  const [candidates, rows] = await Promise.all([
    db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm16.eq)(autonomousResearchCandidates.runId, run.id)).orderBy((0, import_drizzle_orm16.desc)(autonomousResearchCandidates.fitnessScore)).limit(20),
    db.select().from(autonomousResearchBars).where((0, import_drizzle_orm16.eq)(autonomousResearchBars.runId, barRunId)).orderBy(autonomousResearchBars.symbol, autonomousResearchBars.date)
  ]);
...
async function createOrReuseJob(input) {
  ...
  const existing = (await db.select().from(researchRevalidationJobs).where((0, import_drizzle_orm16.eq)(researchRevalidationJobs.jobFingerprint, jobFingerprint)).limit(1))[0];
  ...
  if (classification.scope === "external_verification") {
    if (existing) await db.update(researchRevalidationJobs).set({ status: "blocked", blocker: classification.blocker, lastError: null, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm16.eq)(researchRevalidationJobs.id, existing.id));
    ...
    return (await db.select().from(researchRevalidationJobs).where((0, import_drizzle_orm16.eq)(researchRevalidationJobs.jobFingerprint, jobFingerprint)).limit(1))[0];
  }
  let jobId = existing?.id;
  if (existing) await db.update(researchRevalidationJobs).set({ status: "running", blocker: null, lastError: null, startedAt: /* @__PURE__ */ new Date(), completedAt: null }).where((0, import_drizzle_orm16.eq)(researchRevalidationJobs.id, existing.id));
  else {
  ...
  try {
    ...
    await db.update(researchRevalidationJobs).set({ status: "completed", resultJson: result, completedAt: /* @__PURE__ */ new Date(), lastError: null }).where((0, import_drizzle_orm16.eq)(researchRevalidationJobs.id, jobId));
  } catch (error) {
    const message = error instanceof Error ? error.message.slice(0, 500) : "\uC800\uC7A5 \uC2E4\uC81C \uC77C\uBD09 \uB0B4\uBD80 \uC7AC\uD3C9\uAC00\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4.";
    await db.update(researchRevalidationJobs).set({ status: "failed", lastError: message, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm16.eq)(researchRevalidationJobs.id, jobId));
  }
  return (await db.select().from(researchRevalidationJobs).where((0, import_drizzle_orm16.eq)(researchRevalidationJobs.id, jobId)).limit(1))[0];
}
...
async function getLatestResearchGovernanceCycle() {
  const db = await getDb();
  if (!db) return null;
  const cycle = (await db.select().from(researchGovernanceCycles).orderBy((0, import_drizzle_orm17.desc)(researchGovernanceCycles.updatedAt)).limit(1))[0] ?? null;
...
async function prepareGovernanceCycle() {
  const db = await getDb();
  if (!db) throw new Error("\uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uC5D0 \uC5F0\uACB0\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.");
  const report = (await db.select().from(researchCommitteeReports).where((0, import_drizzle_orm17.eq)(researchCommitteeReports.status, "completed")).orderBy((0, import_drizzle_orm17.desc)(researchCommitteeReports.updatedAt)).limit(1))[0];
  if (!report) return { skipped: "completed-committee-report-not-found" };
  const source = buildCycleSource(report);
  const cycleFingerprint = (0, import_node_crypto5.createHash)("sha256").update(JSON.stringify({ policy: RESEARCH_GOVERNANCE_POLICY_VERSION, reportId: report.id, evidenceFingerprint: source.evidenceFingerprint })).digest("hex");
  const existing = (await db.select().from(researchGovernanceCycles).where((0, import_drizzle_orm17.eq)(researchGovernanceCycles.cycleFingerprint, cycleFingerprint)).limit(1))[0];
  ...
  if (existing) {
    cycleId = existing.id;
    await db.update(researchGovernanceCycles).set({ status: "running", managerModel: model, sourceSummaryJson: source, managerDirectiveJson: null, leaderFollowUpsJson: null, lastError: null, startedAt: /* @__PURE__ */ new Date(), completedAt: null }).where((0, import_drizzle_orm17.eq)(researchGovernanceCycles.id, cycleId));
  } else {
  ...
  const cycle = (await db.select().from(researchGovernanceCycles).where((0, import_drizzle_orm17.eq)(researchGovernanceCycles.id, cycleId)).limit(1))[0];
...
async function completeGovernanceCycle(job) {
  try {
    ...
    await job.db.update(researchGovernanceCycles).set({ status: "completed", managerDirectiveJson: { initial, final }, leaderFollowUpsJson: followUps, completedAt: /* @__PURE__ */ new Date(), lastError: null }).where((0, import_drizzle_orm17.eq)(researchGovernanceCycles.id, job.cycleId));
    ...
    await job.db.update(researchGovernanceCycles).set({ status: "failed", lastError: message, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm17.eq)(researchGovernanceCycles.id, job.cycleId));
  }
}
...
async function getAutonomousOperationsStatus() {
  ...
  const [latestRuns, historicalLookup, latestCommittee, latestCycle, schedule, revalidationRows] = await Promise.all([
    db.select().from(autonomousResearchRuns).orderBy((0, import_drizzle_orm18.desc)(autonomousResearchRuns.updatedAt)).limit(12),
    db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm18.like)(autonomousResearchRuns.runKey, "%:historical%")).orderBy((0, import_drizzle_orm18.desc)(autonomousResearchRuns.updatedAt)).limit(1).then((rows) => ({ rows, unavailable: false, error: null })).catch((error) => {
    ...
    db.select().from(researchCommitteeReports).where((0, import_drizzle_orm18.eq)(researchCommitteeReports.status, "completed")).orderBy((0, import_drizzle_orm18.desc)(researchCommitteeReports.updatedAt)).limit(1),
    db.select().from(researchGovernanceCycles).orderBy((0, import_drizzle_orm18.desc)(researchGovernanceCycles.updatedAt)).limit(1),
    db.select().from(researchGovernanceSchedules).orderBy((0, import_drizzle_orm18.desc)(researchGovernanceSchedules.updatedAt)).limit(1),
    db.select().from(researchRevalidationJobs).orderBy((0, import_drizzle_orm18.desc)(researchRevalidationJobs.updatedAt)).limit(12)
  ]);
...
async function persistDayTradeExperiment(input) {
  ...
  const sourceFingerprint = (0, import_node_crypto6.createHash)("sha256").update(JSON.stringify({ runId: input.run.id, tradingDate: input.run.tradingDate, entries: selected.map(({ candidate, entry, signalCount: count3 }) => [candidate.fingerprint, entry.symbol, entry.entryPrice, entry.lastPrice, entry.exitPrice, count3]) })).digest("hex");
  let experiment = (await db.select().from(dayTradeExperiments).where((0, import_drizzle_orm19.eq)(dayTradeExperiments.runId, input.run.id)).limit(1))[0];
  ...
  if (!experiment) {
    ...
    experiment = (await db.select().from(dayTradeExperiments).where((0, import_drizzle_orm19.eq)(dayTradeExperiments.runId, input.run.id)).limit(1))[0];
  } else {
    await db.update(dayTradeExperiments).set(summary).where((0, import_drizzle_orm19.eq)(dayTradeExperiments.id, experiment.id));
  }
...
async function syncDayTradeExperimentForRun(runId) {
  const db = await getDb();
  if (!db) throw new Error("\uB370\uC774\uD2B8\uB808\uC774\uB4DC \uC2E4\uD5D8 \uC774\uB825 \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.");
  const run = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm19.eq)(autonomousResearchRuns.id, runId)).limit(1))[0];
  if (!run) throw new Error("\uC800\uC7A5\uD560 \uC790\uB3D9 \uC5F0\uAD6C \uC2E4\uD589\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.");
  const candidates = await db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm19.eq)(autonomousResearchCandidates.runId, run.id));
...
async function getDayTradeHistory() {
  ...
  const positions = experiments.length ? await db.select().from(dayTradeExperimentPositions).where((0, import_drizzle_orm19.eq)(dayTradeExperimentPositions.experimentId, experiments[experiments.length - 1].id)).orderBy(dayTradeExperimentPositions.netPnl) : [];
  const allPositions = experiments.length ? await db.select().from(dayTradeExperimentPositions).limit(5e3) : [];
  const latestSyncEvent = (await db.select().from(localResearchNodeSyncEvents).orderBy((0, import_drizzle_orm19.desc)(localResearchNodeSyncEvents.createdAt)).limit(1))[0] ?? null;
...
async function getLatestMinuteValidationHistory() {
  ...
  const experiment = (await db.select().from(dayTradeExperiments).where((0, import_drizzle_orm20.eq)(dayTradeExperiments.status, "tracking")).orderBy((0, import_drizzle_orm20.desc)(dayTradeExperiments.updatedAt)).limit(1))[0] ?? null;
  if (!experiment) return { experiment: null, assumptions: null, results: [] };
  const positions = await db.select({ candidateId: dayTradeExperimentPositions.candidateId, symbol: dayTradeExperimentPositions.symbol, name: dayTradeExperimentPositions.name }).from(dayTradeExperimentPositions).where((0, import_drizzle_orm20.eq)(dayTradeExperimentPositions.experimentId, experiment.id));
  ...
  const [candidates, bars] = await Promise.all([
    db.select({ id: autonomousResearchCandidates.id, fingerprint: autonomousResearchCandidates.fingerprint, rootGenomeJson: autonomousResearchCandidates.rootGenomeJson, minimumScore: autonomousResearchCandidates.minimumScore }).from(autonomousResearchCandidates).where((0, import_drizzle_orm20.inArray)(autonomousResearchCandidates.id, candidateIds)),
    db.select().from(intradayMinuteBars).where((0, import_drizzle_orm20.and)((0, import_drizzle_orm20.eq)(intradayMinuteBars.tradingDate, experiment.tradingDate), (0, import_drizzle_orm20.inArray)(intradayMinuteBars.symbol, symbols))).orderBy((0, import_drizzle_orm20.asc)(intradayMinuteBars.symbol), (0, import_drizzle_orm20.asc)(intradayMinuteBars.minuteAt))
  ]);
...
async function getLatestHistoricalRunId() {
  const db = await getDb();
  if (!db) return null;
  const rows = await db.select({ id: autonomousResearchRuns.id }).from(autonomousResearchRuns).where((0, import_drizzle_orm21.and)((0, import_drizzle_orm21.eq)(autonomousResearchRuns.dataStatus, "ready"), (0, import_drizzle_orm21.like)(autonomousResearchRuns.runKey, "%:historical%"))).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchRuns.updatedAt)).limit(1);
...
var autonomousResearchRouter = router({
  latest: publicProcedure.input(import_zod11.z.object({ includeSimulation: import_zod11.z.boolean().optional() }).optional()).query(async ({ input }) => {
    const db = await getDb();
    ...
    const [runs, historicalRows] = await Promise.all([
      db.select().from(autonomousResearchRuns).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchRuns.updatedAt)).limit(24),
      db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm21.and)((0, import_drizzle_orm21.eq)(autonomousResearchRuns.dataStatus, "ready"), (0, import_drizzle_orm21.like)(autonomousResearchRuns.runKey, "%:historical%"))).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchRuns.updatedAt)).limit(1)
    ]);
    const run = runs.find((item) => !item.runKey.includes(":historical")) ?? null;
    const historicalRun = historicalRows[0] ?? null;
    if (!run) {
      const historicalCandidates2 = historicalRun ? await db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm21.eq)(autonomousResearchCandidates.runId, historicalRun.id)).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchCandidates.fitnessScore)).limit(20) : [];
    ...
    const [tasks, observations, candidates] = await Promise.all([
      db.select().from(autonomousResearchTasks).where((0, import_drizzle_orm21.eq)(autonomousResearchTasks.runId, run.id)).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchTasks.startedAt)).limit(12),
      db.select().from(autonomousResearchObservations).where((0, import_drizzle_orm21.eq)(autonomousResearchObservations.runId, run.id)).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchObservations.capturedAt)).limit(40),
      db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm21.eq)(autonomousResearchCandidates.runId, run.id)).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchCandidates.fitnessScore), (0, import_drizzle_orm21.desc)(autonomousResearchCandidates.updatedAt)).limit(20)
    ]);
    const historicalCandidates = historicalRun ? await db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm21.eq)(autonomousResearchCandidates.runId, historicalRun.id)).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchCandidates.fitnessScore)).limit(20) : [];
  ...
  auditTrail: publicProcedure.query(async () => {
    ...
    const runs = await db.select().from(autonomousResearchRuns).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchRuns.updatedAt)).limit(16);
    const runIds = runs.map((run) => run.id);
    const [tasks, dailyEvidence, candidateEvidence, minuteRows] = await Promise.all([
      runIds.length ? db.select().from(autonomousResearchTasks).where((0, import_drizzle_orm21.inArray)(autonomousResearchTasks.runId, runIds)).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchTasks.startedAt)).limit(80) : Promise.resolve([]),
      runIds.length ? db.select({ runId: autonomousResearchBars.runId, dailyBarRows: import_drizzle_orm21.sql`COUNT(*)`, dailySymbolCount: import_drizzle_orm21.sql`COUNT(DISTINCT ${autonomousResearchBars.symbol})`, firstDailyDate: import_drizzle_orm21.sql`MIN(${autonomousResearchBars.date})`, lastDailyDate: import_drizzle_orm21.sql`MAX(${autonomousResearchBars.date})`, lastDailyCapturedAt: import_drizzle_orm21.sql`MAX(${autonomousResearchBars.capturedAt})` }).from(autonomousResearchBars).where((0, import_drizzle_orm21.inArray)(autonomousResearchBars.runId, runIds)).groupBy(autonomousResearchBars.runId) : Promise.resolve([]),
      runIds.length ? db.select({ runId: autonomousResearchCandidates.runId, candidateRows: import_drizzle_orm21.sql`COUNT(*)`, survivedRows: import_drizzle_orm21.sql`SUM(CASE WHEN ${autonomousResearchCandidates.status} = 'survived' THEN 1 ELSE 0 END)`, rejectedRows: import_drizzle_orm21.sql`SUM(CASE WHEN ${autonomousResearchCandidates.status} = 'rejected' THEN 1 ELSE 0 END)`, lastCandidateUpdatedAt: import_drizzle_orm21.sql`MAX(${autonomousResearchCandidates.updatedAt})` }).from(autonomousResearchCandidates).where((0, import_drizzle_orm21.inArray)(autonomousResearchCandidates.runId, runIds)).groupBy(autonomousResearchCandidates.runId) : Promise.resolve([]),
      db.select({ minuteBarRows: import_drizzle_orm21.sql`COUNT(*)`, minuteTradingDateCount: import_drizzle_orm21.sql`COUNT(DISTINCT ${intradayMinuteBars.tradingDate})`, minuteSymbolCount: import_drizzle_orm21.sql`COUNT(DISTINCT ${intradayMinuteBars.symbol})`, firstMinuteAt: import_drizzle_orm21.sql`MIN(${intradayMinuteBars.minuteAt})`, lastMinuteAt: import_drizzle_orm21.sql`MAX(${intradayMinuteBars.minuteAt})`, lastMinuteCapturedAt: import_drizzle_orm21.sql`MAX(${intradayMinuteBars.capturedAt})` }).from(intradayMinuteBars)
    ]);
  ...
  historicalResearchInsights: publicProcedure.query(async () => {
    const db = await getDb();
    if (!db) return null;
    const runs = await db.select().from(autonomousResearchRuns).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchRuns.updatedAt)).limit(40);
    const run = runs.find((item) => item.dataStatus === "ready" && item.runKey.includes(":historical"));
    if (!run) return null;
    const summary = run.summaryJson;
    const sourceRunId = summary?.dataset?.sourceRunId ?? run.id;
    const [candidates, rows] = await Promise.all([
      db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm21.eq)(autonomousResearchCandidates.runId, run.id)).orderBy((0, import_drizzle_orm21.desc)(autonomousResearchCandidates.fitnessScore)).limit(20),
      db.select().from(autonomousResearchBars).where((0, import_drizzle_orm21.eq)(autonomousResearchBars.runId, sourceRunId)).orderBy(autonomousResearchBars.symbol, autonomousResearchBars.date)
    ]);
    const barsBySymbol = rows.reduce((result, row) => {
      (result[row.symbol] ??= []).push({ date: row.date, open: Number(row.open), high: Number(row.high), low: Number(row.low), close: Number(row.close), volume: Number(row.volume), turnover: Number(row.turnover) });
      return result;
  ...
  minuteCollectionStatus: publicProcedure.query(async () => {
    const db = await getDb();
    if (!db) return null;
    return (await db.select().from(localMinuteCollectionRequests).orderBy((0, import_drizzle_orm21.desc)(localMinuteCollectionRequests.updatedAt)).limit(1))[0] ?? null;
  }),
  minuteBackfillStatus: publicProcedure.query(async () => {
    ...
    const [summary] = await db.select({
      symbolCount: import_drizzle_orm21.sql`COUNT(DISTINCT ${intradayMinuteBars.symbol})`,
      tradingDateCount: import_drizzle_orm21.sql`COUNT(DISTINCT ${intradayMinuteBars.tradingDate})`,
      barCount: import_drizzle_orm21.sql`COUNT(*)`,
      firstTradingDate: import_drizzle_orm21.sql`MIN(${intradayMinuteBars.tradingDate})`,
      lastTradingDate: import_drizzle_orm21.sql`MAX(${intradayMinuteBars.tradingDate})`,
      lastCapturedAt: import_drizzle_orm21.sql`MAX(${intradayMinuteBars.capturedAt})`
    }).from(intradayMinuteBars).where(import_drizzle_orm21.sql`${intradayMinuteBars.tradingDate} LIKE ${`${year}-%`}`);
  ...
  mockOrderOperationStatus: operatorProcedure.query(async () => {
    ...
    const [policy, experiment] = await Promise.all([
      db.select().from(autoTradePolicies).where((0, import_drizzle_orm21.eq)(autoTradePolicies.status, "active")).orderBy((0, import_drizzle_orm21.desc)(autoTradePolicies.updatedAt)).limit(1).then((rows) => rows[0] ?? null),
      db.select().from(dayTradeExperiments).where((0, import_drizzle_orm21.eq)(dayTradeExperiments.tradingDate, tradingDate2)).orderBy((0, import_drizzle_orm21.desc)(dayTradeExperiments.updatedAt)).limit(1).then((rows) => rows[0] ?? null)
    ]);
    const [profile, positions, recentExecutions] = await Promise.all([
      policy ? db.select().from(tradingProfiles).where((0, import_drizzle_orm21.eq)(tradingProfiles.userId, policy.userId)).limit(1).then((rows) => rows[0] ?? null) : Promise.resolve(null),
      experiment ? db.select().from(dayTradeExperimentPositions).where((0, import_drizzle_orm21.eq)(dayTradeExperimentPositions.experimentId, experiment.id)).orderBy((0, import_drizzle_orm21.desc)(dayTradeExperimentPositions.signalCount), dayTradeExperimentPositions.symbol) : Promise.resolve([]),
      policy ? db.select({
      ...
      }).from(orderIntents).leftJoin(orderExecutions, (0, import_drizzle_orm21.eq)(orderExecutions.orderIntentId, orderIntents.id)).where((0, import_drizzle_orm21.and)(
        (0, import_drizzle_orm21.eq)(orderIntents.userId, policy.userId),
        (0, import_drizzle_orm21.eq)(orderIntents.autoPolicyId, policy.id),
        (0, import_drizzle_orm21.eq)(orderIntents.executionOrigin, "local_node"),
        (0, import_drizzle_orm21.like)(orderIntents.dedupeKey, `auto:${tradingDate2}:%`)
      )).orderBy((0, import_drizzle_orm21.desc)(orderExecutions.executedAt), (0, import_drizzle_orm21.desc)(orderIntents.updatedAt)).limit(30) : Promise.resolve([])
    ]);
  ...
  requestMinuteCollection: protectedProcedure.mutation(async () => {
    ...
    const [existing] = await db.select().from(localMinuteCollectionRequests).where((0, import_drizzle_orm21.and)((0, import_drizzle_orm21.eq)(localMinuteCollectionRequests.tradingDate, tradingDate2), (0, import_drizzle_orm21.inArray)(localMinuteCollectionRequests.status, ["queued", "running"]))).orderBy((0, import_drizzle_orm21.desc)(localMinuteCollectionRequests.updatedAt)).limit(1);
    ...
    const request = (await db.select().from(localMinuteCollectionRequests).where((0, import_drizzle_orm21.eq)(localMinuteCollectionRequests.id, inserted.id)).limit(1))[0];
    return { request, reusedPendingRequest: false };
  }),
  syncDayTradeHistory: operatorProcedure.input(import_zod11.z.object({ runId: import_zod11.z.number().int().positive() })).mutation(async ({ input }) => syncDayTradeExperimentForRun(input.runId)),
  dayTradePositionDetail: publicProcedure.input(import_zod11.z.object({ candidateId: import_zod11.z.number().int().positive(), symbol: import_zod11.z.string().regex(/^\d{6}$/) })).query(async ({ input }) => {
    const db = await getDb();
    if (!db) return null;
    const position = (await db.select().from(dayTradeExperimentPositions).where((0, import_drizzle_orm21.and)((0, import_drizzle_orm21.eq)(dayTradeExperimentPositions.candidateId, input.candidateId), (0, import_drizzle_orm21.eq)(dayTradeExperimentPositions.symbol, input.symbol))).orderBy((0, import_drizzle_orm21.desc)(dayTradeExperimentPositions.updatedAt)).limit(1))[0];
    if (!position) return null;
    const [candidate, experiment] = await Promise.all([
      db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm21.eq)(autonomousResearchCandidates.id, position.candidateId)).limit(1),
      db.select().from(dayTradeExperiments).where((0, import_drizzle_orm21.eq)(dayTradeExperiments.id, position.experimentId)).limit(1)
    ]);
    if (!candidate[0] || !experiment[0]) return null;
    const bars = await db.select({ minuteAt: intradayMinuteBars.minuteAt, open: intradayMinuteBars.open, high: intradayMinuteBars.high, low: intradayMinuteBars.low, close: intradayMinuteBars.close, volume: intradayMinuteBars.volume }).from(intradayMinuteBars).where((0, import_drizzle_orm21.and)((0, import_drizzle_orm21.eq)(intradayMinuteBars.symbol, position.symbol), (0, import_drizzle_orm21.eq)(intradayMinuteBars.tradingDate, experiment[0].tradingDate))).orderBy(intradayMinuteBars.minuteAt);
    return {
  ...
  historicalCandidateDetail: publicProcedure.input(import_zod11.z.object({ candidateId: import_zod11.z.number().int().positive() })).query(async ({ input }) => {
    const db = await getDb();
    if (!db) return null;
    const candidate = (await db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm21.eq)(autonomousResearchCandidates.id, input.candidateId)).limit(1))[0];
    if (!candidate) return null;
    const run = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm21.eq)(autonomousResearchRuns.id, candidate.runId)).limit(1))[0];
    if (!run || !run.runKey.includes(":historical")) return null;
    const summary = run.summaryJson;
    const sourceRunId = summary?.dataset?.sourceRunId ?? run.id;
    const rows = await db.select().from(autonomousResearchBars).where((0, import_drizzle_orm21.eq)(autonomousResearchBars.runId, sourceRunId)).orderBy(autonomousResearchBars.symbol, autonomousResearchBars.date);
    const barsBySymbol = rows.reduce((result, row) => {
      (result[row.symbol] ??= []).push({ date: row.date, open: Number(row.open), high: Number(row.high), low: Number(row.low), close: Number(row.close), volume: Number(row.volume), turnover: Number(row.turnover) });
      return result;
    }, {});
...
function assertActualObservationSource(source) {
  if (!isActualObservationSource(source)) {
    throw new import_server14.TRPCError({ code: "PRECONDITION_FAILED", message: "\uD0A4\uC6C0 \uC2E4\uC81C \uAC00\uACA9 \uAD00\uCC30\uB9CC \uBAA8\uC758 \uD3EC\uD2B8\uD3F4\uB9AC\uC624\uC758 \uAC00\uACA9 \uADFC\uAC70\uB85C \uC0AC\uC6A9\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
  }
}
async function requireDb8() {
  const db = await getDb();
  if (!db) throw new import_server14.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uB370\uC774\uD130\uBCA0\uC774\uC2A4 \uC5F0\uACB0\uC744 \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  return db;
}
async function findActualObservation(db, observationId) {
  const observation = (await db.select().from(autonomousResearchObservations).where((0, import_drizzle_orm22.eq)(autonomousResearchObservations.id, observationId)).limit(1))[0];
  if (!observation) throw new import_server14.TRPCError({ code: "NOT_FOUND", message: "\uC2E4\uC81C \uAC00\uACA9 \uAD00\uCC30 \uAE30\uB85D\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  assertActualObservationSource(observation.source);
  if (!observation.candidateId) throw new import_server14.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC5F0\uAD6C \uD6C4\uBCF4\uC640 \uC5F0\uACB0\uB41C \uC2E4\uC81C \uAC00\uACA9 \uAD00\uCC30\uB9CC \uBAA8\uC758 \uD3EC\uD2B8\uD3F4\uB9AC\uC624\uC5D0 \uCD94\uAC00\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
  return observation;
}
var paperPortfolioRouter = router({
  latestActualObservations: operatorProcedure.input(import_zod12.z.object({ limit: import_zod12.z.number().int().min(1).max(100).default(20) })).query(async ({ input }) => {
    const db = await requireDb8();
    const observations = await db.select().from(autonomousResearchObservations).orderBy((0, import_drizzle_orm22.desc)(autonomousResearchObservations.capturedAt)).limit(input.limit * 5);
  ...
  list: operatorProcedure.query(async ({ ctx }) => {
    const db = await requireDb8();
    const portfolios = await db.select().from(paperPortfolios).where((0, import_drizzle_orm22.eq)(paperPortfolios.userId, ctx.user.id)).orderBy((0, import_drizzle_orm22.desc)(paperPortfolios.updatedAt));
    const positions = await db.select().from(paperPositions).orderBy((0, import_drizzle_orm22.desc)(paperPositions.updatedAt));
    return portfolios.map((portfolio) => ({ ...portfolio, positions: positions.filter((position) => position.portfolioId === portfolio.id) }));
  }),
  openFromObservation: operatorProcedure.input(import_zod12.z.object({ observationId: import_zod12.z.number().int().positive(), quantity: import_zod12.z.number().int().positive().max(1e6) })).mutation(async ({ ctx, input }) => {
    const db = await requireDb8();
  ...
  markFromObservation: operatorProcedure.input(import_zod12.z.object({ positionId: import_zod12.z.number().int().positive(), observationId: import_zod12.z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb8();
    const position = (await db.select().from(paperPositions).where((0, import_drizzle_orm22.eq)(paperPositions.id, input.positionId)).limit(1))[0];
    if (!position || position.status !== "open") throw new import_server14.TRPCError({ code: "PRECONDITION_FAILED", message: "\uAC00\uACA9\uC744 \uAC31\uC2E0\uD560 \uC5F4\uB9B0 \uBAA8\uC758 \uD3EC\uC9C0\uC158\uC774 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const portfolio = (await db.select().from(paperPortfolios).where((0, import_drizzle_orm22.and)((0, import_drizzle_orm22.eq)(paperPortfolios.id, position.portfolioId), (0, import_drizzle_orm22.eq)(paperPortfolios.userId, ctx.user.id))).limit(1))[0];
    if (!portfolio) throw new import_server14.TRPCError({ code: "FORBIDDEN", message: "\uC774 \uBAA8\uC758 \uD3EC\uD2B8\uD3F4\uB9AC\uC624\uC5D0 \uC811\uADFC\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const observation = await findActualObservation(db, input.observationId);
    if (observation.symbol !== position.symbol) throw new import_server14.TRPCError({ code: "PRECONDITION_FAILED", message: "\uAC19\uC740 \uC885\uBAA9\uC758 \uC2E4\uC81C \uAC00\uACA9 \uAD00\uCC30\uB9CC \uD3EC\uC9C0\uC158\uC5D0 \uBC18\uC601\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const unrealizedPnl = (observation.price - position.entryPrice) * position.quantity;
    await db.update(paperPositions).set({ latestPrice: observation.price, unrealizedPnl }).where((0, import_drizzle_orm22.eq)(paperPositions.id, position.id));
  ...
  closeFromObservation: operatorProcedure.input(import_zod12.z.object({ positionId: import_zod12.z.number().int().positive(), observationId: import_zod12.z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb8();
    const position = (await db.select().from(paperPositions).where((0, import_drizzle_orm22.eq)(paperPositions.id, input.positionId)).limit(1))[0];
    if (!position || position.status !== "open") throw new import_server14.TRPCError({ code: "PRECONDITION_FAILED", message: "\uC885\uB8CC\uD560 \uC5F4\uB9B0 \uBAA8\uC758 \uD3EC\uC9C0\uC158\uC774 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const portfolio = (await db.select().from(paperPortfolios).where((0, import_drizzle_orm22.and)((0, import_drizzle_orm22.eq)(paperPortfolios.id, position.portfolioId), (0, import_drizzle_orm22.eq)(paperPortfolios.userId, ctx.user.id))).limit(1))[0];
    if (!portfolio) throw new import_server14.TRPCError({ code: "FORBIDDEN", message: "\uC774 \uBAA8\uC758 \uD3EC\uD2B8\uD3F4\uB9AC\uC624\uC5D0 \uC811\uADFC\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const observation = await findActualObservation(db, input.observationId);
    if (observation.symbol !== position.symbol) throw new import_server14.TRPCError({ code: "PRECONDITION_FAILED", message: "\uAC19\uC740 \uC885\uBAA9\uC758 \uC2E4\uC81C \uAC00\uACA9 \uAD00\uCC30\uB9CC \uD3EC\uC9C0\uC158 \uC885\uB8CC\uC5D0 \uBC18\uC601\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const realizedPnl = (observation.price - position.entryPrice) * position.quantity;
    await db.update(paperPositions).set({ latestPrice: observation.price, unrealizedPnl: realizedPnl, status: "closed", closedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm22.eq)(paperPositions.id, position.id));
    await db.update(paperPortfolios).set({ cashBalance: portfolio.cashBalance + observation.price * position.quantity }).where((0, import_drizzle_orm22.eq)(paperPortfolios.id, portfolio.id));
...
var import_cookie5 = require("cookie");
...
function datasetFingerprint(rows) {
  ...
  return (0, import_node_crypto7.createHash)("sha256").update(material).digest("hex");
}
async function runMinuteResearchSweep(programId) {
  const db = await getDb();
  if (!db) throw new Error("1\uBD84\uBD09 \uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.");
  const program = (await db.select().from(minuteResearchPrograms).where((0, import_drizzle_orm23.eq)(minuteResearchPrograms.id, programId)).limit(1))[0];
  ...
  const dateRows = await db.selectDistinct({ tradingDate: intradayMinuteBars.tradingDate }).from(intradayMinuteBars).orderBy((0, import_drizzle_orm23.desc)(intradayMinuteBars.tradingDate)).limit(configuration.lookbackTradingDays);
  const dates = dateRows.map((row) => row.tradingDate).sort();
  if (!dates.length) {
    await db.update(minuteResearchPrograms).set({ lastError: "\uC218\uC9D1\uB41C 1\uBD84\uBD09 \uB370\uC774\uD130\uAC00 \uC5C6\uC5B4 \uC5F0\uAD6C\uB97C \uC2DC\uC791\uD558\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4." }).where((0, import_drizzle_orm23.eq)(minuteResearchPrograms.id, program.id));
    return { status: "waiting_for_data", programId: program.id };
  }
  const rows = await db.select().from(intradayMinuteBars).where((0, import_drizzle_orm23.inArray)(intradayMinuteBars.tradingDate, dates)).orderBy((0, import_drizzle_orm23.asc)(intradayMinuteBars.tradingDate), (0, import_drizzle_orm23.asc)(intradayMinuteBars.symbol), (0, import_drizzle_orm23.asc)(intradayMinuteBars.minuteAt));
  const fingerprint2 = datasetFingerprint(rows);
  const configurationFingerprint = (0, import_node_crypto7.createHash)("sha256").update(JSON.stringify(configuration)).digest("hex").slice(0, 16);
  const runKey = `minute-v1:${program.id}:${dates.at(-1)}:${fingerprint2.slice(0, 16)}:${configurationFingerprint}`;
  let existing = (await db.select().from(minuteResearchSweeps).where((0, import_drizzle_orm23.eq)(minuteResearchSweeps.runKey, runKey)).limit(1))[0];
  ...
  if (existing?.status === "running") {
    ...
    await db.update(minuteResearchSweeps).set({ status: "failed", lastError: staleMessage, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm23.eq)(minuteResearchSweeps.id, existing.id));
  ...
  if (!sweep) {
    ...
    sweep = (await db.select().from(minuteResearchSweeps).where((0, import_drizzle_orm23.eq)(minuteResearchSweeps.id, created.id)).limit(1))[0];
  }
  ...
  try {
    ...
    const seed = Number.parseInt((0, import_node_crypto7.createHash)("sha256").update(runKey).digest("hex").slice(0, 8), 16) || 1;
    const previousSurvivors = program.lastSweepId ? await db.select().from(minuteResearchCandidates).where((0, import_drizzle_orm23.and)((0, import_drizzle_orm23.eq)(minuteResearchCandidates.sweepId, program.lastSweepId), (0, import_drizzle_orm23.eq)(minuteResearchCandidates.status, "promoted"))).limit(1e3) : [];
    ...
    await db.update(minuteResearchSweeps).set({ generatedCount: genomes.length, evaluatedCount: 0, promotedCount: 0, rejectedCount: 0, lastError: null }).where((0, import_drizzle_orm23.eq)(minuteResearchSweeps.id, sweep.id));
    let promotedCount = 0;
    let rejectedCount = 0;
    ...
    for (let offset = 0; offset < genomes.length; offset += MINUTE_RESEARCH_EVALUATION_BATCH_SIZE) {
      ...
      for (const [index2, item] of Array.from(evaluated.entries())) {
        ...
        await db.update(minuteResearchCandidates).set({ collectedPresetId: preset.id }).where((0, import_drizzle_orm23.eq)(minuteResearchCandidates.id, candidateId));
      ...
      await db.update(minuteResearchSweeps).set({
        ...
        promotedCount,
        rejectedCount
      }).where((0, import_drizzle_orm23.eq)(minuteResearchSweeps.id, sweep.id));
    ...
    await db.update(minuteResearchSweeps).set({ status: "completed", generatedCount: genomes.length, evaluatedCount: genomes.length, promotedCount, rejectedCount, summaryJson: summary, lastError: null, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm23.eq)(minuteResearchSweeps.id, sweep.id));
    await db.update(minuteResearchPrograms).set({ lastSweepId: sweep.id, lastError: null }).where((0, import_drizzle_orm23.eq)(minuteResearchPrograms.id, program.id));
    return { status: "completed", programId: program.id, sweepId: sweep.id, ...summary };
  } catch (error) {
    const message = summarizeMinuteResearchError(error);
    await db.update(minuteResearchSweeps).set({ status: "failed", lastError: message, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm23.eq)(minuteResearchSweeps.id, sweep.id));
    await db.update(minuteResearchPrograms).set({ lastError: message }).where((0, import_drizzle_orm23.eq)(minuteResearchPrograms.id, program.id));
    throw error;
  }
}
async function getMinuteResearchDashboard(userId) {
  ...
  const program = (await db.select().from(minuteResearchPrograms).where((0, import_drizzle_orm23.eq)(minuteResearchPrograms.userId, userId)).limit(1))[0] ?? null;
  const sweeps = program ? await db.select().from(minuteResearchSweeps).where((0, import_drizzle_orm23.eq)(minuteResearchSweeps.programId, program.id)).orderBy((0, import_drizzle_orm23.desc)(minuteResearchSweeps.updatedAt)).limit(12) : [];
  const candidateRows = sweeps.length ? await db.select().from(minuteResearchCandidates).where((0, import_drizzle_orm23.inArray)(minuteResearchCandidates.sweepId, sweeps.map((item) => item.id))).orderBy((0, import_drizzle_orm23.desc)(minuteResearchCandidates.fitnessScore)).limit(500) : [];
  const latestSweep = sweeps[0] ?? null;
  const latestCandidates = latestSweep ? await db.select().from(minuteResearchCandidates).where((0, import_drizzle_orm23.eq)(minuteResearchCandidates.sweepId, latestSweep.id)).orderBy((0, import_drizzle_orm23.desc)(minuteResearchCandidates.fitnessScore)).limit(1e4) : [];
  ...
  const symbolMetrics = promoted.length ? await db.select().from(minuteResearchSymbolMetrics).where((0, import_drizzle_orm23.inArray)(minuteResearchSymbolMetrics.candidateId, promoted.map((candidate) => candidate.id))).limit(1e4) : [];
  ...
  const coverage = await db.selectDistinct({ tradingDate: intradayMinuteBars.tradingDate }).from(intradayMinuteBars).orderBy((0, import_drizzle_orm23.desc)(intradayMinuteBars.tradingDate)).limit(30);
...
async function getPublicMinuteResearchDashboard() {
  const db = await getDb();
  if (!db) return { program: null, sweeps: [], promoted: [], cumulative: [], commonRuleTypes: [], distribution: null, failureReasons: [], regimePerformance: [], symbolPerformance: [], dataCoverage: null };
  const program = (await db.select().from(minuteResearchPrograms).orderBy((0, import_drizzle_orm23.desc)(minuteResearchPrograms.updatedAt)).limit(1))[0];
...
async function getMinuteResearchProgramByTaskUid(taskUid) {
  const db = await getDb();
  if (!db) return null;
  return (await db.select().from(minuteResearchPrograms).where((0, import_drizzle_orm23.and)((0, import_drizzle_orm23.eq)(minuteResearchPrograms.scheduleCronTaskUid, taskUid), (0, import_drizzle_orm23.eq)(minuteResearchPrograms.status, "active"))).limit(1))[0] ?? null;
}
...
// server/routers/minuteResearch.ts
var configSchema = import_zod13.z.object({
  combinationsPerSweep: import_zod13.z.number().int().min(100).max(5e4),
  maxUniverseSymbols: import_zod13.z.number().int().min(1).max(100),
  lookbackTradingDays: import_zod13.z.number().int().min(1).max(60),
  validationTradingDays: import_zod13.z.number().int().min(1).max(20),
  minimumTrades: import_zod13.z.number().int().min(1).max(1e4),
  minimumValidationTrades: import_zod13.z.number().int().min(1).max(1e4),
  maxDrawdownPercent: import_zod13.z.number().min(-100).max(0),
  stopLossPercent: import_zod13.z.number().positive().max(30),
  takeProfitPercent: import_zod13.z.number().positive().max(100),
  maxHoldingBars: import_zod13.z.number().int().min(1).max(390),
  feeRate: import_zod13.z.number().min(0).max(0.1),
  slippageBps: import_zod13.z.number().min(0).max(500),
  explorationMode: import_zod13.z.enum(["survivor_core", "diverse_random"]).default("survivor_core")
});
...
async function preparePersonalArenaProgram(userId, configuration) {
  ...
  const current = (await db.select().from(minuteResearchPrograms).where((0, import_drizzle_orm24.eq)(minuteResearchPrograms.userId, userId)).limit(1))[0];
  ...
  await db.update(minuteResearchPrograms).set({
    ...
    configurationJson: configuration,
    lastError: null
  }).where((0, import_drizzle_orm24.eq)(minuteResearchPrograms.id, current.id));
  return current.id;
}
var minuteResearchRouter = router({
  ...
  saveProgram: operatorProcedure.input(import_zod13.z.object({
    name: import_zod13.z.string().trim().min(2).max(120),
    cronExpression: import_zod13.z.string().regex(/^\S+\s+\S+\s+\S+\s+\S+\s+\S+\s+\S+$/),
    configuration: configSchema.default(DEFAULT_MINUTE_RESEARCH_CONFIGURATION),
    enabled: import_zod13.z.boolean().default(true)
  })).mutation(async ({ ctx, input }) => {
    const db = await requireDb9();
    const current = (await db.select().from(minuteResearchPrograms).where((0, import_drizzle_orm24.eq)(minuteResearchPrograms.userId, ctx.user.id)).limit(1))[0];
    const sessionToken = (0, import_cookie5.parse)(ctx.req.headers.cookie ?? "")[COOKIE_NAME] ?? "";
    if (!current) {
    ...
    if (!current.scheduleCronTaskUid) {
      const job2 = await createHeartbeatJob({ name: `minute-research-${ctx.user.id}`, cron: input.cronExpression, path: "/api/scheduled/minute-research", payload: {}, description: "\uC2E4\uC81C \uC800\uC7A5 1\uBD84\uBD09 \uC870\uAC74\uC2DD \uB300\uB7C9 \uAC80\uC99D" }, sessionToken);
      await db.update(minuteResearchPrograms).set({ name: input.name, status: input.enabled ? "active" : "paused", cronExpression: input.cronExpression, scheduleCronTaskUid: job2.taskUid, configurationJson: input.configuration }).where((0, import_drizzle_orm24.eq)(minuteResearchPrograms.id, current.id));
      if (!input.enabled) await updateHeartbeatJob(job2.taskUid, { enable: false }, sessionToken);
    ...
    await db.update(minuteResearchPrograms).set({ name: input.name, status: input.enabled ? "active" : "paused", cronExpression: input.cronExpression, configurationJson: input.configuration }).where((0, import_drizzle_orm24.eq)(minuteResearchPrograms.id, current.id));
  ...
  runNow: operatorProcedure.mutation(async ({ ctx }) => {
    const db = await requireDb9();
    const program = (await db.select().from(minuteResearchPrograms).where((0, import_drizzle_orm24.and)((0, import_drizzle_orm24.eq)(minuteResearchPrograms.userId, ctx.user.id), (0, import_drizzle_orm24.eq)(minuteResearchPrograms.status, "active"))).limit(1))[0];
  ...
  removeSchedule: operatorProcedure.mutation(async ({ ctx }) => {
    const db = await requireDb9();
    const program = (await db.select().from(minuteResearchPrograms).where((0, import_drizzle_orm24.eq)(minuteResearchPrograms.userId, ctx.user.id)).limit(1))[0];
    if (!program?.scheduleCronTaskUid) return { removed: false };
    const sessionToken = (0, import_cookie5.parse)(ctx.req.headers.cookie ?? "")[COOKIE_NAME] ?? "";
    await deleteHeartbeatJob(program.scheduleCronTaskUid, sessionToken);
    await db.update(minuteResearchPrograms).set({ status: "paused", scheduleCronTaskUid: null }).where((0, import_drizzle_orm24.eq)(minuteResearchPrograms.id, program.id));
    return { removed: true };
  })
});
...
var strategyCardsRouter = router({
  myCollectionAnalysis: protectedProcedure.query(async ({ ctx }) => {
    ...
    const presets = await db.select().from(strategyPresets).where((0, import_drizzle_orm25.eq)(strategyPresets.userId, ctx.user.id)).orderBy((0, import_drizzle_orm25.desc)(strategyPresets.updatedAt)).limit(120);
    const presetIds = presets.map((preset) => preset.id);
    const candidates = presetIds.length ? await db.select().from(minuteResearchCandidates).where((0, import_drizzle_orm25.inArray)(minuteResearchCandidates.collectedPresetId, presetIds)).orderBy((0, import_drizzle_orm25.desc)(minuteResearchCandidates.createdAt)).limit(360) : [];
    const candidateIds = candidates.map((candidate) => candidate.id);
    const daily = candidateIds.length ? await db.select().from(minuteResearchDailyMetrics).where((0, import_drizzle_orm25.inArray)(minuteResearchDailyMetrics.candidateId, candidateIds)).orderBy((0, import_drizzle_orm25.desc)(minuteResearchDailyMetrics.tradingDate)).limit(1200) : [];
    const symbols = candidateIds.length ? await db.select().from(minuteResearchSymbolMetrics).where((0, import_drizzle_orm25.inArray)(minuteResearchSymbolMetrics.candidateId, candidateIds)).orderBy((0, import_drizzle_orm25.desc)(minuteResearchSymbolMetrics.tradingDate)).limit(1500) : [];
    const sweepIds = Array.from(new Set(candidates.map((candidate) => candidate.sweepId)));
    const sweeps = sweepIds.length ? await db.select().from(minuteResearchSweeps).where((0, import_drizzle_orm25.inArray)(minuteResearchSweeps.id, sweepIds)).limit(120) : [];
  ...
  listComments: publicProcedure.input(import_zod14.z.object({ cardId: import_zod14.z.number().int().positive() })).query(async ({ input }) => {
    const db = await requireDb10();
    const comments = await db.select().from(publicStrategyCardComments).where((0, import_drizzle_orm25.eq)(publicStrategyCardComments.cardId, input.cardId)).orderBy((0, import_drizzle_orm25.desc)(publicStrategyCardComments.createdAt)).limit(50);
    const commenters = comments.length ? await db.select({ id: users.id, name: users.name }).from(users).where((0, import_drizzle_orm25.inArray)(users.id, comments.map((comment) => comment.userId))) : [];
  ...
  addComment: protectedProcedure.input(import_zod14.z.object({ cardId: import_zod14.z.number().int().positive(), body: import_zod14.z.string().trim().min(1, "\uB313\uAE00 \uB0B4\uC6A9\uC744 \uC785\uB825\uD558\uC138\uC694.").max(800, "\uB313\uAE00\uC740 800\uC790\uAE4C\uC9C0 \uC785\uB825\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4.") })).mutation(async ({ ctx, input }) => {
    const db = await requireDb10();
    const card = (await db.select({ id: publicStrategyCards.id }).from(publicStrategyCards).where((0, import_drizzle_orm25.and)((0, import_drizzle_orm25.eq)(publicStrategyCards.id, input.cardId), (0, import_drizzle_orm25.eq)(publicStrategyCards.visibility, "public"))).limit(1))[0];
  ...
  toggleFavorite: protectedProcedure.input(import_zod14.z.object({ cardId: import_zod14.z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb10();
    const card = (await db.select({ id: publicStrategyCards.id }).from(publicStrategyCards).where((0, import_drizzle_orm25.and)((0, import_drizzle_orm25.eq)(publicStrategyCards.id, input.cardId), (0, import_drizzle_orm25.eq)(publicStrategyCards.visibility, "public"))).limit(1))[0];
    if (!card) throw new Error("\uACF5\uAC1C\uB41C \uC804\uB7B5 \uCE74\uB4DC\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.");
    const existing = (await db.select().from(publicStrategyCardFavorites).where((0, import_drizzle_orm25.and)((0, import_drizzle_orm25.eq)(publicStrategyCardFavorites.cardId, card.id), (0, import_drizzle_orm25.eq)(publicStrategyCardFavorites.userId, ctx.user.id))).limit(1))[0];
    if (existing) await db.delete(publicStrategyCardFavorites).where((0, import_drizzle_orm25.eq)(publicStrategyCardFavorites.id, existing.id));
    else await db.insert(publicStrategyCardFavorites).values({ cardId: card.id, userId: ctx.user.id });
    const count3 = (await db.select().from(publicStrategyCardFavorites).where((0, import_drizzle_orm25.eq)(publicStrategyCardFavorites.cardId, card.id))).length;
    return { favorited: !existing, favoriteCount: count3 };
  }),
  listHeroConditions: protectedProcedure.query(async ({ ctx }) => {
    const db = await requireDb10();
    const snapshots = await db.select().from(htsConditionSnapshots).where((0, import_drizzle_orm25.eq)(htsConditionSnapshots.userId, ctx.user.id)).orderBy((0, import_drizzle_orm25.desc)(htsConditionSnapshots.capturedAt)).limit(100);
  ...
  collectHeroCondition: protectedProcedure.input(import_zod14.z.object({ snapshotId: import_zod14.z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb10();
    const snapshot = (await db.select().from(htsConditionSnapshots).where((0, import_drizzle_orm25.and)((0, import_drizzle_orm25.eq)(htsConditionSnapshots.id, input.snapshotId), (0, import_drizzle_orm25.eq)(htsConditionSnapshots.userId, ctx.user.id))).limit(1))[0];
  ...
  listPublic: publicProcedure.input(import_zod14.z.object({ limit: import_zod14.z.number().int().min(1).max(100).default(24) }).optional()).query(async ({ input, ctx }) => {
    const db = await requireDb10();
    const cards = await db.select().from(publicStrategyCards).where((0, import_drizzle_orm25.eq)(publicStrategyCards.visibility, "public")).orderBy((0, import_drizzle_orm25.desc)(publicStrategyCards.publishedAt)).limit(input?.limit ?? 24);
    const creators = cards.length ? await db.select({ id: users.id, name: users.name }).from(users).where((0, import_drizzle_orm25.inArray)(users.id, cards.map((card) => card.creatorUserId))) : [];
    const creatorName = new Map(creators.map((user) => [user.id, user.name || "\uC775\uBA85 \uC5F0\uAD6C\uC790"]));
    const cardIds = cards.map((card) => card.id);
    const comments = cardIds.length ? await db.select().from(publicStrategyCardComments).where((0, import_drizzle_orm25.inArray)(publicStrategyCardComments.cardId, cardIds)).limit(1e4) : [];
    const favorites = cardIds.length ? await db.select().from(publicStrategyCardFavorites).where((0, import_drizzle_orm25.inArray)(publicStrategyCardFavorites.cardId, cardIds)).limit(1e4) : [];
  ...
  publish: protectedProcedure.input(import_zod14.z.object({ candidateId: import_zod14.z.number().int().positive(), title: import_zod14.z.string().trim().min(2).max(120).optional() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb10();
    const candidate = (await db.select().from(minuteResearchCandidates).where((0, import_drizzle_orm25.eq)(minuteResearchCandidates.id, input.candidateId)).limit(1))[0];
    if (!candidate || candidate.status !== "promoted") throw new Error("\uB3C5\uB9BD \uAC80\uC99D\uC744 \uD1B5\uACFC\uD55C \uC804\uB7B5 \uCE74\uB4DC\uB9CC \uACF5\uAC1C\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4.");
    const sweep = (await db.select().from(minuteResearchSweeps).where((0, import_drizzle_orm25.eq)(minuteResearchSweeps.id, candidate.sweepId)).limit(1))[0];
    const program = sweep ? (await db.select().from(minuteResearchPrograms).where((0, import_drizzle_orm25.and)((0, import_drizzle_orm25.eq)(minuteResearchPrograms.id, sweep.programId), (0, import_drizzle_orm25.eq)(minuteResearchPrograms.userId, ctx.user.id))).limit(1))[0] : null;
    if (!sweep || !program) throw new Error("\uBCF8\uC778\uC774 \uC0DD\uC131\uD55C \uC544\uB808\uB098 \uCE74\uB4DC\uB9CC \uACF5\uAC1C\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4.");
    const repeats = await db.select().from(minuteResearchCandidates).where((0, import_drizzle_orm25.and)((0, import_drizzle_orm25.eq)(minuteResearchCandidates.strategyFingerprint, candidate.strategyFingerprint), (0, import_drizzle_orm25.eq)(minuteResearchCandidates.status, "promoted"))).orderBy((0, import_drizzle_orm25.desc)(minuteResearchCandidates.createdAt)).limit(100);
    ...
    const original = (await db.select().from(publicStrategyCards).where((0, import_drizzle_orm25.and)((0, import_drizzle_orm25.eq)(publicStrategyCards.sourceCandidateId, candidate.id), (0, import_drizzle_orm25.isNull)(publicStrategyCards.parentCardId))).limit(1))[0];
    if (original) await db.update(publicStrategyCards).set({ title: values.title, rootGenomeJson: values.rootGenomeJson, minimumScore: values.minimumScore, datasetFingerprint: values.datasetFingerprint, arenaEvidenceJson: values.arenaEvidenceJson, validationEvidenceJson: values.validationEvidenceJson, visibility: "public" }).where((0, import_drizzle_orm25.eq)(publicStrategyCards.id, original.id));
    else await db.insert(publicStrategyCards).values(values);
    const card = original ?? (await db.select().from(publicStrategyCards).where((0, import_drizzle_orm25.and)((0, import_drizzle_orm25.eq)(publicStrategyCards.sourceCandidateId, candidate.id), (0, import_drizzle_orm25.isNull)(publicStrategyCards.parentCardId))).limit(1))[0];
    return { cardId: card.id, title: card.title, strategyFingerprint: card.strategyFingerprint };
  }),
  fork: protectedProcedure.input(import_zod14.z.object({ cardId: import_zod14.z.number().int().positive(), title: import_zod14.z.string().trim().min(2).max(120).optional() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb10();
    const parent = (await db.select().from(publicStrategyCards).where((0, import_drizzle_orm25.and)((0, import_drizzle_orm25.eq)(publicStrategyCards.id, input.cardId), (0, import_drizzle_orm25.eq)(publicStrategyCards.visibility, "public"))).limit(1))[0];
    if (!parent) throw new Error("\uACF5\uAC1C\uB41C \uC804\uB7B5 \uCE74\uB4DC\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.");
    const latestChild = (await db.select().from(publicStrategyCards).where((0, import_drizzle_orm25.eq)(publicStrategyCards.parentCardId, parent.id)).orderBy((0, import_drizzle_orm25.desc)(publicStrategyCards.version)).limit(1))[0];
  ...
  collect: protectedProcedure.input(import_zod14.z.object({ cardId: import_zod14.z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb10();
    const card = (await db.select().from(publicStrategyCards).where((0, import_drizzle_orm25.and)((0, import_drizzle_orm25.eq)(publicStrategyCards.id, input.cardId), (0, import_drizzle_orm25.eq)(publicStrategyCards.visibility, "public"))).limit(1))[0];
    if (!card) throw new Error("\uACF5\uAC1C\uB41C \uC804\uB7B5 \uCE74\uB4DC\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.");
    const existing = (await db.select().from(publicStrategyCardCollections).where((0, import_drizzle_orm25.and)((0, import_drizzle_orm25.eq)(publicStrategyCardCollections.cardId, card.id), (0, import_drizzle_orm25.eq)(publicStrategyCardCollections.userId, ctx.user.id))).limit(1))[0];
...
var profileRouter = router({
  me: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [user] = await db.select({ name: users.name, email: users.email, avatarId: users.avatarId }).from(users).where((0, import_drizzle_orm26.eq)(users.id, ctx.user.id)).limit(1);
    return user ?? { name: ctx.user.name, email: ctx.user.email, avatarId: "nebula" };
  }),
  updateAvatar: protectedProcedure.input(import_zod15.z.object({ avatarId: import_zod15.z.enum(AVATAR_IDS) })).mutation(async ({ ctx, input }) => {
    const db = await requireDb11();
    await db.update(users).set({ avatarId: input.avatarId }).where((0, import_drizzle_orm26.eq)(users.id, ctx.user.id));
...
var symbolSchema = import_zod16.z.object({ symbol: import_zod16.z.string().regex(/^\d{6}$/), name: import_zod16.z.string().min(1).max(120) });
var rulesSchema = import_zod16.z.array(import_zod16.z.object({ id: import_zod16.z.string(), type: import_zod16.z.string(), enabled: import_zod16.z.boolean(), weight: import_zod16.z.number(), config: import_zod16.z.record(import_zod16.z.string(), import_zod16.z.union([import_zod16.z.string(), import_zod16.z.number(), import_zod16.z.boolean()])) }));
var collectionSchema = import_zod16.z.object({
  symbolCount: import_zod16.z.number().int().min(4).max(20).default(10),
  sampleDays: import_zod16.z.number().int().min(5).max(60).default(15),
  randomSeed: import_zod16.z.number().int().min(1).max(2147483647).optional()
});
var PUBLIC_DATASET_PAGE_SIZE = 6;
async function requireDb12() {
  const db = await getDb();
  if (!db) throw new import_server15.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uACF5\uC6A9 \uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  return db;
}
async function requireConnectedKiwoomTerminal(db, userId) {
  const latest = (await db.select().from(kiwoomTerminalConnectionChecks).where((0, import_drizzle_orm27.eq)(kiwoomTerminalConnectionChecks.userId, userId)).orderBy((0, import_drizzle_orm27.desc)(kiwoomTerminalConnectionChecks.checkedAt)).limit(1))[0];
  if (!latest || latest.status !== "connected") {
    throw new import_server15.TRPCError({ code: "PRECONDITION_FAILED", message: "\uACF5\uC6A9 \uB370\uC774\uD130 \uC218\uC9D1 \uC804 \uD604\uC7AC \uCEF4\uD4E8\uD130\uC5D0\uC11C check-kiwoom-rest-connection.cmd\uB97C \uC2E4\uD589\uD574 \uD0A4\uC6C0 REST \uB2E8\uB9D0 \uC778\uC99D\uC744 \uC644\uB8CC\uD558\uC138\uC694." });
  }
  return latest;
}
var sharedDatasetsRouter = router({
  listPublic: publicProcedure.input(import_zod16.z.object({ page: import_zod16.z.number().int().min(1).max(1e4).optional() }).optional()).query(async ({ input }) => {
    const db = await requireDb12();
    const page = input?.page ?? 1;
    const publicReady = (0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(researchDatasets.visibility, "shared_public"), (0, import_drizzle_orm27.eq)(researchDatasets.qualityStatus, "ready"));
    const [totalRow] = await db.select({ total: (0, import_drizzle_orm27.count)() }).from(researchDatasets).where(publicReady);
    ...
    }).from(researchDatasets).where(publicReady).orderBy((0, import_drizzle_orm27.desc)(researchDatasets.readyAt)).limit(PUBLIC_DATASET_PAGE_SIZE).offset((safePage - 1) * PUBLIC_DATASET_PAGE_SIZE);
  ...
  vaultSummary: publicProcedure.query(async () => {
    const db = await requireDb12();
    const ready = (0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(researchDatasets.visibility, "shared_public"), (0, import_drizzle_orm27.eq)(researchDatasets.qualityStatus, "ready"));
    const [summary] = await db.select({
      datasetCount: (0, import_drizzle_orm27.count)(),
      dailyBarCount: import_drizzle_orm27.sql`coalesce(sum(${researchDatasets.barCount}), 0)`,
      fiveMinuteBarCount: import_drizzle_orm27.sql`coalesce(sum(${researchDatasets.minuteBarCount}), 0)`,
      oldestReadyAt: import_drizzle_orm27.sql`min(${researchDatasets.readyAt})`,
      newestReadyAt: import_drizzle_orm27.sql`max(${researchDatasets.readyAt})`
  ...
  collectRandomShared: protectedProcedure.input(collectionSchema).mutation(async ({ ctx, input }) => {
    ...
    const activeRequest = (await db.select().from(sharedDatasetCollectionRequests).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(sharedDatasetCollectionRequests.requestedByUserId, ctx.user.id), (0, import_drizzle_orm27.inArray)(sharedDatasetCollectionRequests.status, ["queued", "running"]))).orderBy((0, import_drizzle_orm27.desc)(sharedDatasetCollectionRequests.requestedAt)).limit(1))[0];
    ...
    const seed = input.randomSeed ?? (0, import_node_crypto8.randomInt)(1, 2147483647);
    const requestFingerprint = `shared-local:${ctx.user.id}:${seed}:${input.symbolCount}:${input.sampleDays}`;
    const existing = (await db.select().from(sharedDatasetCollectionRequests).where((0, import_drizzle_orm27.eq)(sharedDatasetCollectionRequests.requestFingerprint, requestFingerprint)).limit(1))[0];
  ...
  resumeCollection: protectedProcedure.input(import_zod16.z.object({ requestId: import_zod16.z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    const db = await requireDb12();
    await requireConnectedKiwoomTerminal(db, ctx.user.id);
    const request = (await db.select().from(sharedDatasetCollectionRequests).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(sharedDatasetCollectionRequests.id, input.requestId), (0, import_drizzle_orm27.eq)(sharedDatasetCollectionRequests.requestedByUserId, ctx.user.id))).limit(1))[0];
    if (!request) throw new import_server15.TRPCError({ code: "NOT_FOUND", message: "\uB0B4 \uC218\uC9D1 \uC694\uCCAD\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    if (!["failed", "cancelled"].includes(request.status)) throw new import_server15.TRPCError({ code: "BAD_REQUEST", message: "\uC911\uB2E8\uB418\uC5C8\uAC70\uB098 \uC2E4\uD328\uD55C \uC218\uC9D1 \uC694\uCCAD\uB9CC \uB2E4\uC2DC \uC774\uC5B4\uAC08 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const active = (await db.select().from(sharedDatasetCollectionRequests).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(sharedDatasetCollectionRequests.requestedByUserId, ctx.user.id), (0, import_drizzle_orm27.inArray)(sharedDatasetCollectionRequests.status, ["queued", "running"]))).orderBy((0, import_drizzle_orm27.desc)(sharedDatasetCollectionRequests.requestedAt)).limit(1))[0];
    if (active) return { status: active.status, requestId: active.id, reusedActiveRequest: true };
    await db.update(sharedDatasetCollectionRequests).set({ status: "queued", startedAt: null, lastError: null, completedAt: null, progressJson: { stage: "resume_queued", message: "\uC774\uC804 \uC218\uC9D1 \uC9C0\uC810\uBD80\uD130 \uB2E4\uC2DC \uC774\uC5B4\uAC08 \uC900\uBE44\uAC00 \uB418\uC5C8\uC2B5\uB2C8\uB2E4.", updatedAt: (/* @__PURE__ */ new Date()).toISOString() }, resumeCount: import_drizzle_orm27.sql`${sharedDatasetCollectionRequests.resumeCount} + 1` }).where((0, import_drizzle_orm27.eq)(sharedDatasetCollectionRequests.id, request.id));
  ...
  listMyCollectionRequests: protectedProcedure.query(async ({ ctx }) => {
    const db = await requireDb12();
    return db.select().from(sharedDatasetCollectionRequests).where((0, import_drizzle_orm27.eq)(sharedDatasetCollectionRequests.requestedByUserId, ctx.user.id)).orderBy((0, import_drizzle_orm27.desc)(sharedDatasetCollectionRequests.requestedAt)).limit(50);
  }),
  runBacktest: protectedProcedure.input(import_zod16.z.object({ datasetId: import_zod16.z.number().int().positive(), presetId: import_zod16.z.number().int().positive(), timeframe: import_zod16.z.enum(["daily", "five_minute"]), symbol: import_zod16.z.string().regex(/^\d{6}$/), minScore: import_zod16.z.number().min(0).max(100).default(70), holdingBars: import_zod16.z.number().int().min(1).max(120).default(5), feeRate: import_zod16.z.number().min(0).max(0.1).default(3e-4), slippageBps: import_zod16.z.number().min(0).max(1e4).default(8) })).mutation(async ({ ctx, input }) => {
    const db = await requireDb12();
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(researchDatasets.id, input.datasetId), (0, import_drizzle_orm27.eq)(researchDatasets.visibility, "shared_public"), (0, import_drizzle_orm27.eq)(researchDatasets.qualityStatus, "ready"))).limit(1))[0];
    if (!dataset) throw new import_server15.TRPCError({ code: "NOT_FOUND", message: "\uACF5\uC6A9\uC73C\uB85C \uC900\uBE44\uB41C \uB370\uC774\uD130\uC14B\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const preset = (await db.select().from(strategyPresets).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(strategyPresets.id, input.presetId), (0, import_drizzle_orm27.eq)(strategyPresets.userId, ctx.user.id))).limit(1))[0];
    if (!preset) throw new import_server15.TRPCError({ code: "NOT_FOUND", message: "\uB0B4 \uC870\uAC74\uC2DD \uCE74\uB4DC\uB9CC \uACF5\uC6A9 \uB370\uC774\uD130\uC14B\uC5D0\uC11C \uBC31\uD14C\uC2A4\uD2B8\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const rows = input.timeframe === "daily" ? await db.select().from(researchDailyBars).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(researchDailyBars.datasetId, dataset.id), (0, import_drizzle_orm27.eq)(researchDailyBars.symbol, input.symbol))).orderBy((0, import_drizzle_orm27.asc)(researchDailyBars.date)) : await db.select().from(researchFiveMinuteBars).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(researchFiveMinuteBars.datasetId, dataset.id), (0, import_drizzle_orm27.eq)(researchFiveMinuteBars.symbol, input.symbol))).orderBy((0, import_drizzle_orm27.asc)(researchFiveMinuteBars.intervalAt));
    const dailyContextRows = input.timeframe === "five_minute" ? await db.select().from(researchDailyBars).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(researchDailyBars.datasetId, dataset.id), (0, import_drizzle_orm27.eq)(researchDailyBars.symbol, input.symbol))).orderBy((0, import_drizzle_orm27.asc)(researchDailyBars.date)) : [];
    ...
    if (bars.length < 60) throw new import_server15.TRPCError({ code: "PRECONDITION_FAILED", message: `${input.timeframe === "daily" ? "\uC77C\uBD09" : "5\uBD84\uBD09"} \uBC31\uD14C\uC2A4\uD2B8\uC5D0\uB294 \uCD5C\uC18C 60\uAC1C\uC758 \uACE0\uC815 \uC6D0\uBCF8 \uBD09\uC774 \uD544\uC694\uD569\uB2C8\uB2E4.` });
  ...
  runMultiDatasetBacktest: protectedProcedure.input(import_zod16.z.object({ datasetIds: import_zod16.z.array(import_zod16.z.number().int().positive()).min(2).max(12), presetId: import_zod16.z.number().int().positive(), timeframe: import_zod16.z.enum(["daily", "five_minute"]), minScore: import_zod16.z.number().min(0).max(100).default(70), holdingBars: import_zod16.z.number().int().min(1).max(120).default(5), feeRate: import_zod16.z.number().min(0).max(0.1).default(3e-4), slippageBps: import_zod16.z.number().min(0).max(1e4).default(8) })).mutation(async ({ ctx, input }) => {
    const ids = Array.from(new Set(input.datasetIds));
    if (ids.length !== input.datasetIds.length) throw new import_server15.TRPCError({ code: "BAD_REQUEST", message: "\uBE44\uAD50\uD560 \uB370\uC774\uD130\uC14B\uC740 \uC911\uBCF5 \uC5C6\uC774 \uC120\uD0DD\uD558\uC138\uC694." });
    const db = await requireDb12();
    const preset = (await db.select().from(strategyPresets).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(strategyPresets.id, input.presetId), (0, import_drizzle_orm27.eq)(strategyPresets.userId, ctx.user.id))).limit(1))[0];
    if (!preset) throw new import_server15.TRPCError({ code: "NOT_FOUND", message: "\uB0B4 \uC870\uAC74\uC2DD \uCE74\uB4DC\uB9CC \uC5EC\uB7EC \uACF5\uC6A9 \uB370\uC774\uD130\uC14B\uC5D0\uC11C \uBE44\uAD50\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." });
    const datasets = await db.select().from(researchDatasets).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.inArray)(researchDatasets.id, ids), (0, import_drizzle_orm27.eq)(researchDatasets.visibility, "shared_public"), (0, import_drizzle_orm27.eq)(researchDatasets.qualityStatus, "ready")));
    if (datasets.length !== ids.length) throw new import_server15.TRPCError({ code: "NOT_FOUND", message: "\uC120\uD0DD\uD55C \uACF5\uC6A9 \uB370\uC774\uD130\uC14B \uC911 \uC900\uBE44\uB418\uC9C0 \uC54A\uC558\uAC70\uB098 \uC811\uADFC\uD560 \uC218 \uC5C6\uB294 \uD56D\uBAA9\uC774 \uC788\uC2B5\uB2C8\uB2E4." });
    ...
    for (const dataset of datasets.sort((left, right) => ids.indexOf(left.id) - ids.indexOf(right.id))) {
      ...
      for (const symbol of universe) {
        const rows = input.timeframe === "daily" ? await db.select().from(researchDailyBars).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(researchDailyBars.datasetId, dataset.id), (0, import_drizzle_orm27.eq)(researchDailyBars.symbol, symbol))).orderBy((0, import_drizzle_orm27.asc)(researchDailyBars.date)) : await db.select().from(researchFiveMinuteBars).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(researchFiveMinuteBars.datasetId, dataset.id), (0, import_drizzle_orm27.eq)(researchFiveMinuteBars.symbol, symbol))).orderBy((0, import_drizzle_orm27.asc)(researchFiveMinuteBars.intervalAt));
        const dailyContextRows = input.timeframe === "five_minute" ? await db.select().from(researchDailyBars).where((0, import_drizzle_orm27.and)((0, import_drizzle_orm27.eq)(researchDailyBars.datasetId, dataset.id), (0, import_drizzle_orm27.eq)(researchDailyBars.symbol, symbol))).orderBy((0, import_drizzle_orm27.asc)(researchDailyBars.date)) : [];
        const bars = input.timeframe === "daily" ? rows.map((row) => ({ date: row.date, open: row.open, high: row.high, low: row.low, close: row.close, volume: Number(row.volume), turnover: Number(row.turnover) })) : rows.map((row) => ({ date: row.intervalAt.toISOString(), open: row.open, high: row.high, low: row.low, close: row.close, volume: Number(row.volume), turnover: Number(row.close) * Number(row.volume) }));
        if (bars.length < 60) {
  ...
  listMyBacktests: protectedProcedure.query(async ({ ctx }) => {
    const db = await requireDb12();
    return db.select().from(sharedDatasetBacktests).where((0, import_drizzle_orm27.eq)(sharedDatasetBacktests.userId, ctx.user.id)).orderBy((0, import_drizzle_orm27.desc)(sharedDatasetBacktests.createdAt)).limit(30);
  })
});
...
async function requireDb13() {
  const db = await getDb();
  if (!db) throw new import_server16.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "\uC0DD\uC874 \uCE74\uB4DC \uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
  return db;
}
...
var survivalResearchRouter = router({
  list: protectedProcedure.query(async ({ ctx }) => {
    ...
    const ledgers = await db.select().from(strategySurvivalLedgers).where((0, import_drizzle_orm28.eq)(strategySurvivalLedgers.userId, ctx.user.id)).orderBy((0, import_drizzle_orm28.desc)(strategySurvivalLedgers.createdAt)).limit(30);
    if (!ledgers.length) return [];
    const presetIds = Array.from(new Set(ledgers.map((item) => item.presetId)));
    const presets = await db.select({ id: strategyPresets.id, name: strategyPresets.name, rulesJson: strategyPresets.rulesJson }).from(strategyPresets).where((0, import_drizzle_orm28.and)((0, import_drizzle_orm28.eq)(strategyPresets.userId, ctx.user.id), (0, import_drizzle_orm28.inArray)(strategyPresets.id, presetIds)));
    return ledgers.map((ledger) => ({ ...ledger, preset: presets.find((preset) => preset.id === ledger.presetId) ?? null }));
  }),
  evaluate: protectedProcedure.input(import_zod17.z.object({ presetIds: import_zod17.z.array(import_zod17.z.number().int().positive()).min(1).max(12) })).mutation(async ({ ctx, input }) => {
    const db = await requireDb13();
    const presets = await db.select().from(strategyPresets).where((0, import_drizzle_orm28.and)((0, import_drizzle_orm28.eq)(strategyPresets.userId, ctx.user.id), (0, import_drizzle_orm28.inArray)(strategyPresets.id, input.presetIds)));
    if (!presets.length) throw new import_server16.TRPCError({ code: "NOT_FOUND", message: "\uD3C9\uAC00\uD560 \uB0B4 \uC870\uAC74\uC2DD \uCE74\uB4DC\uB97C \uCC3E\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4." });
    const records = await db.select().from(sharedDatasetBacktests).where((0, import_drizzle_orm28.and)((0, import_drizzle_orm28.eq)(sharedDatasetBacktests.userId, ctx.user.id), (0, import_drizzle_orm28.eq)(sharedDatasetBacktests.timeframe, "five_minute"))).orderBy((0, import_drizzle_orm28.desc)(sharedDatasetBacktests.createdAt));
...
var chartDataRouter = router({
   ...
   * - days: 최근 N거래??(기본 5)
   */
  minuteBars: publicProcedure.input(import_zod18.z.object({
    symbol: import_zod18.z.string().regex(/^\d{6}$/),
    tradingDate: import_zod18.z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
    days: import_zod18.z.number().int().min(1).max(60).default(5)
  })).query(async ({ input }) => {
    const db = await getDb();
    if (!db) throw new import_server17.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB ?\uFFFD\uACB0 \uBD88\uFFFD?" });
    let query;
    if (input.tradingDate) {
      ...
      }).from(intradayMinuteBars).where((0, import_drizzle_orm29.and)(
        (0, import_drizzle_orm29.eq)(intradayMinuteBars.symbol, input.symbol),
        (0, import_drizzle_orm29.eq)(intradayMinuteBars.tradingDate, input.tradingDate)
      )).orderBy((0, import_drizzle_orm29.asc)(intradayMinuteBars.minuteAt));
    } else {
      const latestDates = await db.selectDistinct({ tradingDate: intradayMinuteBars.tradingDate }).from(intradayMinuteBars).where((0, import_drizzle_orm29.eq)(intradayMinuteBars.symbol, input.symbol)).orderBy((0, import_drizzle_orm29.desc)(intradayMinuteBars.tradingDate)).limit(input.days);
      ...
      query = db.select({
        ...
        (0, import_drizzle_orm29.eq)(intradayMinuteBars.symbol, input.symbol),
        (0, import_drizzle_orm29.inArray)(intradayMinuteBars.tradingDate, dates)
      )).orderBy((0, import_drizzle_orm29.asc)(intradayMinuteBars.minuteAt));
    }
   ...
   * 종목???�봉 ?�이??조회
   */
  dailyBars: publicProcedure.input(import_zod18.z.object({
    symbol: import_zod18.z.string().regex(/^\d{6}$/),
    startDate: import_zod18.z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
    endDate: import_zod18.z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
    limit: import_zod18.z.number().int().min(1).max(1e3).default(600)
  })).query(async ({ input }) => {
    const db = await getDb();
    if (!db) throw new import_server17.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB ?\uFFFD\uACB0 \uBD88\uFFFD?" });
    const conditions = [
      (0, import_drizzle_orm29.eq)(localResearchDailyBars.symbol, input.symbol),
      import_drizzle_orm29.sql`${localResearchDailyBars.adjustmentBasis}::text = 'adjusted'`
    ];
    if (input.startDate) conditions.push((0, import_drizzle_orm29.gte)(localResearchDailyBars.date, input.startDate));
    if (input.endDate) conditions.push((0, import_drizzle_orm29.lte)(localResearchDailyBars.date, input.endDate));
    ...
    }).from(localResearchDailyBars).where((0, import_drizzle_orm29.and)(...conditions)).orderBy((0, import_drizzle_orm29.asc)(localResearchDailyBars.date)).limit(input.limit);
    const bars = rows.map((row) => ({
  ...
  availableSymbols: publicProcedure.query(async () => {
    const db = await getDb();
    if (!db) throw new import_server17.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB ?\uFFFD\uACB0 \uBD88\uFFFD?" });
    const daily = await db.selectDistinct({ symbol: localResearchDailyBars.symbol }).from(localResearchDailyBars).where(import_drizzle_orm29.sql`${localResearchDailyBars.adjustmentBasis}::text = 'adjusted'`).limit(100);
...
var oneClickBacktestRouter = router({
   ...
   */
  run: publicProcedure.input(import_zod19.z.object({
    /** 생성할 조건식 수 (기본 10) */
    count: import_zod19.z.number().int().min(1).max(50).default(10),
    /** 규칙 수 범위 */
    minRules: import_zod19.z.number().int().min(2).max(10).default(3),
    maxRules: import_zod19.z.number().int().min(3).max(12).default(6),
    /** 보유 기간 (일) */
    holdingDays: import_zod19.z.number().int().min(1).max(60).default(5),
    /** 수수료율 */
    feeRate: import_zod19.z.number().min(0).max(0.01).default(3e-4),
    /** 슬리피지 */
    slippageBps: import_zod19.z.number().min(0).max(100).default(8),
    /** 최소 점수 */
    minScore: import_zod19.z.number().min(0).max(100).default(50),
    /** 타임프레임 */
    timeframe: import_zod19.z.enum(["daily", "minute"]).default("daily")
    ...
    const db = await getDb();
    if (!db) throw new import_server18.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB \uC5F0\uACB0 \uBD88\uAC00" });
    const allSymbols = await db.selectDistinct({ symbol: localResearchDailyBars.symbol }).from(localResearchDailyBars).where((0, import_drizzle_orm30.eq)(localResearchDailyBars.adjustmentBasis, "adjusted")).limit(100);
    ...
    for (const { symbol } of selectedSymbols) {
      const rows = await db.select({
        ...
        volume: localResearchDailyBars.volume,
        turnover: localResearchDailyBars.turnover
      }).from(localResearchDailyBars).where((0, import_drizzle_orm30.and)(
        (0, import_drizzle_orm30.eq)(localResearchDailyBars.symbol, symbol),
        (0, import_drizzle_orm30.eq)(localResearchDailyBars.adjustmentBasis, "adjusted")
      )).orderBy((0, import_drizzle_orm30.asc)(localResearchDailyBars.date)).limit(600);
      if (rows.length >= 60) {
   ...
   */
  adopt: protectedProcedure.input(import_zod19.z.object({
    name: import_zod19.z.string().min(1).max(100),
    root: import_zod19.z.any(),
    // EvolutionGroup JSON
    minimumScore: import_zod19.z.number().int().min(1).max(100),
    fingerprint: import_zod19.z.string().min(1),
    backtestSummary: import_zod19.z.object({
      averageReturn: import_zod19.z.number(),
      averageWinRate: import_zod19.z.number(),
      totalTrades: import_zod19.z.number(),
      worstDrawdown: import_zod19.z.number(),
      fitnessScore: import_zod19.z.number()
    })
  })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new import_server18.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB \uC5F0\uACB0 \uBD88\uAC00" });
   ...
   */
  evolve: publicProcedure.input(import_zod19.z.object({
    /** 부모 조건식의 root genome */
    parentRoot: import_zod19.z.any(),
    parentMinimumScore: import_zod19.z.number().int().min(1).max(100),
    /** 변형 수 */
    mutationCount: import_zod19.z.number().int().min(1).max(20).default(8),
    /** 백테스트 설정 */
    holdingDays: import_zod19.z.number().int().min(1).max(60).default(5),
    feeRate: import_zod19.z.number().min(0).max(0.01).default(3e-4),
    slippageBps: import_zod19.z.number().min(0).max(100).default(8),
    ...
    for (const { symbol } of shuffled) {
      const rows = await db.select({
        ...
        volume: localResearchDailyBars.volume,
        turnover: localResearchDailyBars.turnover
      }).from(localResearchDailyBars).where((0, import_drizzle_orm30.and)((0, import_drizzle_orm30.eq)(localResearchDailyBars.symbol, symbol), (0, import_drizzle_orm30.eq)(localResearchDailyBars.adjustmentBasis, "adjusted"))).orderBy((0, import_drizzle_orm30.asc)(localResearchDailyBars.date)).limit(600);
      if (rows.length >= 60) {
    ...
    const eligibleSymbols = Object.keys(barsBySymbol);
    if (!eligibleSymbols.length) {
      throw new import_server18.TRPCError({ code: "PRECONDITION_FAILED", message: "\uBC31\uD14C\uC2A4\uD2B8\uD560 \uB370\uC774\uD130\uAC00 \uBD80\uC871\uD569\uB2C8\uB2E4." });
    }
  ...
  adopted: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) throw new import_server18.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB \uC5F0\uACB0 \uBD88\uAC00" });
    const presets = await db.select().from(strategyPresets).where((0, import_drizzle_orm30.eq)(strategyPresets.userId, ctx.user.id)).orderBy((0, import_drizzle_orm30.desc)(strategyPresets.createdAt)).limit(50);
...
var mockTradingRouter = router({
  ...
  positions: publicProcedure.query(async () => {
    const db = await getDb();
    if (!db) throw new import_server19.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB \uC5F0\uACB0 \uBD88\uAC00" });
    const snapshots = await db.select().from(positionSnapshots).orderBy((0, import_drizzle_orm31.desc)(positionSnapshots.capturedAt)).limit(100);
    const bySymbol = /* @__PURE__ */ new Map();
   ...
   * 최근 주문 내역
   */
  recentOrders: publicProcedure.input(import_zod20.z.object({ limit: import_zod20.z.number().int().min(1).max(100).default(30) }).optional()).query(async ({ input }) => {
    const db = await getDb();
    if (!db) throw new import_server19.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB \uC5F0\uACB0 \uBD88\uAC00" });
    ...
    }).from(orderIntents).where((0, import_drizzle_orm31.eq)(orderIntents.executionOrigin, "local_node")).orderBy((0, import_drizzle_orm31.desc)(orderIntents.createdAt)).limit(limit);
    return { orders };
  }),
  /**
   * 주문별 체결 상세
   */
  executions: publicProcedure.input(import_zod20.z.object({ orderIntentId: import_zod20.z.number().int().positive() })).query(async ({ input }) => {
    const db = await getDb();
    if (!db) throw new import_server19.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB \uC5F0\uACB0 \uBD88\uAC00" });
    const executions = await db.select().from(orderExecutions).where((0, import_drizzle_orm31.eq)(orderExecutions.orderIntentId, input.orderIntentId)).orderBy((0, import_drizzle_orm31.desc)(orderExecutions.executedAt));
    return { executions };
  }),
  /**
   * 현재 활성 자동매매 정책
  ...
  activePolicy: publicProcedure.query(async () => {
    const db = await getDb();
    if (!db) throw new import_server19.TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB \uC5F0\uACB0 \uBD88\uAC00" });
    const [policy] = await db.select().from(autoTradePolicies).where((0, import_drizzle_orm31.eq)(autoTradePolicies.status, "active")).orderBy((0, import_drizzle_orm31.desc)(autoTradePolicies.createdAt)).limit(1);
  ...
  todayPnl: publicProcedure.query(async () => {
    ...
    const todayOrders = await db.select({
      ...
      status: orderIntents.status,
      symbol: orderIntents.symbol
    }).from(orderIntents).where((0, import_drizzle_orm31.and)(
      (0, import_drizzle_orm31.eq)(orderIntents.executionOrigin, "local_node"),
      (0, import_drizzle_orm31.eq)(orderIntents.status, "filled"),
      (0, import_drizzle_orm31.gte)(orderIntents.createdAt, todayStart)
    ));
...
var conditionBuilderRouter = router({
  ...
  save: protectedProcedure.input(
    import_zod21.z.object({
      name: import_zod21.z.string().min(1).max(120),
      description: import_zod21.z.string().max(500).optional(),
      expressionJson: import_zod21.z.unknown()
    })
  ).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db)
  ...
  list: protectedProcedure.query(async ({ ctx }) => {
    ...
    const presets = await db.select().from(strategyPresets).where((0, import_drizzle_orm32.eq)(strategyPresets.userId, ctx.user.id)).orderBy((0, import_drizzle_orm32.desc)(strategyPresets.createdAt)).limit(50);
    return presets.map((p) => ({
      id: p.id,
      name: p.name,
      rulesJson: p.rulesJson,
   ...
   * 단일 프리셋 불러오기
   */
  load: protectedProcedure.input(import_zod21.z.object({ id: import_zod21.z.number() })).query(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db)
      throw new import_server20.TRPCError({
        code: "INTERNAL_SERVER_ERROR",
    ...
    const [preset] = await db.select().from(strategyPresets).where(
      (0, import_drizzle_orm32.and)(
        (0, import_drizzle_orm32.eq)(strategyPresets.id, input.id),
        (0, import_drizzle_orm32.eq)(strategyPresets.userId, ctx.user.id)
      )
    ).limit(1);
    if (!preset) {
      throw new import_server20.TRPCError({
   ...
   * 조건식 삭제
   */
  delete: protectedProcedure.input(import_zod21.z.object({ id: import_zod21.z.number() })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db)
      throw new import_server20.TRPCError({
        code: "INTERNAL_SERVER_ERROR",
   ...
   * 조건식 복제 (" (복사)" 접미사 추가)
   */
  duplicate: protectedProcedure.input(import_zod21.z.object({ id: import_zod21.z.number() })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db)
      throw new import_server20.TRPCError({
        code: "INTERNAL_SERVER_ERROR",
   ...
   * 중복 이름 체크
   */
  checkNameExists: protectedProcedure.input(import_zod21.z.object({ name: import_zod21.z.string() })).query(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db)
      throw new import_server20.TRPCError({
        code: "INTERNAL_SERVER_ERROR",
    ...
    const [existing] = await db.select({ id: strategyPresets.id }).from(strategyPresets).where(
      (0, import_drizzle_orm32.and)(
        (0, import_drizzle_orm32.eq)(strategyPresets.userId, ctx.user.id),
        (0, import_drizzle_orm32.eq)(strategyPresets.name, input.name)
      )
    ).limit(1);
  ...
  runBacktest: protectedProcedure.input(
    import_zod21.z.object({
      expressionJson: import_zod21.z.unknown(),
      holdingDays: import_zod21.z.number().int().min(1).max(60).default(5),
      feeRate: import_zod21.z.number().min(0).max(0.01).default(3e-4),
      slippageBps: import_zod21.z.number().min(0).max(100).default(8),
      minScore: import_zod21.z.number().min(0).max(200).default(0)
    })
  ).mutation(async ({ input }) => {
    ...
    const allSymbols = await db.selectDistinct({ symbol: localResearchDailyBars.symbol }).from(localResearchDailyBars).where((0, import_drizzle_orm32.eq)(localResearchDailyBars.adjustmentBasis, "adjusted")).limit(100);
    if (!allSymbols.length) {
      throw new import_server20.TRPCError({
        code: "PRECONDITION_FAILED",
        message: "\uBC31\uD14C\uC2A4\uD2B8\uD560 \uC77C\uBD09 \uB370\uC774\uD130\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4. \uB85C\uCEEC \uC218\uC9D1\uAE30\uB85C \uB370\uC774\uD130\uB97C \uBA3C\uC800 \uC218\uC9D1\uD558\uC138\uC694."
    ...
    for (const { symbol } of selectedSymbols) {
      const rows = await db.select({
        ...
        (0, import_drizzle_orm32.and)(
          (0, import_drizzle_orm32.eq)(localResearchDailyBars.symbol, symbol),
          (0, import_drizzle_orm32.eq)(localResearchDailyBars.adjustmentBasis, "adjusted")
        )
      ).orderBy((0, import_drizzle_orm32.asc)(localResearchDailyBars.date)).limit(600);
      if (rows.length >= 60) {
        barsBySymbol[symbol] = rows.map((r) => ({
          date: r.date,
          open: r.open,
...
async function rankingRefreshHandler(req, res) {
  try {
    ...
    const profile = (await db.select().from(rankingRefreshProfiles).where((0, import_drizzle_orm33.eq)(rankingRefreshProfiles.scheduleCronTaskUid, user.taskUid)).limit(1))[0];
    ...
    await db.update(rankingRefreshProfiles).set({ status: "running", lastRunKey: runKey, lastRunAt: /* @__PURE__ */ new Date(), lastError: null }).where((0, import_drizzle_orm33.eq)(rankingRefreshProfiles.id, profile.id));
    try {
      ...
      await db.update(rankingRefreshProfiles).set({ status: "ready", lastCompletedAt: /* @__PURE__ */ new Date(), lastError: result.failedSymbols.length ? `${result.failedSymbols.length}\uAC1C \uC885\uBAA9 \uC218\uC9D1 \uC2E4\uD328` : null }).where((0, import_drizzle_orm33.eq)(rankingRefreshProfiles.id, profile.id));
      ...
      await db.update(rankingRefreshProfiles).set({ status: "error", lastError: message.slice(0, 500) }).where((0, import_drizzle_orm33.eq)(rankingRefreshProfiles.id, profile.id));
      throw error;
    }
  } catch (error) {
...
async function getOrCreateDailyRun(db, tradingDate2) {
  ...
  const existing = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm34.eq)(autonomousResearchRuns.runKey, runKey)).limit(1))[0];
  ...
  const created = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm34.eq)(autonomousResearchRuns.runKey, runKey)).limit(1))[0];
  if (!created) throw new Error("\uC790\uB3D9 \uB9AC\uC11C\uCE58 \uC77C\uC77C \uC2E4\uD589\uC744 \uC0DD\uC131\uD558\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4.");
  return created;
}
async function claimTask(db, input) {
  const existing = (await db.select().from(autonomousResearchTasks).where((0, import_drizzle_orm34.eq)(autonomousResearchTasks.runKey, input.runKey)).limit(1))[0];
  ...
  const task = (await db.select().from(autonomousResearchTasks).where((0, import_drizzle_orm34.eq)(autonomousResearchTasks.runKey, input.runKey)).limit(1))[0];
...
async function autonomousResearchHandler(req, res, options = {}) {
  try {
    ...
    const completeTask = async (input) => {
      await db.update(autonomousResearchTasks).set({ ...input, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm34.eq)(autonomousResearchTasks.id, claim.task.id));
    };
    if (!isExternalResearchVerificationEnabled()) {
      const transition = getWaitingForDataTransition(externalVerificationPausedMessage);
      await db.update(autonomousResearchRuns).set({ ...transition, updatedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm34.eq)(autonomousResearchRuns.id, dailyRun.id));
    ...
    try {
      ...
      if (phase === "opening") {
        ...
        const survivorRows = await db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm34.and)((0, import_drizzle_orm34.eq)(autonomousResearchCandidates.runId, dailyRun.id), (0, import_drizzle_orm34.eq)(autonomousResearchCandidates.status, "survived")));
        ...
        for (const candidate of survivorRows) {
          ...
          await db.update(autonomousResearchCandidates).set({ simulationJson: simulation }).where((0, import_drizzle_orm34.eq)(autonomousResearchCandidates.id, candidate.id));
      ...
      if (phase === "intraday" || phase === "closing") {
        const trackingCandidates = await db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm34.and)((0, import_drizzle_orm34.eq)(autonomousResearchCandidates.runId, dailyRun.id), (0, import_drizzle_orm34.eq)(autonomousResearchCandidates.status, "survived")));
        ...
        for (const candidate of trackingCandidates) {
          ...
          await db.update(autonomousResearchCandidates).set({ simulationJson: next }).where((0, import_drizzle_orm34.eq)(autonomousResearchCandidates.id, candidate.id));
        ...
        const updatedSurvivors = await db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm34.and)((0, import_drizzle_orm34.eq)(autonomousResearchCandidates.runId, dailyRun.id), (0, import_drizzle_orm34.eq)(autonomousResearchCandidates.status, "survived")));
      ...
      if (phase === "closing") {
        const [survivors, storedBars] = await Promise.all([
          db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm34.and)((0, import_drizzle_orm34.eq)(autonomousResearchCandidates.runId, dailyRun.id), (0, import_drizzle_orm34.eq)(autonomousResearchCandidates.status, "survived"))).orderBy((0, import_drizzle_orm34.desc)(autonomousResearchCandidates.fitnessScore)),
          db.select().from(autonomousResearchBars).where((0, import_drizzle_orm34.eq)(autonomousResearchBars.runId, dailyRun.id))
        ]);
        ...
        for (const candidate of survivors) {
          ...
          const maxDrawdown = folds.reduce((sum, item) => sum + item.worstFoldDrawdown, 0) / folds.length;
          const tradeCount = folds.reduce((sum, item) => sum + item.tradeCount, 0);
          await db.update(autonomousResearchCandidates).set({ walkForwardMetricsJson: { configuration: { trainingDays: 60, validationDays: 20, stepDays: 20 }, metrics: { totalReturn, maxDrawdown, tradeCount }, foldCount: folds.length } }).where((0, import_drizzle_orm34.eq)(autonomousResearchCandidates.id, candidate.id));
          walkForwardCount += 1;
        }
      ...
      await db.update(autonomousResearchRuns).set({ phase: phase === "closing" ? "completed" : phase, dataStatus: "ready", universeJson: universe.map((item) => ({ symbol: item.symbol, name: item.name })), summaryJson: summary, lastError: null, lastObservedAt: /* @__PURE__ */ new Date(), ...phase === "closing" ? { completedAt: /* @__PURE__ */ new Date() } : {} }).where((0, import_drizzle_orm34.eq)(autonomousResearchRuns.id, dailyRun.id));
...
async function researchGovernanceHandler(req, res) {
  try {
    ...
    const db = await getDb();
    if (!db) return res.status(500).json({ error: "database-unavailable", timestamp: (/* @__PURE__ */ new Date()).toISOString() });
    const schedule = (await db.select().from(researchGovernanceSchedules).where((0, import_drizzle_orm35.and)((0, import_drizzle_orm35.eq)(researchGovernanceSchedules.taskUid, user.taskUid), (0, import_drizzle_orm35.eq)(researchGovernanceSchedules.isEnabled, true))).limit(1))[0];
    if (!schedule) return res.json({ ok: true, skipped: "unknown-or-disabled-schedule" });
    await db.update(researchGovernanceSchedules).set({ lastRequestedAt: /* @__PURE__ */ new Date(), lastError: null }).where((0, import_drizzle_orm35.eq)(researchGovernanceSchedules.id, schedule.id));
    const latestActualRun = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm35.and)((0, import_drizzle_orm35.eq)(autonomousResearchRuns.dataStatus, "ready"), (0, import_drizzle_orm35.eq)(autonomousResearchRuns.phase, "completed"), (0, import_drizzle_orm35.like)(autonomousResearchRuns.runKey, "%:day"))).orderBy((0, import_drizzle_orm35.desc)(autonomousResearchRuns.updatedAt)).limit(1))[0];
    ...
    await db.update(researchGovernanceSchedules).set({ latestCycleId: result.cycle.id }).where((0, import_drizzle_orm35.eq)(researchGovernanceSchedules.id, schedule.id));
...
function minuteBarFingerprint(bar) {
  return (0, import_node_crypto9.createHash)("sha256").update(JSON.stringify({ symbol: bar.symbol, minuteAt: bar.minuteAt.toISOString(), open: bar.open, high: bar.high, low: bar.low, close: bar.close, volume: bar.volume, source: "kiwoom_ka10080" })).digest("hex");
}
...
function dailyBarFingerprint(input) {
  return (0, import_node_crypto9.createHash)("sha256").update(JSON.stringify({ symbol: input.symbol, adjustmentBasis: input.adjustmentBasis, ...input.bar, source: "kiwoom_ka10081" })).digest("hex");
}
...
function buildLocalDailyDatasetVersion(input) {
  ...
  const sourceFingerprint = (0, import_node_crypto9.createHash)("sha256").update(`${input.symbol}:${input.adjustmentBasis}:${material}`).digest("hex");
...
function buildLocalDailyUniverseDatasetVersion(input) {
  ...
  const sourceFingerprint = (0, import_node_crypto9.createHash)("sha256").update(`${input.symbols.slice().sort().join(",")}:${input.adjustmentBasis}:${material}`).digest("hex");
...
async function ensureLocalIntradayExperiment(db, tradingDate2) {
  await db.update(dayTradeExperiments).set({ status: "closed", closedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(dayTradeExperiments.status, "tracking"), (0, import_drizzle_orm36.ne)(dayTradeExperiments.tradingDate, tradingDate2)));
  const [closed] = await db.select().from(dayTradeExperiments).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(dayTradeExperiments.status, "closed"), (0, import_drizzle_orm36.eq)(dayTradeExperiments.tradingDate, tradingDate2))).orderBy((0, import_drizzle_orm36.desc)(dayTradeExperiments.updatedAt)).limit(1);
  if (closed) return closed;
  const [existing] = await db.select().from(dayTradeExperiments).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(dayTradeExperiments.status, "tracking"), (0, import_drizzle_orm36.eq)(dayTradeExperiments.tradingDate, tradingDate2))).orderBy((0, import_drizzle_orm36.desc)(dayTradeExperiments.updatedAt)).limit(1);
  const minuteRows = await db.select().from(intradayMinuteBars).where((0, import_drizzle_orm36.eq)(intradayMinuteBars.tradingDate, tradingDate2)).orderBy((0, import_drizzle_orm36.asc)(intradayMinuteBars.minuteAt));
  ...
  const sourceRun = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(autonomousResearchRuns.dataStatus, "ready"), (0, import_drizzle_orm36.like)(autonomousResearchRuns.runKey, "%:historical%"))).orderBy((0, import_drizzle_orm36.desc)(autonomousResearchRuns.updatedAt)).limit(1))[0];
  if (!sourceRun) return null;
  const sourceCandidates = await db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(autonomousResearchCandidates.runId, sourceRun.id), (0, import_drizzle_orm36.eq)(autonomousResearchCandidates.status, "survived"))).orderBy((0, import_drizzle_orm36.desc)(autonomousResearchCandidates.fitnessScore));
  ...
  const dailyRows = await db.select().from(autonomousResearchBars).where((0, import_drizzle_orm36.eq)(autonomousResearchBars.runId, sourceRun.id)).orderBy((0, import_drizzle_orm36.asc)(autonomousResearchBars.symbol), (0, import_drizzle_orm36.asc)(autonomousResearchBars.date));
  ...
  let run = existing ? (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm36.eq)(autonomousResearchRuns.id, existing.runId)).limit(1))[0] : (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm36.eq)(autonomousResearchRuns.runKey, runKey)).limit(1))[0];
  if (!run) {
    ...
    run = (await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm36.eq)(autonomousResearchRuns.runKey, runKey)).limit(1))[0];
  }
  if (!run) return null;
  ...
  const candidates = await db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(autonomousResearchCandidates.runId, run.id), (0, import_drizzle_orm36.eq)(autonomousResearchCandidates.status, "survived")));
  await persistDayTradeExperiment({ run, candidates, isClosing: false });
  return (await db.select().from(dayTradeExperiments).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(dayTradeExperiments.runId, run.id), (0, import_drizzle_orm36.eq)(dayTradeExperiments.status, "tracking"))).limit(1))[0] ?? null;
}
async function closeLocalIntradayExperimentAtMarketClose(db, input) {
  if (!shouldCloseIntradayExperiment(input)) return null;
  const [experiment] = await db.select().from(dayTradeExperiments).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(dayTradeExperiments.status, "tracking"), (0, import_drizzle_orm36.eq)(dayTradeExperiments.tradingDate, input.tradingDate))).orderBy((0, import_drizzle_orm36.desc)(dayTradeExperiments.updatedAt)).limit(1);
  if (!experiment) return null;
  const [run] = await db.select().from(autonomousResearchRuns).where((0, import_drizzle_orm36.eq)(autonomousResearchRuns.id, experiment.runId)).limit(1);
  if (!run) return null;
  const candidates = await db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(autonomousResearchCandidates.runId, run.id), (0, import_drizzle_orm36.eq)(autonomousResearchCandidates.status, "survived")));
  for (const candidate of candidates) {
    await db.update(autonomousResearchCandidates).set({ simulationJson: closeIntradayCandidateSimulation(candidate.simulationJson, input.capturedAt), updatedAt: input.capturedAt }).where((0, import_drizzle_orm36.eq)(autonomousResearchCandidates.id, candidate.id));
  }
  const closedCandidates = await db.select().from(autonomousResearchCandidates).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(autonomousResearchCandidates.runId, run.id), (0, import_drizzle_orm36.eq)(autonomousResearchCandidates.status, "survived")));
  await db.update(autonomousResearchRuns).set({ phase: "completed", lastObservedAt: input.capturedAt }).where((0, import_drizzle_orm36.eq)(autonomousResearchRuns.id, run.id));
  await persistDayTradeExperiment({ run, candidates: closedCandidates, isClosing: true });
  return (await db.select().from(dayTradeExperiments).where((0, import_drizzle_orm36.eq)(dayTradeExperiments.id, experiment.id)).limit(1))[0] ?? null;
}
function isLocalResearchNodeAuthorized(request) {
  ...
  return expectedBytes.length === suppliedBytes.length && (0, import_crypto.timingSafeEqual)(expectedBytes, suppliedBytes);
}
...
function registerLocalResearchNodeRoutes(app2) {
  ...
  app2.post("/api/local-research-node/kiwoom-terminal-connection", async (request, response) => {
    ...
    let owner = ownerOpenId ? (await db.select({ id: users.id }).from(users).where((0, import_drizzle_orm36.eq)(users.openId, ownerOpenId)).limit(1))[0] : null;
    if (!owner) {
      const adminCandidates = await db.select({ id: users.id }).from(users).where((0, import_drizzle_orm36.eq)(users.role, "admin")).limit(2);
  ...
  app2.get("/api/local-research-node/kiwoom-terminal-connection", async (request, response) => {
    ...
    const check = (await db.select({ terminalIp: kiwoomTerminalConnectionChecks.terminalIp, status: kiwoomTerminalConnectionChecks.status, errorCode: kiwoomTerminalConnectionChecks.errorCode, message: kiwoomTerminalConnectionChecks.message, verificationJson: kiwoomTerminalConnectionChecks.verificationJson, checkedAt: kiwoomTerminalConnectionChecks.checkedAt }).from(kiwoomTerminalConnectionChecks).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(kiwoomTerminalConnectionChecks.userId, owner.id), (0, import_drizzle_orm36.eq)(kiwoomTerminalConnectionChecks.terminalIp, terminalIp))).orderBy((0, import_drizzle_orm36.desc)(kiwoomTerminalConnectionChecks.checkedAt)).limit(1))[0] ?? null;
  ...
  app2.get("/api/local-research-node/intraday-price-plan", async (request, response) => {
    ...
    const db = await getDb();
    if (!db) return response.status(503).json({ status: "unavailable", message: "\uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const [experiment] = await db.select().from(dayTradeExperiments).where((0, import_drizzle_orm36.eq)(dayTradeExperiments.status, "tracking")).orderBy((0, import_drizzle_orm36.desc)(dayTradeExperiments.updatedAt)).limit(1);
    if (!experiment) return response.status(409).json({ status: "waiting_for_data", message: "\uCD94\uC801 \uC911\uC778 \uC7A5\uC911 \uBAA8\uC758\uD22C\uC790 \uAE30\uB85D\uC774 \uC544\uC9C1 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const positions = await db.select({ symbol: dayTradeExperimentPositions.symbol, name: dayTradeExperimentPositions.name }).from(dayTradeExperimentPositions).where((0, import_drizzle_orm36.eq)(dayTradeExperimentPositions.experimentId, experiment.id));
  ...
  app2.get("/api/local-research-node/daily-bar-collection-plan", async (request, response) => {
    ...
    const db = await getDb();
    if (!db) return response.status(503).json({ status: "unavailable", message: "\uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const runs = await db.select({ universeJson: autonomousResearchRuns.universeJson }).from(autonomousResearchRuns).where((0, import_drizzle_orm36.eq)(autonomousResearchRuns.dataStatus, "ready")).orderBy((0, import_drizzle_orm36.desc)(autonomousResearchRuns.updatedAt)).limit(40);
  ...
  app2.get("/api/local-research-node/shared-dataset-collection-plan", async (request, response) => {
    ...
    const db = await getDb();
    if (!db) return response.status(503).json({ status: "unavailable", message: "\uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const queued = (await db.select().from(sharedDatasetCollectionRequests).where((0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.status, "queued")).orderBy((0, import_drizzle_orm36.desc)(sharedDatasetCollectionRequests.requestedAt)).limit(1))[0];
    if (!queued) return response.status(409).json({ status: "idle", message: "\uC5F0\uACB0 \uC2DC \uCC98\uB9AC\uD560 \uACF5\uC6A9 \uB370\uC774\uD130\uC14B \uC218\uC9D1 \uC694\uCCAD\uC774 \uC5C6\uC2B5\uB2C8\uB2E4." });
    await db.update(sharedDatasetCollectionRequests).set({ status: "running", startedAt: /* @__PURE__ */ new Date(), lastError: null, progressJson: { stage: "accepted", message: "\uC9C0\uC815 \uB2E8\uB9D0 \uC218\uC9D1\uAE30\uAC00 \uC694\uCCAD\uC744 \uC811\uC218\uD588\uC2B5\uB2C8\uB2E4.", totalSymbols: queued.symbolCount, updatedAt: (/* @__PURE__ */ new Date()).toISOString() } }).where((0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.id, queued.id));
  ...
  app2.post("/api/local-research-node/shared-dataset-collection-sync", async (request, response) => {
    ...
    const db = await getDb();
    if (!db) return response.status(503).json({ status: "unavailable", message: "\uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const collection = (await db.select().from(sharedDatasetCollectionRequests).where((0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.id, requestId)).limit(1))[0];
    ...
    const sourceFingerprint = (0, import_node_crypto9.createHash)("sha256").update(JSON.stringify({ source: ["kiwoom_ka10081", "kiwoom_ka10080"], universe, window: selected.window, daily: selected.selectedDailyBars, fiveMinute: selected.selectedFiveMinuteBars.map((bar) => ({ ...bar, intervalAt: bar.intervalAt.toISOString() })) })).digest("hex");
    const existing = (await db.select().from(researchDatasets).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(researchDatasets.sourceFingerprint, sourceFingerprint), (0, import_drizzle_orm36.eq)(researchDatasets.visibility, "shared_public"), (0, import_drizzle_orm36.eq)(researchDatasets.qualityStatus, "ready"))).limit(1))[0];
    if (existing) {
      await db.update(sharedDatasetCollectionRequests).set({ status: "completed", datasetId: existing.id, plannedUniverseJson: universe, acceptedDailyBarCount: existing.barCount, acceptedFiveMinuteBarCount: existing.minuteBarCount, progressJson: { stage: "completed", message: "\uAC19\uC740 \uC6D0\uBCF8\uC744 \uCC3E\uC544 \uAE30\uC874 \uACF5\uC6A9 \uB370\uC774\uD130\uC14B\uC744 \uC7AC\uC0AC\uC6A9\uD588\uC2B5\uB2C8\uB2E4.", totalSymbols: universe.length, completedDailySymbols: universe.length, completedFiveMinuteSymbols: universe.length, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.id, collection.id));
    ...
    try {
      ...
      await db.update(researchDatasets).set({ qualityStatus: "error", qualityReportJson: { state: "error", sourceFingerprint, error: message } }).where((0, import_drizzle_orm36.eq)(researchDatasets.id, created.id));
      await db.update(sharedDatasetCollectionRequests).set({ status: "failed", lastError: message, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.id, collection.id));
  ...
  app2.post("/api/local-research-node/shared-dataset-collection-stream-start", async (request, response) => {
    ...
    if (!collection || collection.status !== "running") return response.status(409).json({ status: "invalid_request_state", message: "\uC2E4\uD589 \uC911\uC778 \uACF5\uC6A9 \uB370\uC774\uD130\uC14B \uC218\uC9D1 \uC694\uCCAD\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    if (new Set(universe.map((item) => item.symbol)).size !== universe.length || universe.length !== collection.symbolCount) return response.status(400).json({ status: "invalid_universe", message: "\uC218\uC9D1 \uC694\uCCAD\uC758 \uC885\uBAA9 \uC218\uC640 \uB3D9\uAE30\uD654 \uC720\uB2C8\uBC84\uC2A4\uAC00 \uC77C\uCE58\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4." });
    const existingReady = (await db.select().from(researchDatasets).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(researchDatasets.sourceFingerprint, sourceFingerprint), (0, import_drizzle_orm36.eq)(researchDatasets.visibility, "shared_public"), (0, import_drizzle_orm36.eq)(researchDatasets.qualityStatus, "ready"))).limit(1))[0];
    if (existingReady) {
    ...
    const attached = collection.datasetId ? (await db.select().from(researchDatasets).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(researchDatasets.id, collection.datasetId), (0, import_drizzle_orm36.eq)(researchDatasets.qualityStatus, "collecting"))).limit(1))[0] : null;
  ...
  app2.post("/api/local-research-node/shared-dataset-collection-stream-chunk", async (request, response) => {
    ...
    const db = await getDb();
    if (!db) return response.status(503).json({ status: "unavailable", message: "\uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const collection = (await db.select().from(sharedDatasetCollectionRequests).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.id, requestId), (0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.datasetId, datasetId), (0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.status, "running"))).limit(1))[0];
    const dataset = (await db.select().from(researchDatasets).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(researchDatasets.id, datasetId), (0, import_drizzle_orm36.eq)(researchDatasets.qualityStatus, "collecting"))).limit(1))[0];
    ...
    if (kind === "daily") {
      ...
      await db.insert(researchDailyBars).values(bars).onConflictDoUpdate({
        ...
        set: { open: import_drizzle_orm36.sql`excluded.open`, high: import_drizzle_orm36.sql`excluded.high`, low: import_drizzle_orm36.sql`excluded.low`, close: import_drizzle_orm36.sql`excluded.close`, volume: import_drizzle_orm36.sql`excluded.volume`, turnover: import_drizzle_orm36.sql`excluded.turnover`, source: import_drizzle_orm36.sql`excluded.source` }
      });
    } else {
      const bars = submitted.flatMap((raw) => {
        const item = raw;
        ...
        const rawFingerprint = (0, import_node_crypto9.createHash)("sha256").update(JSON.stringify({ symbol, tradingDate: tradingDate2, intervalAt: intervalAt.toISOString(), open, high, low, close, volume })).digest("hex");
      ...
      await db.insert(researchFiveMinuteBars).values(bars).onConflictDoUpdate({
        ...
        set: { open: import_drizzle_orm36.sql`excluded.open`, high: import_drizzle_orm36.sql`excluded.high`, low: import_drizzle_orm36.sql`excluded.low`, close: import_drizzle_orm36.sql`excluded.close`, volume: import_drizzle_orm36.sql`excluded.volume`, source: import_drizzle_orm36.sql`excluded.source`, rawFingerprint: import_drizzle_orm36.sql`excluded.rawFingerprint` }
      });
    }
  ...
  app2.post("/api/local-research-node/shared-dataset-collection-stream-finalize", async (request, response) => {
    ...
    const [daily] = await db.select({ total: (0, import_drizzle_orm36.count)() }).from(researchDailyBars).where((0, import_drizzle_orm36.eq)(researchDailyBars.datasetId, datasetId));
    const [minute] = await db.select({ total: (0, import_drizzle_orm36.count)() }).from(researchFiveMinuteBars).where((0, import_drizzle_orm36.eq)(researchFiveMinuteBars.datasetId, datasetId));
    ...
    const now = /* @__PURE__ */ new Date();
    await db.update(researchDatasets).set({ barCount: dailyBarCount, minuteBarCount: fiveMinuteBarCount, qualityStatus: "ready", readyAt: now, qualityReportJson: { ...report, state: "ready", dailyBarCount, fiveMinuteBarCount, immutable: true, completedAt: now.toISOString() } }).where((0, import_drizzle_orm36.eq)(researchDatasets.id, datasetId));
    await db.update(sharedDatasetCollectionRequests).set({ status: "completed", acceptedDailyBarCount: dailyBarCount, acceptedFiveMinuteBarCount: fiveMinuteBarCount, progressJson: { stage: "completed", message: "\uB300\uC6A9\uB7C9 \uC77C\uBD09\xB75\uBD84\uBD09 \uC6D0\uBCF8\uC744 \uCCAD\uD06C \uAC80\uC99D \uD6C4 \uACF5\uC6A9 \uBCF4\uAD00\uC18C\uC5D0 \uACE0\uC815\uD588\uC2B5\uB2C8\uB2E4.", totalSymbols: collection.symbolCount, completedDailySymbols: collection.symbolCount, completedFiveMinuteSymbols: collection.symbolCount, updatedAt: now.toISOString() }, completedAt: now }).where((0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.id, requestId));
  ...
  app2.post("/api/local-research-node/shared-dataset-collection-progress", async (request, response) => {
    ...
    await db.update(sharedDatasetCollectionRequests).set({ progressJson: { stage, message, totalSymbols, completedDailySymbols, completedFiveMinuteSymbols, updatedAt: (/* @__PURE__ */ new Date()).toISOString() } }).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.id, requestId), (0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.status, "running")));
  ...
  app2.post("/api/local-research-node/shared-dataset-collection-status", async (request, response) => {
    ...
    const db = await getDb();
    if (!db) return response.status(503).json({ status: "unavailable", message: "\uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    await db.update(sharedDatasetCollectionRequests).set({ status: "failed", lastError: message, progressJson: { stage: "failed", message, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }, completedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.id, requestId), (0, import_drizzle_orm36.eq)(sharedDatasetCollectionRequests.status, "running")));
    return response.json({ status: "recorded" });
  });
  app2.get("/api/local-research-node/intraday-minute-backfill-plan", async (request, response) => {
    ...
    const dailyBars = await db.select({ symbol: localResearchDailyBars.symbol, date: localResearchDailyBars.date, turnover: localResearchDailyBars.turnover }).from(localResearchDailyBars).where((0, import_drizzle_orm36.eq)(localResearchDailyBars.adjustmentBasis, "adjusted")).orderBy((0, import_drizzle_orm36.desc)(localResearchDailyBars.date)).limit(2e4);
    const runs = await db.select({ universeJson: autonomousResearchRuns.universeJson }).from(autonomousResearchRuns).where((0, import_drizzle_orm36.eq)(autonomousResearchRuns.dataStatus, "ready")).orderBy((0, import_drizzle_orm36.desc)(autonomousResearchRuns.updatedAt)).limit(40);
  ...
  app2.get("/api/local-research-node/intraday-minute-collection-plan", async (request, response) => {
    ...
    const positions = experiment?.status === "tracking" ? await db.select({ symbol: dayTradeExperimentPositions.symbol, name: dayTradeExperimentPositions.name }).from(dayTradeExperimentPositions).where((0, import_drizzle_orm36.eq)(dayTradeExperimentPositions.experimentId, experiment.id)) : [];
    const runs = await db.select({ universeJson: autonomousResearchRuns.universeJson }).from(autonomousResearchRuns).where((0, import_drizzle_orm36.eq)(autonomousResearchRuns.dataStatus, "ready")).orderBy((0, import_drizzle_orm36.desc)(autonomousResearchRuns.updatedAt)).limit(40);
    const knownNames = selectLocalDailyCollectionUniverse(runs, 120);
    const recentDailyBars = await db.select({ symbol: localResearchDailyBars.symbol, date: localResearchDailyBars.date, turnover: localResearchDailyBars.turnover }).from(localResearchDailyBars).where((0, import_drizzle_orm36.eq)(localResearchDailyBars.adjustmentBasis, "adjusted")).orderBy((0, import_drizzle_orm36.desc)(localResearchDailyBars.date)).limit(2e4);
    ...
    const [requestRow] = await db.select().from(localMinuteCollectionRequests).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(localMinuteCollectionRequests.tradingDate, tradingDate2), (0, import_drizzle_orm36.eq)(localMinuteCollectionRequests.status, "queued"))).orderBy((0, import_drizzle_orm36.desc)(localMinuteCollectionRequests.requestedAt)).limit(1);
    if (requestRow) await db.update(localMinuteCollectionRequests).set({ status: "running", startedAt: /* @__PURE__ */ new Date(), lastSeenAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm36.eq)(localMinuteCollectionRequests.id, requestRow.id));
  ...
  app2.post("/api/local-research-node/intraday-minute-sync", async (request, response) => {
    ...
    await db.insert(intradayMinuteBars).values(values).onConflictDoUpdate({
      ...
      set: { open: import_drizzle_orm36.sql`excluded.open`, high: import_drizzle_orm36.sql`excluded.high`, low: import_drizzle_orm36.sql`excluded.low`, close: import_drizzle_orm36.sql`excluded.close`, volume: import_drizzle_orm36.sql`excluded.volume`, rawFingerprint: import_drizzle_orm36.sql`excluded.rawFingerprint`, capturedAt: import_drizzle_orm36.sql`excluded.capturedAt` }
    });
  ...
  app2.post("/api/local-research-node/daily-bar-sync", async (request, response) => {
    ...
    await db.insert(localResearchDailyBars).values(values).onConflictDoUpdate({
      ...
      set: {
        open: import_drizzle_orm36.sql`excluded.open`,
        high: import_drizzle_orm36.sql`excluded.high`,
        low: import_drizzle_orm36.sql`excluded.low`,
        close: import_drizzle_orm36.sql`excluded.close`,
        volume: import_drizzle_orm36.sql`excluded.volume`,
        turnover: import_drizzle_orm36.sql`excluded.turnover`,
        rawFingerprint: import_drizzle_orm36.sql`excluded.rawFingerprint`,
        capturedAt: import_drizzle_orm36.sql`excluded.capturedAt`
      }
    });
  ...
  app2.post("/api/local-research-node/daily-dataset-promote", async (request, response) => {
    ...
    const db = await getDb();
    if (!db) return response.status(503).json({ status: "unavailable", message: "\uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const bars = await db.select().from(localResearchDailyBars).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.inArray)(localResearchDailyBars.symbol, requestedSymbols), (0, import_drizzle_orm36.eq)(localResearchDailyBars.adjustmentBasis, adjustmentBasis))).orderBy((0, import_drizzle_orm36.asc)(localResearchDailyBars.symbol), (0, import_drizzle_orm36.asc)(localResearchDailyBars.date));
    ...
    const owner = (await db.select().from(users).where((0, import_drizzle_orm36.eq)(users.role, "admin")).orderBy((0, import_drizzle_orm36.asc)(users.id)).limit(1))[0];
    ...
    const existing = (await db.select().from(researchDatasets).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(researchDatasets.userId, owner.id), (0, import_drizzle_orm36.eq)(researchDatasets.versionKey, version.versionKey))).limit(1))[0];
    ...
    try {
      ...
      await db.update(researchDatasets).set({ qualityStatus: "error", qualityReportJson: { state: "error", source: "local_research_daily_bars", sourceFingerprint: version.sourceFingerprint, error: message } }).where((0, import_drizzle_orm36.eq)(researchDatasets.id, datasetId));
      return response.status(500).json({ status: "snapshot_failed", message });
    }
  });
  ...
  app2.post("/api/local-research-node/intraday-minute-collection-status", async (request, response) => {
    ...
    const db = await getDb();
    if (!db) return response.status(503).json({ status: "unavailable", message: "\uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    await db.update(localMinuteCollectionRequests).set({ status, acceptedBarCount, rejectedBarCount, lastError: status === "failed" ? message ?? "1\uBD84\uBD09 \uC218\uC9D1 \uC2E4\uD328" : null, completedAt: /* @__PURE__ */ new Date(), lastSeenAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm36.eq)(localMinuteCollectionRequests.id, requestId));
    return response.json({ status: "recorded" });
  });
  app2.post("/api/local-research-node/intraday-price-sync", async (request, response) => {
    ...
    const db = await getDb();
    if (!db) return response.status(503).json({ status: "unavailable", message: "\uC5F0\uAD6C \uB370\uC774\uD130\uBCA0\uC774\uC2A4\uB97C \uC0AC\uC6A9\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const [experiment] = await db.select().from(dayTradeExperiments).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(dayTradeExperiments.status, "tracking"), (0, import_drizzle_orm36.eq)(dayTradeExperiments.tradingDate, body.tradingDate))).orderBy((0, import_drizzle_orm36.desc)(dayTradeExperiments.updatedAt)).limit(1);
    if (!experiment) return response.status(409).json({ status: "waiting_for_data", message: "\uD574\uB2F9 \uAC70\uB798\uC77C\uC758 \uCD94\uC801 \uC911\uC778 \uC7A5\uC911 \uBAA8\uC758\uD22C\uC790 \uAE30\uB85D\uC774 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const positions = await db.select().from(dayTradeExperimentPositions).where((0, import_drizzle_orm36.eq)(dayTradeExperimentPositions.experimentId, experiment.id));
    ...
    for (const position of positions) {
      ...
      if (!ledger) continue;
      await db.update(dayTradeExperimentPositions).set({ lastPrice: quote?.price ?? position.lastPrice, lastObservedAt: quote?.observedAt ?? position.lastObservedAt, buyFee: ledger.buyFee, estimatedExitFee: ledger.estimatedExitFee, netValue: Math.round(ledger.netValue), netPnl: Math.round(ledger.netPnl), netReturnPercent: ledger.netReturnPercent.toFixed(4) }).where((0, import_drizzle_orm36.eq)(dayTradeExperimentPositions.id, position.id));
    }
    await db.update(dayTradeExperiments).set({ netValue: Math.round(portfolio.netValue), netPnl: Math.round(portfolio.netPnl), netReturnPercent: portfolio.netReturnPercent.toFixed(4) }).where((0, import_drizzle_orm36.eq)(dayTradeExperiments.id, experiment.id));
  ...
  app2.get("/api/local-research-node/auto-order-plan", async (request, response) => {
    ...
    const [policy] = await db.select().from(autoTradePolicies).where((0, import_drizzle_orm36.eq)(autoTradePolicies.status, "active")).orderBy((0, import_drizzle_orm36.desc)(autoTradePolicies.updatedAt)).limit(1);
    if (!policy) return response.status(409).json({ status: "waiting_for_policy", message: "\uD65C\uC131 \uC790\uB3D9 \uC2E4\uD22C \uC815\uCC45\uC774 \uC544\uC9C1 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const profile = (await db.select().from(tradingProfiles).where((0, import_drizzle_orm36.eq)(tradingProfiles.userId, policy.userId)).limit(1))[0];
    ...
    const positions = await db.select().from(dayTradeExperimentPositions).where((0, import_drizzle_orm36.eq)(dayTradeExperimentPositions.experimentId, experiment.id));
    const candidateIds = Array.from(new Set(positions.map((position) => position.candidateId)));
    const candidates = candidateIds.length ? await db.select({ id: autonomousResearchCandidates.id, fitnessScore: autonomousResearchCandidates.fitnessScore }).from(autonomousResearchCandidates).where((0, import_drizzle_orm36.inArray)(autonomousResearchCandidates.id, candidateIds)) : [];
  ...
  app2.post("/api/local-research-node/execution-sync", async (request, response) => {
    ...
    const [policy] = await db.select().from(autoTradePolicies).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(autoTradePolicies.id, policyId), (0, import_drizzle_orm36.eq)(autoTradePolicies.version, policyVersion))).limit(1);
    ...
    for (const raw of body.orders) {
      ...
      let intent = (await db.select().from(orderIntents).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(orderIntents.userId, policy.userId), (0, import_drizzle_orm36.eq)(orderIntents.dedupeKey, dedupeKey))).limit(1))[0];
      if (!intent) {
        ...
        intent = (await db.select().from(orderIntents).where((0, import_drizzle_orm36.eq)(orderIntents.id, created.id)).limit(1))[0];
      } else if (["submitted", "filled"].includes(status) && intent.status !== "filled") {
        await db.update(orderIntents).set({ status, brokerOrderId: typeof item.brokerOrderId === "string" ? item.brokerOrderId : intent.brokerOrderId }).where((0, import_drizzle_orm36.eq)(orderIntents.id, intent.id));
      }
      if (!intent) continue;
      const prior = await db.select({ id: orderExecutions.id }).from(orderExecutions).where((0, import_drizzle_orm36.and)((0, import_drizzle_orm36.eq)(orderExecutions.orderIntentId, intent.id), (0, import_drizzle_orm36.eq)(orderExecutions.executionStatus, status))).limit(1);
