...
var oneClickBacktestRouter = router({
  ...
  run: publicProcedure.input(z19.object({
    ...
    return {
      ...
      results: results.map((r, rank) => ({
        ...
        robustnessScore: Number(((r.averageWinRate >= 50 ? 20 : r.averageWinRate * 0.4) + (r.totalTrades >= 10 ? 15 : r.totalTrades * 1.5) + (Math.abs(r.worstDrawdown) <= 10 ? 20 : Math.max(0, 20 - Math.abs(r.worstDrawdown))) + (r.averageReturn > 0 ? Math.min(25, r.averageReturn * 2) : 0) + (r.fitnessScore > 0 ? Math.min(20, r.fitnessScore * 2) : 0)).toFixed(1)),
   ...
   */
  dataQuality: publicProcedure.input(z19.object({ symbol: z19.string().regex(/^\d{6}$/) }).optional()).query(async ({ input }) => {
    const db = await getDb();
    if (!db) throw new TRPCError17({ code: "INTERNAL_SERVER_ERROR", message: "DB \uC5F0\uACB0 \uBD88\uAC00" });
    const conditions = [eq31(localResearchDailyBars.adjustmentBasis, "adjusted")];
    if (input?.symbol) conditions.push(eq31(localResearchDailyBars.symbol, input.symbol));
    const rows = await db.select({
      ...
      close: localResearchDailyBars.close,
      volume: localResearchDailyBars.volume
    }).from(localResearchDailyBars).where(and21(...conditions)).orderBy(asc7(localResearchDailyBars.date)).limit(5e3);
    const issues = [];
   ...
   */
  monteCarloValidation: publicProcedure.input(z19.object({
    root: z19.unknown(),
    minimumScore: z19.number().int().min(0).max(200),
    simulations: z19.number().int().min(100).max(2e3).default(500),
    holdingDays: z19.number().int().min(1).max(60).default(5),
    stopLossPercent: z19.number().min(0).max(20).default(3),
    takeProfitPercent: z19.number().min(0).max(50).default(5)
  })).mutation(async ({ input }) => {
    ...
    if (!db) throw new TRPCError17({ code: "INTERNAL_SERVER_ERROR", message: "DB \uC5F0\uACB0 \uBD88\uAC00" });
    const feeRate = 3e-4 + 8 / 1e4;
    const allSymbols = await db.selectDistinct({ symbol: localResearchDailyBars.symbol }).from(localResearchDailyBars).where(eq31(localResearchDailyBars.adjustmentBasis, "adjusted")).limit(20);
    const allTrades = [];
    for (const { symbol } of allSymbols.slice(0, 5)) {
      const rows = await db.select({ date: localResearchDailyBars.date, open: localResearchDailyBars.open, high: localResearchDailyBars.high, low: localResearchDailyBars.low, close: localResearchDailyBars.close, volume: localResearchDailyBars.volume, turnover: localResearchDailyBars.turnover }).from(localResearchDailyBars).where(and21(eq31(localResearchDailyBars.symbol, symbol), eq31(localResearchDailyBars.adjustmentBasis, "adjusted"))).orderBy(asc7(localResearchDailyBars.date)).limit(600);
      if (rows.length < 60) continue;
      const bars = rows.map((r) => ({ date: r.date, open: r.open, high: r.high, low: r.low, close: r.close, volume: Number(r.volume), turnover: Number(r.turnover) }));
      const result = runDailyBacktest({ bars, expression: input.root, minScore: input.minimumScore, holdingDays: input.holdingDays, feeRate, entryDelayDays: 1, entryTiming: "open", maxOpenGapPercent: 3, stopLossPercent: input.stopLossPercent, takeProfitPercent: input.takeProfitPercent });
      for (const t2 of result.trades) allTrades.push({ returnPercent: t2.returnPercent });
    }
    if (allTrades.length < 5) throw new TRPCError17({ code: "PRECONDITION_FAILED", message: `Monte Carlo\uC5D0 \uD544\uC694\uD55C \uAC70\uB798\uAC00 \uBD80\uC871\uD569\uB2C8\uB2E4 (${allTrades.length}\uAC74). \uCD5C\uC18C 5\uAC74 \uC774\uC0C1 \uD544\uC694.` });
    const simResults = [];
    ...
    const mean4 = (arr) => arr.reduce((s, v) => s + v, 0) / arr.length;
    const bankruptRate = equities.filter((e) => e < 50).length / equities.length * 100;
    return {
      originalTrades: allTrades.length,
      simulations: input.simulations,
      statistics: {
        meanFinalEquity: Number(mean4(equities).toFixed(2)),
        ...
        meanMaxDrawdown: Number(mean4(drawdowns).toFixed(2)),
        worstMaxDrawdown: Number(Math.min(...drawdowns).toFixed(2))
      },
      distribution: {
        equities: equities.slice(0, 100).map((e) => Number(e.toFixed(1))),
        // 차트용 100개 샘플
   ...
   */
  updateStrategyStatus: protectedProcedure.input(z19.object({
    presetId: z19.number().int().positive(),
    status: z19.enum(["candidate", "testing", "survivor", "rejected", "retired"])
  })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new TRPCError17({ code: "INTERNAL_SERVER_ERROR", message: "DB \uC5F0\uACB0 \uBD88\uAC00" });
    const [preset] = await db.select().from(strategyPresets).where(and21(eq31(strategyPresets.id, input.presetId), eq31(strategyPresets.userId, ctx.user.id))).limit(1);
    if (!preset) throw new TRPCError17({ code: "NOT_FOUND", message: "\uC804\uB7B5\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." });
    const currentScoring = preset.scoringJson ?? {};
    const updatedScoring = { ...currentScoring, lifecycleStatus: input.status, lifecycleUpdatedAt: (/* @__PURE__ */ new Date()).toISOString() };
    await db.update(strategyPresets).set({ scoringJson: updatedScoring }).where(eq31(strategyPresets.id, input.presetId));
    const labels = { candidate: "\uD6C4\uBCF4", testing: "\uAC80\uC99D \uC911", survivor: "\uC0DD\uC874", rejected: "\uD0C8\uB77D", retired: "\uC740\uD1F4" };
...
var mockTradingRouter = router({
  ...
  checkAndTriggerSafety: publicProcedure.mutation(async () => {
    ...
    const today = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Seoul" }).format(/* @__PURE__ */ new Date());
    const todayStart = /* @__PURE__ */ new Date(today + "T00:00:00+09:00");
    const todaySells = await db.select({ symbol: orderIntents.symbol, price: orderIntents.price, quantity: orderIntents.quantity }).from(orderIntents).where(and22(eq32(orderIntents.executionOrigin, "local_node"), eq32(orderIntents.status, "filled"), eq32(orderIntents.side, "sell"), gte5(orderIntents.createdAt, todayStart)));
    let realizedPnl = 0;
    if (todaySells.length > 0) {
      const sellSymbols = Array.from(new Set(todaySells.map((o) => o.symbol)));
      const buyHistory = await db.select({ symbol: orderIntents.symbol, price: orderIntents.price, quantity: orderIntents.quantity }).from(orderIntents).where(and22(eq32(orderIntents.executionOrigin, "local_node"), eq32(orderIntents.status, "filled"), eq32(orderIntents.side, "buy"), inArray9(orderIntents.symbol, sellSymbols)));
      const avgBuyBySymbol = /* @__PURE__ */ new Map();
      const grouped = /* @__PURE__ */ new Map();
      for (const b of buyHistory) {
        const list = grouped.get(b.symbol) ?? [];
      ...
      for (const sell of todaySells) {
        realizedPnl += (sell.price - (avgBuyBySymbol.get(sell.symbol) ?? sell.price)) * sell.quantity;
      }
    }
    const realizedPnlPercent = policy.totalCapital > 0 ? realizedPnl / Number(policy.totalCapital) * 100 : 0;
    const dailyLossLimit = Number(policy.dailyLossLimitPercent);
...
var performanceTrackerRouter = router({
  ...
  summary: publicProcedure.query(async () => {
    ...
    const totalGrossProfit = roundTrips.filter((t2) => t2.returnPercent > 0).reduce((s, t2) => s + (t2.sellPrice - t2.buyPrice) * t2.quantity, 0);
    const totalGrossLoss = Math.abs(roundTrips.filter((t2) => t2.returnPercent <= 0).reduce((s, t2) => s + (t2.sellPrice - t2.buyPrice) * t2.quantity, 0));
    return {
