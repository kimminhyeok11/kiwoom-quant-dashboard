...
import { asc as asc6, and as and20, eq as eq29, desc as desc23, gte, lte, inArray as inArray7, sql as sql4 } from "drizzle-orm";
...
var chartDataRouter = router({
  ...
  minuteBars: publicProcedure.input(z18.object({
    ...
    const db = await getDb();
    if (!db) throw new TRPCError17({ code: "INTERNAL_SERVER_ERROR", message: "DB ?\uFFFD\uACB0 \uBD88\uFFFD?" });
    let query;
  ...
  dailyBars: publicProcedure.input(z18.object({
    ...
    const conditions = [
      ...
      sql4`${localResearchDailyBars.adjustmentBasis}::text = 'adjusted'`
    ];
  ...
  availableSymbols: publicProcedure.query(async () => {
    const db = await getDb();
    if (!db) throw new TRPCError17({ code: "INTERNAL_SERVER_ERROR", message: "DB ?\uFFFD\uACB0 \uBD88\uFFFD?" });
    const daily = await db.selectDistinct({ symbol: localResearchDailyBars.symbol }).from(localResearchDailyBars).where(sql4`${localResearchDailyBars.adjustmentBasis}::text = 'adjusted'`).limit(100);
...
import { and as and26, asc as asc9, count as count2, desc as desc29, eq as eq36, inArray as inArray9, like as like6, ne, sql as sql5 } from "drizzle-orm";
...
function registerLocalResearchNodeRoutes(app2) {
  ...
  app2.post("/api/local-research-node/shared-dataset-collection-stream-chunk", async (request, response) => {
    ...
    if (kind === "daily") {
      ...
      await db.insert(researchDailyBars).values(bars).onConflictDoUpdate({
        ...
        set: { open: sql5`excluded.open`, high: sql5`excluded.high`, low: sql5`excluded.low`, close: sql5`excluded.close`, volume: sql5`excluded.volume`, turnover: sql5`excluded.turnover`, source: sql5`excluded.source` }
      });
    } else {
      const bars = submitted.flatMap((raw) => {
        const item = raw;
      ...
      await db.insert(researchFiveMinuteBars).values(bars).onConflictDoUpdate({
        ...
        set: { open: sql5`excluded.open`, high: sql5`excluded.high`, low: sql5`excluded.low`, close: sql5`excluded.close`, volume: sql5`excluded.volume`, source: sql5`excluded.source`, rawFingerprint: sql5`excluded.rawFingerprint` }
      });
    }
  ...
  app2.post("/api/local-research-node/intraday-minute-sync", async (request, response) => {
    ...
    await db.insert(intradayMinuteBars).values(values).onConflictDoUpdate({
      ...
      set: { open: sql5`excluded.open`, high: sql5`excluded.high`, low: sql5`excluded.low`, close: sql5`excluded.close`, volume: sql5`excluded.volume`, rawFingerprint: sql5`excluded.rawFingerprint`, capturedAt: sql5`excluded.capturedAt` }
    });
  ...
  app2.post("/api/local-research-node/daily-bar-sync", async (request, response) => {
    ...
    await db.insert(localResearchDailyBars).values(values).onConflictDoUpdate({
      ...
      set: {
        open: sql5`excluded.open`,
        high: sql5`excluded.high`,
        low: sql5`excluded.low`,
        close: sql5`excluded.close`,
        volume: sql5`excluded.volume`,
        turnover: sql5`excluded.turnover`,
        rawFingerprint: sql5`excluded.rawFingerprint`,
        capturedAt: sql5`excluded.capturedAt`
      }
    });
